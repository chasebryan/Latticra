...
int main(void) {
    ...
    char processes_contract_report[LATTICRA_CONSOLE_PROCESSES_CONTRACT_REPORT_MAX];
    ...
    failures += require_int("result.packages_contract_present", result.packages_contract_present, 1);
    failures += require_int("result.init_contract_present", result.init_contract_present, 1);
    failures += require_int("result.services_contract_present", result.services_contract_present, 1);
    failures += require_int("result.service_schema_contract_present", result.service_schema_contract_present, 1);
    failures += require_int(
    ...
    failures += require_int("result.processes_contract_present", result.processes_contract_present, 1);
    ...
    command = latticra_console_find_command("lc packages");
    failures += require_int("find lc packages", command != 0, 1);
    if (command != 0) {
        failures += require_text(
            "lc packages capability",
            command->capability_label,
            "lc.packages.contract");
        failures += require_int("lc packages no_effect", command->no_effect, 1);
        failures += require_int("lc packages host launch", command->launches_host_process, 0);
    }

    command = latticra_console_find_command("lc init");
    failures += require_int("find lc init", command != 0, 1);
    if (command != 0) {
        failures += require_text(
            "lc init capability",
            command->capability_label,
            "lc.init.contract");
        failures += require_int("lc init no_effect", command->no_effect, 1);
        failures += require_int("lc init host launch", command->launches_host_process, 0);
    }

    command = latticra_console_find_command("lc services");
    failures += require_int("find lc services", command != 0, 1);
    if (command != 0) {
        failures += require_text(
            "lc services capability",
            command->capability_label,
            "lc.services.contract");
        failures += require_int("lc services no_effect", command->no_effect, 1);
        failures += require_int("lc services host launch", command->launches_host_process, 0);
    }

    command = latticra_console_find_command("lc service-schema");
    failures += require_int("find lc service-schema", command != 0, 1);
    if (command != 0) {
        failures += require_text(
            "lc service-schema capability",
            command->capability_label,
            "lc.service.schema.contract");
        failures += require_int("lc service-schema no_effect", command->no_effect, 1);
        failures += require_int(
            "lc service-schema host launch",
            command->launches_host_process,
            0);
    }

    command = latticra_console_find_command("lc service-definitions");
    failures += require_int("find lc service-definitions", command != 0, 1);
    if (command != 0) {
        failures += require_text(
            "lc service-definitions capability",
            command->capability_label,
            "lc.service.definitions.contract");
        failures += require_int("lc service-definitions no_effect", command->no_effect, 1);
        failures += require_int(
            "lc service-definitions host launch",
            command->launches_host_process,
            0);
    }

    command = latticra_console_find_command("lc service-plan");
    failures += require_int("find lc service-plan", command != 0, 1);
    if (command != 0) {
        failures += require_text(
            "lc service-plan capability",
            command->capability_label,
            "lc.service.plan.contract");
        failures += require_int("lc service-plan no_effect", command->no_effect, 1);
    ...
    command = latticra_console_find_command("lc processes");
    failures += require_int("find lc processes", command != 0, 1);
    if (command != 0) {
        failures += require_text(
            "lc processes capability",
            command->capability_label,
            "lc.processes.contract");
        failures += require_int("lc processes no_effect", command->no_effect, 1);
        failures += require_int("lc processes host launch", command->launches_host_process, 0);
    }
    ...
    failures += require_contains("report", report, "packages_contract_status=metadata-only-contract-ready");
    failures += require_contains("report", report, "packages_contract_present=1");
    failures += require_contains("report", report, "init_contract_status=metadata-only-contract-ready");
    failures += require_contains("report", report, "init_contract_present=1");
    failures += require_contains("report", report, "services_contract_status=metadata-only-contract-ready");
    failures += require_contains("report", report, "services_contract_present=1");
    failures += require_contains(
        "report",
        report,
        "service_schema_contract_status=metadata-only-contract-ready");
    failures += require_contains("report", report, "service_schema_contract_present=1");
    failures += require_contains(
        "report",
        report,
        "service_definitions_contract_status=metadata-only-contract-ready");
    failures += require_contains("report", report, "service_definitions_contract_present=1");
    failures += require_contains(
        "report",
        report,
    ...
    failures += require_contains(
        "report",
        report,
        "processes_contract_status=metadata-only-contract-ready");
    failures += require_contains("report", report, "processes_contract_present=1");
    failures += require_contains("report", report, "workspace_contract_status=metadata-only-contract-ready");
    failures += require_contains("report", report, "workspace_contract_present=1");
    ...
    failures += require_contains("registry_report", registry_report, "command=lc packages");
    failures += require_contains("registry_report", registry_report, "capability=lc.packages.contract");
    failures += require_contains("registry_report", registry_report, "command=lc init");
    failures += require_contains("registry_report", registry_report, "capability=lc.init.contract");
    failures += require_contains("registry_report", registry_report, "command=lc services");
    failures += require_contains("registry_report", registry_report, "capability=lc.services.contract");
    failures += require_contains("registry_report", registry_report, "command=lc service-schema");
    failures += require_contains(
        "registry_report",
        registry_report,
        "capability=lc.service.schema.contract");
    failures += require_contains("registry_report", registry_report, "command=lc service-definitions");
    failures += require_contains(
        "registry_report",
        registry_report,
        "capability=lc.service.definitions.contract");
    failures += require_contains("registry_report", registry_report, "command=lc service-plan");
    failures += require_contains(
        "registry_report",
        registry_report,
    ...
    failures += require_contains("registry_report", registry_report, "command=lc processes");
    failures += require_contains("registry_report", registry_report, "capability=lc.processes.contract");
    ...
    failures += require_contains("help_report", help_report, "lc packages");
    failures += require_contains("help_report", help_report, "lc init");
    failures += require_contains("help_report", help_report, "lc services");
    ...
    failures += require_contains("help_report", help_report, "lc processes");
    ...
    failures += require_contains("manpage_report", manpage_report, "latticra-lc services");
    ...
    failures += require_contains("manpage_report", manpage_report, "latticra-lc processes");
    ...
    failures += require_contains("manpage_report", manpage_report, "latticra-lc packages");
    failures += require_contains("manpage_report", manpage_report, "latticra-lc init");
    ...
    command = latticra_console_find_command("lc packages");
    failures += require_int(
        "lc packages boundary",
        latticra_console_command_boundary_classify(command, &boundary),
        LATTICRA_STATUS_OK);
    failures += require_text(
        "lc packages seal capability",
        boundary.seal_capability_label,
        "seal.capability.report");
    failures += require_int(
        "lc packages runtime kind",
        boundary.runtime_request_kind,
        LATTICRA_RUNTIME_BOUNDARY_AUTHORITY_CHECK);
    failures += require_int("lc packages boundary future gate", boundary.requires_future_gate, 0);
    failures += require_int("lc packages boundary no effect", boundary.no_effect, 1);
    failures += require_int("lc packages boundary host mutation allowed", boundary.host_mutation_allowed, 0);
    ...
    failures += require_int(
        ...
        LATTICRA_STATUS_OK);
    failures += require_text(
        "lc init seal capability",
        boundary.seal_capability_label,
        "seal.capability.report");
    failures += require_int(
        "lc init runtime kind",
        boundary.runtime_request_kind,
        LATTICRA_RUNTIME_BOUNDARY_AUTHORITY_CHECK);
    failures += require_int("lc init boundary future gate", boundary.requires_future_gate, 0);
    failures += require_int("lc init boundary no effect", boundary.no_effect, 1);
    failures += require_int("lc init boundary host mutation allowed", boundary.host_mutation_allowed, 0);

    command = latticra_console_find_command("lc services");
    failures += require_int(
        "lc services boundary",
        latticra_console_command_boundary_classify(command, &boundary),
        LATTICRA_STATUS_OK);
    failures += require_text(
        "lc services seal capability",
        boundary.seal_capability_label,
        "seal.capability.report");
    failures += require_int(
        "lc services runtime kind",
        boundary.runtime_request_kind,
        LATTICRA_RUNTIME_BOUNDARY_AUTHORITY_CHECK);
    failures += require_int("lc services boundary future gate", boundary.requires_future_gate, 0);
    failures += require_int("lc services boundary no effect", boundary.no_effect, 1);
    failures += require_int("lc services boundary host mutation allowed", boundary.host_mutation_allowed, 0);

    command = latticra_console_find_command("lc service-schema");
    failures += require_int(
        "lc service-schema boundary",
        latticra_console_command_boundary_classify(command, &boundary),
        LATTICRA_STATUS_OK);
    failures += require_text(
        "lc service-schema seal capability",
        boundary.seal_capability_label,
        "seal.capability.report");
    failures += require_int(
        "lc service-schema runtime kind",
        boundary.runtime_request_kind,
        LATTICRA_RUNTIME_BOUNDARY_AUTHORITY_CHECK);
    failures += require_int("lc service-schema boundary future gate", boundary.requires_future_gate, 0);
    failures += require_int("lc service-schema boundary no effect", boundary.no_effect, 1);
    failures += require_int(
        "lc service-schema boundary host mutation allowed",
        boundary.host_mutation_allowed,
        0);

    command = latticra_console_find_command("lc service-definitions");
    failures += require_int(
        "lc service-definitions boundary",
        latticra_console_command_boundary_classify(command, &boundary),
        LATTICRA_STATUS_OK);
    failures += require_text(
        "lc service-definitions seal capability",
        boundary.seal_capability_label,
        "seal.capability.report");
    failures += require_int(
        "lc service-definitions runtime kind",
        boundary.runtime_request_kind,
        LATTICRA_RUNTIME_BOUNDARY_AUTHORITY_CHECK);
    failures += require_int("lc service-definitions boundary future gate", boundary.requires_future_gate, 0);
    failures += require_int("lc service-definitions boundary no effect", boundary.no_effect, 1);
    failures += require_int(
        "lc service-definitions boundary host mutation allowed",
        boundary.host_mutation_allowed,
        0);

    command = latticra_console_find_command("lc service-plan");
    failures += require_int(
        "lc service-plan boundary",
        latticra_console_command_boundary_classify(command, &boundary),
        LATTICRA_STATUS_OK);
    failures += require_text(
        "lc service-plan seal capability",
        boundary.seal_capability_label,
        "seal.capability.report");
    failures += require_int(
        "lc service-plan runtime kind",
        boundary.runtime_request_kind,
        LATTICRA_RUNTIME_BOUNDARY_AUTHORITY_CHECK);
    failures += require_int("lc service-plan boundary future gate", boundary.requires_future_gate, 0);
    ...
    command = latticra_console_find_command("lc processes");
    failures += require_int(
        "lc processes boundary",
        latticra_console_command_boundary_classify(command, &boundary),
        LATTICRA_STATUS_OK);
    failures += require_text(
        "lc processes seal capability",
        boundary.seal_capability_label,
        "seal.capability.report");
    failures += require_int(
        "lc processes runtime kind",
        boundary.runtime_request_kind,
        LATTICRA_RUNTIME_BOUNDARY_AUTHORITY_CHECK);
    failures += require_int("lc processes boundary future gate", boundary.requires_future_gate, 0);
    failures += require_int("lc processes boundary no effect", boundary.no_effect, 1);
    failures += require_int(
        "lc processes boundary host mutation allowed",
        boundary.host_mutation_allowed,
        0);
    ...
    failures += require_contains("boundary_report", boundary_report, "command=lc packages");
    failures += require_contains("boundary_report", boundary_report, "command=lc init");
    failures += require_contains("boundary_report", boundary_report, "command=lc services");
    ...
    failures += require_contains("boundary_report", boundary_report, "command=lc processes");
    ...
    failures += require_int(
        ...
        latticra_console_packages_contract_report(packages_contract_report, sizeof(packages_contract_report)),
        LATTICRA_STATUS_OK);
    failures += require_contains(
        "packages_contract_report",
        packages_contract_report,
    ...
    failures += require_int(
        ...
        latticra_console_init_contract_report(init_contract_report, sizeof(init_contract_report)),
        LATTICRA_STATUS_OK);
    failures += require_contains(
        "init_contract_report",
        init_contract_report,
    ...
    failures += require_int(
        ...
        latticra_console_services_contract_report(services_contract_report, sizeof(services_contract_report)),
        LATTICRA_STATUS_OK);
    failures += require_contains(
        "services_contract_report",
        services_contract_report,
    ...
    failures += require_int(
        ...
        latticra_console_service_schema_contract_report(
            service_schema_contract_report,
            sizeof(service_schema_contract_report)),
        LATTICRA_STATUS_OK);
    failures += require_contains(
        "service_schema_contract_report",
        service_schema_contract_report,
    ...
    failures += require_int(
        ...
        latticra_console_service_definitions_contract_report(
            service_definitions_contract_report,
            sizeof(service_definitions_contract_report)),
        LATTICRA_STATUS_OK);
    failures += require_contains(
        "service_definitions_contract_report",
        service_definitions_contract_report,
    ...
    failures += require_int(
        ...
        latticra_console_service_plan_contract_report(
            service_plan_contract_report,
            sizeof(service_plan_contract_report)),
        LATTICRA_STATUS_OK);
    failures += require_contains(
        "service_plan_contract_report",
        service_plan_contract_report,
    ...
    failures += require_int(
        "processes_contract_report",
        latticra_console_processes_contract_report(
            processes_contract_report,
            sizeof(processes_contract_report)),
        LATTICRA_STATUS_OK);
    failures += require_contains(
        "processes_contract_report",
        processes_contract_report,
