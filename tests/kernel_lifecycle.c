...
static int default_request_is_denied(void) {
    ...
    EXPECT_TRUE(request.target_state == LATTICRA_KERNEL_STATE_RUN_QUEUE_READY,
        "default target run-queue-ready");
...
static int allowed_lifecycle_reaches_run_queue_ready(void) {
    latticra_kernel_lifecycle_request_t request;
    latticra_kernel_lifecycle_result_t result;
    ...
    EXPECT_TRUE(strcmp(result.policy_status, "gate-allowed") == 0,
        "policy gate allowed");
    EXPECT_TRUE(result.final_state == LATTICRA_KERNEL_STATE_RUN_QUEUE_READY,
        "final state run-queue-ready");
    EXPECT_TRUE(result.step_count == 14u,
        "fourteen steps to run-queue-ready");
    EXPECT_TRUE(result.state_change_count == 14u,
        "fourteen state changes");
    EXPECT_TRUE(result.lifecycle_complete == 1,
        "complete flag set");
    EXPECT_TRUE(result.external_effect_performed == 0,
        "external effects absent");
    EXPECT_TRUE(result.machine.log_count == 14u,
        "machine log has fourteen entries");
...
static int lifecycle_report_is_deterministic(void) {
    ...
    EXPECT_TRUE(strstr(report, "final_state=run-queue-ready\n") != 0,
        "final state emitted");
    EXPECT_TRUE(strstr(report, "step_count=14\n") != 0,
        "step count emitted");
    EXPECT_TRUE(strstr(report, "state_change_count=14\n") != 0,
    ...
    EXPECT_TRUE(strstr(report, "machine_log_count=14\n") != 0,
...
int main(void) {
    ...
    if (allowed_lifecycle_reaches_run_queue_ready() != 0) return 1;
