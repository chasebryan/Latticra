...
static latticra_seal_policy_decision_t unknown_tool_policy_fixture(void) {
    ...
    (void)snprintf(policy_decision.requested_tool, sizeof(policy_decision.requested_tool), "%s", "unknown-tool");
    ...
    (void)snprintf(policy_decision.reason, sizeof(policy_decision.reason), "%s", "unknown-tool-case");
...
static int unknown_tool_case_remains_blocked(void) {
    latticra_seal_policy_decision_t policy_decision = unknown_tool_policy_fixture();
    ...
    EXPECT_TRUE(strstr(rendered, "unknown_tool_blocked=1") != 0, "render unknown tool blocked");
...
int main(void) {
    if (unknown_tool_case_remains_blocked() != 0) {
        return 1;
    }
    printf("seal unknown tool case: ok\n");
    return 0;
}
