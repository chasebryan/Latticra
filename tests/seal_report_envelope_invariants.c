...
static int envelope_allows_report_only_metadata(void) {
    ...
    EXPECT_TRUE(envelope.error == LATTICRA_SEAL_REPORT_ENVELOPE_OK, "report envelope ok");
    ...
    EXPECT_TRUE(envelope.verified == 1u, "verified");
    EXPECT_TRUE(envelope.authority_usable == 0u, "authority usable");
    EXPECT_TRUE(envelope.receipt_capability_gate_allowed == 0u, "receipt capability flag");
    EXPECT_TRUE(envelope.gate_allowed == 1u, "gate allowed");
    EXPECT_TRUE(strcmp(envelope.gate_state, "allowed-metadata-only") == 0, "gate state");
    EXPECT_TRUE(strcmp(envelope.decision_state, "allowed-report-only") == 0, "decision state");
    EXPECT_TRUE(envelope.effect_allowed == 1u, "effect allowed");
    EXPECT_TRUE(strcmp(envelope.handoff_state, "eligible-report-only") == 0, "handoff state");
    EXPECT_TRUE(envelope.handoff_eligible == 1u, "handoff eligible");
    ...
    EXPECT_TRUE(strstr(rendered, "signature_performed=0") != 0, "render signature");
    EXPECT_TRUE(strstr(rendered, "handoff_performed=0") != 0, "render handoff");
...
static int envelope_allows_evaluate_only_metadata(void) {
    ...
    EXPECT_TRUE(envelope.signature_performed == 0u, "evaluate signature");
    EXPECT_TRUE(envelope.handoff_performed == 0u, "evaluate handoff");
    EXPECT_TRUE(envelope.effect_performed == 0u, "evaluate effect");
...
static int envelope_fails_closed(void) {
    ...
    EXPECT_TRUE(latticra_seal_report_envelope_from_report(&report, "report-only", &envelope) == LATTICRA_STATUS_OK, "report flag status");
    EXPECT_TRUE(envelope.error == LATTICRA_SEAL_REPORT_ENVELOPE_DENIED_REPORT, "report flag error");
    report = fixture_report("report-only");
