...
static int default_request_is_stable(void) {
    ...
    EXPECT_TRUE(
        ...
        "default recovery audit review disposition observation view count");
    EXPECT_TRUE(request.runtime_entry_recovery_audit_review_observation_view_request.
...
static int recovery_audit_review_disposition_observation_view_is_metadata_only(void) {
    latticra_kernel_runtime_entry_recovery_audit_review_disposition_observation_view_request_t
        request;
    ...
    EXPECT_TRUE(
        ...
        "recovery audit review disposition observation view evaluates");
    EXPECT_TRUE(strcmp(result.recovery_audit_review_disposition_observation_view_status,
    ...
    EXPECT_TRUE(strcmp(result.runtime_entry_recovery_audit_review_observation_view.
    ...
    EXPECT_TRUE(result.recovery_audit_review_disposition_observation_view_count == 4u,
        "recovery audit review disposition observation view count");
    ...
    EXPECT_TRUE(result.host_effect_allowed == 0,
        "host effect denied");
    EXPECT_TRUE(result.evidence_level == 51u, "result evidence level");

    EXPECT_TRUE(strcmp(result.recovery_audit_review_disposition_observation_views[0].
            process_label, "kernel-report-process-metadata") == 0,
        "entry zero process");
    EXPECT_TRUE(strcmp(result.recovery_audit_review_disposition_observation_views[0].
            recovery_audit_review_disposition_observation_view_class,
            "kernel-report-runtime-entry-recovery-audit-review-disposition-observation-view") == 0,
        "entry zero class");
    EXPECT_TRUE(result.recovery_audit_review_disposition_observation_views[0].
    ...
    EXPECT_TRUE(result.recovery_audit_review_disposition_observation_views[0].no_effect == 1,
        "entry no effect");
    EXPECT_TRUE(result.recovery_audit_review_disposition_observation_views[0].
            evidence_level == 51u,
...
static int recovery_audit_review_disposition_observation_view_caps_count(void) {
    latticra_kernel_runtime_entry_recovery_audit_review_disposition_observation_view_request_t
        request;
    latticra_kernel_runtime_entry_recovery_audit_review_disposition_observation_view_result_t
        result;
    ...
    request.requested_recovery_audit_review_disposition_observation_view_count = 99u;
    ...
    EXPECT_TRUE(
        latticra_kernel_runtime_entry_recovery_audit_review_disposition_observation_view_evaluate(
            &request, &result) == LATTICRA_STATUS_OK,
        "recovery audit review disposition observation view evaluates cap");
    EXPECT_TRUE(result.recovery_audit_review_disposition_observation_view_count ==
            LATTICRA_KERNEL_RUNTIME_ENTRY_RECOVERY_AUDIT_REVIEW_DISPOSITION_OBSERVATION_VIEW_MAX,
        "recovery audit review disposition observation view capped");
    EXPECT_TRUE(strcmp(result.recovery_audit_review_disposition_observation_views[4].
            recovery_audit_review_disposition_observation_view_class,
            "reserved-runtime-entry-recovery-audit-review-disposition-observation-view") == 0,
        "reserved class");
    EXPECT_TRUE(result.recovery_audit_review_disposition_observation_views[4].
            recovery_audit_review_disposition_observation_view_token == 39004ul,
        "reserved recovery audit review disposition observation token");
    return 0;
}
...
static int report_is_deterministic(void) {
    ...
    latticra_kernel_runtime_entry_recovery_audit_review_disposition_observation_view_result_t
        result;
    char report[
        LATTICRA_KERNEL_RUNTIME_ENTRY_RECOVERY_AUDIT_REVIEW_DISPOSITION_OBSERVATION_VIEW_REPORT_MAX];
    ...
    EXPECT_TRUE(strstr(report,
        ...
        "report title");
    EXPECT_TRUE(strstr(report,
            "recovery_audit_review_disposition_observation_view_status=runtime-entry-recovery-audit-review-disposition-observation-view-seed-ready\n") != 0,
        "status emitted");
    EXPECT_TRUE(strstr(report,
    ...
    EXPECT_TRUE(strstr(report, "evidence_level=51\n") != 0,
        "evidence emitted");
    EXPECT_TRUE(strstr(report,
            "recovery_audit_review_disposition_observation_view[0].recovery_audit_review_disposition_observation_slot=18432\n") != 0,
        "entry audit review disposition observation slot emitted");
    EXPECT_TRUE(strstr(report,
            "recovery_audit_review_disposition_observation_view[0].recovery_audit_review_observation_slot=17408\n") != 0,
        "entry audit review observation slot emitted");
    EXPECT_TRUE(strstr(report,
            "recovery_audit_review_disposition_observation_view[0].recovery_audit_review_disposition_slot=39\n") != 0,
        "entry audit review disposition slot emitted");
    EXPECT_TRUE(strstr(report,
            "recovery_audit_review_disposition_observation_view[0].recovery_audit_review_slot=38\n") != 0,
        "entry audit review slot emitted");
    EXPECT_TRUE(strstr(report,
            "recovery_audit_review_disposition_observation_view[0].recovery_audit_review_disposition_observation_allowed=0\n") != 0,
        "entry audit review disposition observation denied emitted");
    EXPECT_TRUE(strstr(report,
            "recovery_audit_review_disposition_observation_view[0].recovery_audit_review_disposition_allowed=0\n") != 0,
        "entry audit review disposition denied emitted");
    EXPECT_TRUE(strstr(report,
            "recovery_audit_review_disposition_observation_view[0].recovery_audit_review_observation_allowed=0\n") != 0,
        "entry audit review observation denied emitted");
    EXPECT_TRUE(strstr(report,
            "recovery_audit_review_disposition_observation_view[0].no_effect=1\n") != 0,
...
int main(void) {
    ...
    if (recovery_audit_review_disposition_observation_view_is_metadata_only() != 0) {
        return 1;
    }
    if (recovery_audit_review_disposition_observation_view_caps_count() != 0) return 1;
    if (report_is_deterministic() != 0) return 1;
    if (null_guards_are_safe() != 0) return 1;

    puts("kernel_runtime_entry_recovery_audit_review_disposition_observation_view: ok");
    return 0;
}
