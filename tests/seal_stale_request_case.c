...
static latticra_seal_policy_decision_t stale_request_policy_fixture(void) {
    ...
    (void)snprintf(policy_decision.requested_tool, sizeof(policy_decision.requested_tool), "%s", "declared-tool-stale-request");
    ...
    (void)snprintf(policy_decision.reason, sizeof(policy_decision.reason), "%s", "stale-request-case");
...
static int stale_request_case_remains_blocked(void) {
    latticra_seal_policy_decision_t policy_decision = stale_request_policy_fixture();
    ...
    EXPECT_TRUE(strstr(rendered, "stale_request_blocked=1") != 0, "render stale request blocked");
...
int main(void) {
    if (stale_request_case_remains_blocked() != 0) {
        return 1;
    }
    printf("seal stale request case: ok\n");
    return 0;
}
