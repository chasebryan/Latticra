...
int main(void) {
    ...
    if (latticra_seal_runtime_gate_from_policy_decision(&policy_decision, &runtime_gate) != LATTICRA_STATUS_OK) {
        fprintf(stderr, "failed to build Seal runtime gate metadata\n");
        return 1;
    }

    if (latticra_seal_runtime_dry_run_from_policy_and_gate(&policy_decision, &runtime_gate, &dry_run) != LATTICRA_STATUS_OK) {
        fprintf(stderr, "failed to build Seal runtime dry-run metadata\n");
        return 1;
    }

    if (dry_run.error != LATTICRA_SEAL_RUNTIME_DRY_RUN_OK) {
        fprintf(stderr,
                "invalid Seal runtime dry-run metadata: %s\n",
                latticra_seal_runtime_dry_run_error_label(dry_run.error));
        return 1;
    }

    if (latticra_seal_runtime_dry_run_report(&dry_run, rendered, sizeof(rendered)) != LATTICRA_STATUS_OK) {
        fprintf(stderr, "failed to render Seal runtime dry-run report\n");
        return 1;
    }
