...
static int default_request_is_stable(void) {
    ...
    EXPECT_TRUE(request.requested_recovery_closeout_observation_view_count == 4u,
        "default recovery closeout observation view count");
    EXPECT_TRUE(request.runtime_entry_recovery_outcome_observation_view_request.
...
static int recovery_closeout_observation_view_is_metadata_only(void) {
    latticra_kernel_runtime_entry_recovery_closeout_observation_view_request_t
        request;
    ...
    EXPECT_TRUE(
        ...
        "recovery closeout observation view evaluates");
    EXPECT_TRUE(strcmp(result.recovery_closeout_observation_view_status,
            "runtime-entry-recovery-closeout-observation-view-seed-ready") == 0,
        "recovery closeout observation view ready");
    EXPECT_TRUE(strcmp(result.runtime_entry_recovery_outcome_observation_view.
    ...
    EXPECT_TRUE(result.recovery_closeout_observation_view_count == 4u,
        "recovery closeout observation view count");
    ...
    EXPECT_TRUE(result.host_effect_allowed == 0,
        "host effect denied");
    EXPECT_TRUE(result.evidence_level == 48u, "result evidence level");

    EXPECT_TRUE(strcmp(result.recovery_closeout_observation_views[0].
            process_label, "kernel-report-process-metadata") == 0,
        "entry zero process");
    EXPECT_TRUE(strcmp(result.recovery_closeout_observation_views[0].
            recovery_closeout_observation_view_class,
            "kernel-report-runtime-entry-recovery-closeout-observation-view") == 0,
        "entry zero class");
    EXPECT_TRUE(result.recovery_closeout_observation_views[0].
    ...
    EXPECT_TRUE(result.recovery_closeout_observation_views[0].
            recovery_closeout_observation_declared == 1,
        "recovery closeout observation declared");
    EXPECT_TRUE(result.recovery_closeout_observation_views[0].
            recovery_closeout_observation_planned == 0,
        "recovery closeout observation not planned");
    EXPECT_TRUE(result.recovery_closeout_observation_views[0].
            runtime_entry_recovery_closeout_observation_view_allowed == 0,
        "entry recovery closeout observation view denied");
    EXPECT_TRUE(result.recovery_closeout_observation_views[0].
            recovery_closeout_observation_allowed == 0,
        "entry recovery closeout observation denied");
    EXPECT_TRUE(result.recovery_closeout_observation_views[0].
            recovery_closeout_allowed == 0,
        "entry recovery closeout denied");
    EXPECT_TRUE(result.recovery_closeout_observation_views[0].no_effect == 1,
        "entry no effect");
    EXPECT_TRUE(result.recovery_closeout_observation_views[0].
            evidence_level == 48u,
...
static int recovery_closeout_observation_view_caps_count(void) {
    latticra_kernel_runtime_entry_recovery_closeout_observation_view_request_t
        request;
    latticra_kernel_runtime_entry_recovery_closeout_observation_view_result_t
        result;
    ...
    request.requested_recovery_closeout_observation_view_count = 99u;
    request.runtime_entry_recovery_outcome_observation_view_request.
        requested_recovery_outcome_observation_view_count = 99u;
    EXPECT_TRUE(
        latticra_kernel_runtime_entry_recovery_closeout_observation_view_evaluate(
            &request, &result) == LATTICRA_STATUS_OK,
        "recovery closeout observation view evaluates cap");
    EXPECT_TRUE(result.recovery_closeout_observation_view_count ==
            LATTICRA_KERNEL_RUNTIME_ENTRY_RECOVERY_CLOSEOUT_OBSERVATION_VIEW_MAX,
        "recovery closeout observation view capped");
    EXPECT_TRUE(strcmp(result.recovery_closeout_observation_views[4].
            recovery_closeout_observation_view_class,
            "reserved-runtime-entry-recovery-closeout-observation-view") == 0,
        "reserved class");
    EXPECT_TRUE(result.recovery_closeout_observation_views[4].
            recovery_closeout_observation_view_token == 36004ul,
        "reserved recovery closeout observation token");
    return 0;
}
...
static int report_is_deterministic(void) {
    ...
    latticra_kernel_runtime_entry_recovery_closeout_observation_view_result_t
        result;
    char report[
        LATTICRA_KERNEL_RUNTIME_ENTRY_RECOVERY_CLOSEOUT_OBSERVATION_VIEW_REPORT_MAX];
    ...
    EXPECT_TRUE(strstr(report,
        ...
        "report title");
    EXPECT_TRUE(strstr(report,
            "recovery_closeout_observation_view_status=runtime-entry-recovery-closeout-observation-view-seed-ready\n") != 0,
        "status emitted");
    EXPECT_TRUE(strstr(report,
    ...
    EXPECT_TRUE(strstr(report,
            "runtime_entry_recovery_closeout_observation_view_allowed=0\n") != 0,
        "recovery closeout observation view denied emitted");
    EXPECT_TRUE(strstr(report, "recovery_closeout_observation_allowed=0\n") != 0,
        "recovery closeout observation emitted");
    ...
    EXPECT_TRUE(strstr(report, "evidence_level=48\n") != 0,
        "evidence emitted");
    EXPECT_TRUE(strstr(report,
            "recovery_closeout_observation_view[0].recovery_outcome_observation_view_token=0\n") != 0,
        "entry recovery outcome token emitted");
    EXPECT_TRUE(strstr(report,
            "recovery_closeout_observation_view[0].recovery_closeout_observation_slot=15360\n") != 0,
        "entry closeout observation slot emitted");
    EXPECT_TRUE(strstr(report,
            "recovery_closeout_observation_view[0].recovery_closeout_slot=36\n") != 0,
        "entry closeout slot emitted");
    EXPECT_TRUE(strstr(report,
            "recovery_closeout_observation_view[0].recovery_closeout_observation_allowed=0\n") != 0,
        "entry closeout observation denied emitted");
    EXPECT_TRUE(strstr(report,
            "recovery_closeout_observation_view[0].no_effect=1\n") != 0,
...
int main(void) {
    ...
    if (recovery_closeout_observation_view_is_metadata_only() != 0) return 1;
    if (recovery_closeout_observation_view_caps_count() != 0) return 1;
    if (report_is_deterministic() != 0) return 1;
    if (null_guards_are_safe() != 0) return 1;

    puts("kernel_runtime_entry_recovery_closeout_observation_view: ok");
    return 0;
}
