...
static latticra_lir_module_t lat_lir_module(void) {
    ...
    copy_text(lir.effect, sizeof(lir.effect), "none");
    copy_text(lir.boundary, sizeof(lir.boundary), "lat_semantic_only");
    ...
    copy_text(lir.nodes[0].name, sizeof(lir.nodes[0].name), "RuntimeEvidenceModule");
    copy_text(lir.nodes[0].value, sizeof(lir.nodes[0].value), "lat_module");
    lir.nodes[0].parent_index = 0u;
    lir.nodes[0].first_child_index = 1u;
    lir.nodes[0].child_count = 5u;
    ...
    lir.nodes[1].parent_index = 0u;
    lir.nodes[1].first_child_index = 3u;
    lir.nodes[1].child_count = 1u;
    ...
    lir.nodes[2].parent_index = 0u;
    lir.nodes[2].first_child_index = 6u;
    lir.nodes[2].child_count = 1u;
...
static int runtime_boundary_allows_valid_lat_pipeline_metadata(void) {
    ...
    EXPECT_TRUE(strcmp(result.record.lat_lir_card_name, "") == 0, "lat lir card name copied");
    EXPECT_TRUE(strcmp(result.record.lat_lir_effect, "none") == 0, "lat lir effect copied");
    EXPECT_TRUE(strcmp(result.record.lat_lir_boundary, "lat_semantic_only") == 0, "lat lir boundary copied");
    ...
    EXPECT_TRUE(result.record.lat_lir_first_lat_node_parent_index == 0u, "lat lir first node parent copied");
    EXPECT_TRUE(result.record.lat_lir_first_lat_node_first_child_index == 3u, "lat lir first node first child copied");
    EXPECT_TRUE(result.record.lat_lir_first_lat_node_child_count == 1u, "lat lir first node child count copied");
    ...
    EXPECT_TRUE(result.record.lat_lir_first_transition_node_parent_index == 0u, "lat lir first transition node parent copied");
    EXPECT_TRUE(result.record.lat_lir_first_transition_node_first_child_index == 6u, "lat lir first transition node first child copied");
    EXPECT_TRUE(result.record.lat_lir_first_transition_node_child_count == 1u, "lat lir first transition node child count copied");
    ...
    EXPECT_TRUE(result.record.lat_lir_first_edge_from_node_kind == LATTICRA_LIR_NODE_MODULE, "lat lir first edge from node kind copied");
    EXPECT_TRUE(strcmp(result.record.lat_lir_first_edge_from_node_name, "RuntimeEvidenceModule") == 0, "lat lir first edge from node name copied");
    EXPECT_TRUE(strcmp(result.record.lat_lir_first_edge_from_node_value, "lat_module") == 0, "lat lir first edge from node value copied");
    EXPECT_TRUE(strcmp(result.record.lat_lir_first_edge_from_node_operator, "") == 0, "lat lir first edge from node operator copied");
    EXPECT_TRUE(strcmp(result.record.lat_lir_first_edge_from_node_binding, "") == 0, "lat lir first edge from node binding copied");
    EXPECT_TRUE(result.record.lat_lir_first_edge_from_node_parent_index == 0u, "lat lir first edge from node parent copied");
    EXPECT_TRUE(result.record.lat_lir_first_edge_from_node_first_child_index == 1u, "lat lir first edge from node first child copied");
    EXPECT_TRUE(result.record.lat_lir_first_edge_from_node_child_count == 5u, "lat lir first edge from node child count copied");
    EXPECT_TRUE(result.record.lat_lir_first_edge_to_node_kind == LATTICRA_LIR_NODE_LAT_STATE, "lat lir first edge to node kind copied");
    EXPECT_TRUE(strcmp(result.record.lat_lir_first_edge_to_node_name, "RootCell") == 0, "lat lir first edge to node name copied");
    EXPECT_TRUE(strcmp(result.record.lat_lir_first_edge_to_node_value, "root") == 0, "lat lir first edge to node value copied");
    EXPECT_TRUE(strcmp(result.record.lat_lir_first_edge_to_node_operator, "=") == 0, "lat lir first edge to node operator copied");
    EXPECT_TRUE(strcmp(result.record.lat_lir_first_edge_to_node_binding, "lat.state.root") == 0, "lat lir first edge to node binding copied");
    EXPECT_TRUE(result.record.lat_lir_first_edge_to_node_parent_index == 0u, "lat lir first edge to node parent copied");
    EXPECT_TRUE(result.record.lat_lir_first_edge_to_node_first_child_index == 3u, "lat lir first edge to node first child copied");
    EXPECT_TRUE(result.record.lat_lir_first_edge_to_node_child_count == 1u, "lat lir first edge to node child count copied");
    ...
    EXPECT_TRUE(result.record.lat_lir_first_transition_source_edge_from_node_parent_index == 0u, "first transition source edge from node parent copied");
    EXPECT_TRUE(result.record.lat_lir_first_transition_source_edge_from_node_first_child_index == 6u, "first transition source edge from node first child copied");
    EXPECT_TRUE(result.record.lat_lir_first_transition_source_edge_from_node_child_count == 1u, "first transition source edge from node child count copied");
    ...
    EXPECT_TRUE(result.record.lat_lir_first_transition_source_edge_from_node_span.start_column == 21u, "first transition source edge from node span column copied");
    EXPECT_TRUE(result.record.lat_lir_first_transition_source_edge_from_node_span.end_line == 4u, "first transition source edge from node span end line copied");
    ...
    EXPECT_TRUE(result.record.lat_lir_first_transition_source_edge_to_node_parent_index == 0u, "first transition source edge to node parent copied");
    EXPECT_TRUE(result.record.lat_lir_first_transition_source_edge_to_node_first_child_index == 3u, "first transition source edge to node first child copied");
    EXPECT_TRUE(result.record.lat_lir_first_transition_source_edge_to_node_child_count == 1u, "first transition source edge to node child count copied");
    ...
    EXPECT_TRUE(result.record.lat_lir_first_transition_source_edge_to_node_span.start_column == 5u, "first transition source edge to node span column copied");
    EXPECT_TRUE(result.record.lat_lir_first_transition_source_edge_to_node_span.end_line == 3u, "first transition source edge to node span end line copied");
    EXPECT_TRUE(result.record.lat_lir_first_transition_source_edge_to_node_span.end_column == 38u, "first transition source edge to node span end column copied");
...
static int runtime_boundary_reports_lat_pipeline_evidence(void) {
    ...
    EXPECT_TRUE(strstr(report, "lat_lir_card_name=\n") != 0, "lat lir card name report present");
    EXPECT_TRUE(strstr(report, "lat_lir_effect=none\n") != 0, "lat lir effect report present");
    EXPECT_TRUE(strstr(report, "lat_lir_boundary=lat_semantic_only\n") != 0, "lat lir boundary report present");
    ...
    EXPECT_TRUE(strstr(report, "lat_lir_first_lat_node_parent_index=0\n") != 0, "lat lir first node parent report present");
    EXPECT_TRUE(strstr(report, "lat_lir_first_lat_node_first_child_index=3\n") != 0, "lat lir first node first child report present");
    EXPECT_TRUE(strstr(report, "lat_lir_first_lat_node_child_count=1\n") != 0, "lat lir first node child count report present");
    ...
    EXPECT_TRUE(strstr(report, "lat_lir_first_transition_node_parent_index=0\n") != 0, "lat lir first transition node parent report present");
    EXPECT_TRUE(strstr(report, "lat_lir_first_transition_node_first_child_index=6\n") != 0, "lat lir first transition node first child report present");
    EXPECT_TRUE(strstr(report, "lat_lir_first_transition_node_child_count=1\n") != 0, "lat lir first transition node child count report present");
    ...
    EXPECT_TRUE(strstr(report, "lat_lir_first_edge_from_node_kind=module\n") != 0, "lat lir first edge from node kind report present");
    EXPECT_TRUE(strstr(report, "lat_lir_first_edge_from_node_name=RuntimeEvidenceModule\n") != 0, "lat lir first edge from node name report present");
    EXPECT_TRUE(strstr(report, "lat_lir_first_edge_from_node_value=lat_module\n") != 0, "lat lir first edge from node value report present");
    EXPECT_TRUE(strstr(report, "lat_lir_first_edge_from_node_operator=\n") != 0, "lat lir first edge from node operator report present");
    EXPECT_TRUE(strstr(report, "lat_lir_first_edge_from_node_binding=\n") != 0, "lat lir first edge from node binding report present");
    EXPECT_TRUE(strstr(report, "lat_lir_first_edge_from_node_parent_index=0\n") != 0, "lat lir first edge from node parent report present");
    EXPECT_TRUE(strstr(report, "lat_lir_first_edge_from_node_first_child_index=1\n") != 0, "lat lir first edge from node first child report present");
    EXPECT_TRUE(strstr(report, "lat_lir_first_edge_from_node_child_count=5\n") != 0, "lat lir first edge from node child count report present");
    EXPECT_TRUE(strstr(report, "lat_lir_first_edge_to_node_kind=lat_state\n") != 0, "lat lir first edge to node kind report present");
    EXPECT_TRUE(strstr(report, "lat_lir_first_edge_to_node_name=RootCell\n") != 0, "lat lir first edge to node name report present");
    EXPECT_TRUE(strstr(report, "lat_lir_first_edge_to_node_value=root\n") != 0, "lat lir first edge to node value report present");
    EXPECT_TRUE(strstr(report, "lat_lir_first_edge_to_node_operator==\n") != 0, "lat lir first edge to node operator report present");
    EXPECT_TRUE(strstr(report, "lat_lir_first_edge_to_node_binding=lat.state.root\n") != 0, "lat lir first edge to node binding report present");
    EXPECT_TRUE(strstr(report, "lat_lir_first_edge_to_node_parent_index=0\n") != 0, "lat lir first edge to node parent report present");
    EXPECT_TRUE(strstr(report, "lat_lir_first_edge_to_node_first_child_index=3\n") != 0, "lat lir first edge to node first child report present");
    EXPECT_TRUE(strstr(report, "lat_lir_first_edge_to_node_child_count=1\n") != 0, "lat lir first edge to node child count report present");
    ...
    EXPECT_TRUE(strstr(report, "lat_lir_first_transition_source_edge_from_node_parent_index=0\n") != 0, "first transition source edge from node parent report present");
    EXPECT_TRUE(strstr(report, "lat_lir_first_transition_source_edge_from_node_first_child_index=6\n") != 0, "first transition source edge from node first child report present");
    EXPECT_TRUE(strstr(report, "lat_lir_first_transition_source_edge_from_node_child_count=1\n") != 0, "first transition source edge from node child count report present");
    ...
    EXPECT_TRUE(strstr(report, "lat_lir_first_transition_source_edge_from_node_span_start_column=21\n") != 0, "first transition source edge from node span column report present");
    EXPECT_TRUE(strstr(report, "lat_lir_first_transition_source_edge_from_node_span_end_line=4\n") != 0, "first transition source edge from node span end line report present");
    ...
    EXPECT_TRUE(strstr(report, "lat_lir_first_transition_source_edge_to_node_parent_index=0\n") != 0, "first transition source edge to node parent report present");
    EXPECT_TRUE(strstr(report, "lat_lir_first_transition_source_edge_to_node_first_child_index=3\n") != 0, "first transition source edge to node first child report present");
    EXPECT_TRUE(strstr(report, "lat_lir_first_transition_source_edge_to_node_child_count=1\n") != 0, "first transition source edge to node child count report present");
    ...
    EXPECT_TRUE(strstr(report, "lat_lir_first_transition_source_edge_to_node_span_start_column=5\n") != 0, "first transition source edge to node span column report present");
    EXPECT_TRUE(strstr(report, "lat_lir_first_transition_source_edge_to_node_span_end_line=3\n") != 0, "first transition source edge to node span end line report present");
    EXPECT_TRUE(strstr(report, "lat_lir_first_transition_source_edge_to_node_span_end_column=38\n") != 0, "first transition source edge to node span end column report present");
