...
static int default_request_is_stable(void) {
    ...
    EXPECT_TRUE(
        ...
        "default persistence boundary observation view count");
    EXPECT_TRUE(request.runtime_entry_quiescent_return_observation_view_request.
            requested_quiescent_return_observation_view_count == 4u,
        "quiescent return observation view request preserved");
    EXPECT_TRUE(request.runtime_entry_quiescent_return_observation_view_request.
...
static int persistence_boundary_observation_view_is_metadata_only(void) {
    latticra_kernel_runtime_entry_persistence_boundary_observation_view_request_t
        request;
    ...
    EXPECT_TRUE(
        ...
        "persistence boundary observation view evaluates");
    EXPECT_TRUE(strcmp(result.persistence_boundary_observation_view_status,
    ...
    EXPECT_TRUE(result.persistence_boundary_observation_view_count == 4u,
        "persistence boundary observation view count");
    ...
    EXPECT_TRUE(strcmp(result.persistence_boundary_observation_views[0].
            process_label, "kernel-report-process-metadata") == 0,
        "entry zero process");
    EXPECT_TRUE(strcmp(result.persistence_boundary_observation_views[0].
            persistence_boundary_observation_view_class,
            "kernel-report-runtime-entry-persistence-boundary-observation-view") == 0,
        "entry zero class");
    EXPECT_TRUE(result.persistence_boundary_observation_views[0].
    ...
    EXPECT_TRUE(result.persistence_boundary_observation_views[0].
            persistence_epoch_slot == 27ul,
        "entry zero persistence epoch slot");
    EXPECT_TRUE(result.persistence_boundary_observation_views[0].
            persistence_reason_slot == 28ul,
        "entry zero persistence reason slot");
    EXPECT_TRUE(result.persistence_boundary_observation_views[0].
            recovery_authority_slot == 29ul,
        "entry zero recovery authority slot");
    EXPECT_TRUE(result.persistence_boundary_observation_views[0].
            persistence_boundary_declared == 1,
        "persistence boundary declared");
    EXPECT_TRUE(result.persistence_boundary_observation_views[0].
            persistence_boundary_planned == 0,
        "persistence boundary not planned");
    EXPECT_TRUE(result.persistence_boundary_observation_views[0].
    ...
    EXPECT_TRUE(result.persistence_boundary_observation_views[0].no_effect == 1,
        "entry no effect");
    EXPECT_TRUE(result.persistence_boundary_observation_views[0].evidence_level == 43u,
        "entry evidence level");
    return 0;
}

static int persistence_boundary_observation_view_caps_count(void) {
    latticra_kernel_runtime_entry_persistence_boundary_observation_view_request_t
        request;
    latticra_kernel_runtime_entry_persistence_boundary_observation_view_result_t
        result;
    ...
    request.requested_persistence_boundary_observation_view_count = 99u;
    request.runtime_entry_quiescent_return_observation_view_request.
        requested_quiescent_return_observation_view_count = 99u;
    EXPECT_TRUE(
        latticra_kernel_runtime_entry_persistence_boundary_observation_view_evaluate(
            &request, &result) == LATTICRA_STATUS_OK,
        "persistence boundary observation view evaluates cap");
    EXPECT_TRUE(result.persistence_boundary_observation_view_count ==
            LATTICRA_KERNEL_RUNTIME_ENTRY_PERSISTENCE_BOUNDARY_OBSERVATION_VIEW_MAX,
        "persistence boundary observation view capped");
    EXPECT_TRUE(strcmp(result.persistence_boundary_observation_views[4].
            persistence_boundary_observation_view_class,
            "reserved-runtime-entry-persistence-boundary-observation-view") == 0,
        "reserved class");
    EXPECT_TRUE(result.persistence_boundary_observation_views[4].
            persistence_boundary_observation_view_token == 31004ul,
        "reserved persistence boundary observation token");
    return 0;
}
...
static int report_is_deterministic(void) {
    ...
    latticra_kernel_runtime_entry_persistence_boundary_observation_view_result_t
        result;
    char report[
        LATTICRA_KERNEL_RUNTIME_ENTRY_PERSISTENCE_BOUNDARY_OBSERVATION_VIEW_REPORT_MAX];
    ...
    EXPECT_TRUE(strstr(report, "recovery_authority_allowed=0\n") != 0,
        "recovery emitted");
    EXPECT_TRUE(strstr(report, "evidence_level=43\n") != 0,
        "evidence emitted");
    EXPECT_TRUE(strstr(report,
            "persistence_boundary_observation_view[0].persistence_boundary_slot=10240\n") != 0,
        "entry slot emitted");
    EXPECT_TRUE(strstr(report,
            "persistence_boundary_observation_view[0].persistence_boundary_allowed=0\n") != 0,
        "entry denied emitted");
    EXPECT_TRUE(strstr(report,
            "persistence_boundary_observation_view[0].no_effect=1\n") != 0,
...
int main(void) {
    ...
    if (persistence_boundary_observation_view_is_metadata_only() != 0) return 1;
    if (persistence_boundary_observation_view_caps_count() != 0) return 1;
    if (report_is_deterministic() != 0) return 1;
    if (null_guards_are_safe() != 0) return 1;

    puts("kernel_runtime_entry_persistence_boundary_observation_view: ok");
    return 0;
}
