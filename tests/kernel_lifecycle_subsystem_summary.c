...
static int default_request_targets_time_accounting_ready(void) {
    ...
    EXPECT_TRUE(request.lifecycle_request.target_state ==
            LATTICRA_KERNEL_STATE_TIME_ACCOUNTING_READY,
        "summary default target time-accounting-ready");
...
static int summary_reaches_ready_without_authority(void) {
    ...
    EXPECT_TRUE(strcmp(result.summary_status, "summary-ready") == 0,
        "summary ready");
    EXPECT_TRUE(strcmp(result.final_state, "time-accounting-ready") == 0,
        "summary final state time-accounting-ready");
    EXPECT_TRUE(result.lifecycle_complete == 1,
        "summary lifecycle complete");
    EXPECT_TRUE(result.lifecycle_step_count == 16u,
        "summary sixteen lifecycle steps");
    EXPECT_TRUE(result.lifecycle_state_change_count == 16u,
        "summary sixteen lifecycle state changes");
    ...
    EXPECT_TRUE(strcmp(result.entries[2].lifecycle_relation, "time-accounting-ready") == 0,
        "summary time accounting ready");
...
static int summary_report_is_deterministic(void) {
    ...
    EXPECT_TRUE(strstr(report, "final_state=time-accounting-ready\n") != 0,
        "summary report final state");
    EXPECT_TRUE(strstr(report, "lifecycle_complete=1\n") != 0,
        "summary report lifecycle complete");
    EXPECT_TRUE(strstr(report, "lifecycle_step_count=16\n") != 0,
        "summary report step count");
    EXPECT_TRUE(strstr(report, "lifecycle_state_change_count=16\n") != 0,
    ...
    EXPECT_TRUE(strstr(report, "subsystem[2].lifecycle_relation=time-accounting-ready\n") != 0,
...
int main(void) {
    if (default_request_targets_time_accounting_ready() != 0) return 1;
