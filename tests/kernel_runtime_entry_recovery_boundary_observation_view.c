...
static int default_request_is_stable(void) {
    ...
    EXPECT_TRUE(
        ...
        "default recovery boundary observation view count");
    EXPECT_TRUE(request.runtime_entry_persistence_boundary_observation_view_request.
...
static int recovery_boundary_observation_view_is_metadata_only(void) {
    latticra_kernel_runtime_entry_recovery_boundary_observation_view_request_t
        request;
    ...
    EXPECT_TRUE(
        ...
        "recovery boundary observation view evaluates");
    EXPECT_TRUE(strcmp(result.recovery_boundary_observation_view_status,
    ...
    EXPECT_TRUE(result.recovery_boundary_observation_view_count == 4u,
        "recovery boundary observation view count");
    ...
    EXPECT_TRUE(strcmp(result.recovery_boundary_observation_views[0].
            process_label, "kernel-report-process-metadata") == 0,
        "entry zero process");
    EXPECT_TRUE(strcmp(result.recovery_boundary_observation_views[0].
            recovery_boundary_observation_view_class,
            "kernel-report-runtime-entry-recovery-boundary-observation-view") == 0,
        "entry zero class");
    EXPECT_TRUE(result.recovery_boundary_observation_views[0].
    ...
    EXPECT_TRUE(result.recovery_boundary_observation_views[0].no_effect == 1,
        "entry no effect");
    EXPECT_TRUE(result.recovery_boundary_observation_views[0].evidence_level == 44u,
        "entry evidence level");
    return 0;
}

static int recovery_boundary_observation_view_caps_count(void) {
    latticra_kernel_runtime_entry_recovery_boundary_observation_view_request_t
        request;
    latticra_kernel_runtime_entry_recovery_boundary_observation_view_result_t
        result;
    ...
    request.requested_recovery_boundary_observation_view_count = 99u;
    request.runtime_entry_persistence_boundary_observation_view_request.
        requested_persistence_boundary_observation_view_count = 99u;
    EXPECT_TRUE(
        latticra_kernel_runtime_entry_recovery_boundary_observation_view_evaluate(
            &request, &result) == LATTICRA_STATUS_OK,
        "recovery boundary observation view evaluates cap");
    EXPECT_TRUE(result.recovery_boundary_observation_view_count ==
            LATTICRA_KERNEL_RUNTIME_ENTRY_RECOVERY_BOUNDARY_OBSERVATION_VIEW_MAX,
        "recovery boundary observation view capped");
    EXPECT_TRUE(strcmp(result.recovery_boundary_observation_views[4].
            recovery_boundary_observation_view_class,
            "reserved-runtime-entry-recovery-boundary-observation-view") == 0,
        "reserved class");
    EXPECT_TRUE(result.recovery_boundary_observation_views[4].
            recovery_boundary_observation_view_token == 32004ul,
        "reserved recovery boundary observation token");
    return 0;
}
...
static int report_is_deterministic(void) {
    ...
    latticra_kernel_runtime_entry_recovery_boundary_observation_view_result_t
        result;
    char report[
        LATTICRA_KERNEL_RUNTIME_ENTRY_RECOVERY_BOUNDARY_OBSERVATION_VIEW_REPORT_MAX];
    ...
    EXPECT_TRUE(strstr(report,
        ...
        "report title");
    EXPECT_TRUE(strstr(report,
            "recovery_boundary_observation_view_status=runtime-entry-recovery-boundary-observation-view-seed-ready\n") != 0,
        "status emitted");
    EXPECT_TRUE(strstr(report,
    ...
    EXPECT_TRUE(strstr(report, "recovery_authority_allowed=0\n") != 0,
        "recovery emitted");
    EXPECT_TRUE(strstr(report, "evidence_level=44\n") != 0,
        "evidence emitted");
    EXPECT_TRUE(strstr(report,
            "recovery_boundary_observation_view[0].recovery_boundary_slot=11264\n") != 0,
        "entry slot emitted");
    EXPECT_TRUE(strstr(report,
            "recovery_boundary_observation_view[0].recovery_boundary_allowed=0\n") != 0,
        "entry denied emitted");
    EXPECT_TRUE(strstr(report,
            "recovery_boundary_observation_view[0].no_effect=1\n") != 0,
...
int main(void) {
    ...
    if (recovery_boundary_observation_view_is_metadata_only() != 0) return 1;
    if (recovery_boundary_observation_view_caps_count() != 0) return 1;
    if (report_is_deterministic() != 0) return 1;
    if (null_guards_are_safe() != 0) return 1;

    puts("kernel_runtime_entry_recovery_boundary_observation_view: ok");
    return 0;
}
