...
static latticra_seal_policy_decision_t replayed_request_policy_fixture(void) {
    ...
    (void)snprintf(policy_decision.requested_tool, sizeof(policy_decision.requested_tool), "%s", "declared-tool-replayed-request");
    ...
    policy_decision.replay_detected = 1u;
    ...
    (void)snprintf(policy_decision.reason, sizeof(policy_decision.reason), "%s", "replayed-request-case");
...
static int replayed_request_case_remains_blocked(void) {
    latticra_seal_policy_decision_t policy_decision = replayed_request_policy_fixture();
    ...
    EXPECT_TRUE(gate.error == LATTICRA_SEAL_RUNTIME_GATE_INVALID_POLICY_DECISION, "gate rejects replayed report input");
    ...
    EXPECT_TRUE(strstr(rendered, "replayed_request_blocked=1") != 0, "render replayed request blocked");
...
int main(void) {
    if (replayed_request_case_remains_blocked() != 0) {
        return 1;
    }
    printf("seal replayed request case: ok\n");
    return 0;
}
