...
The **LC Standalone** profile narrows that flow to the Console component only. It records `lc-standalone-install-v0`, keeps `lc.profile = "standalone"`, installs standalone/session/workspace/namespace/rootfs/packages/init/services/service-schema/service-definitions/service-plan contracts, disables Panel embedding for the installed console, and preserves the no-effect host and network authority floor.
