...
check_contains "service definitions contract profile" 'service_definitions_contract_profile = "lc-service-definitions-v0"' "$LC_CONFIG"
check_contains "service definitions contract present" 'service_definitions_contract_present = true' "$LC_CONFIG"
check_contains "service plan contract profile" 'service_plan_contract_profile = "lc-service-plan-v0"' "$LC_CONFIG"
check_contains "service plan contract present" 'service_plan_contract_present = true' "$LC_CONFIG"
check_contains "service runtime contract profile" 'service_runtime_contract_profile = "lc-service-runtime-v0"' "$LC_CONFIG"
check_contains "service runtime contract present" 'service_runtime_contract_present = true' "$LC_CONFIG"
check_contains "processes contract profile" 'processes_contract_profile = "lc-processes-v0"' "$LC_CONFIG"
check_contains "processes contract present" 'processes_contract_present = true' "$LC_CONFIG"
...
check_contains "service definitions registry command" 'name=lc service-definitions category=core effect=none capability=lc.service.definitions.contract' "$LC_REGISTRY"
check_contains "service plan registry command" 'name=lc service-plan category=core effect=none capability=lc.service.plan.contract' "$LC_REGISTRY"
check_contains "service runtime registry command" 'name=lc service-runtime category=core effect=none capability=lc.service.runtime.contract' "$LC_REGISTRY"
check_contains "processes registry command" 'name=lc processes category=core effect=none capability=lc.processes.contract' "$LC_REGISTRY"
...
check_contains "service definitions command surface" 'command_surface = "lc service-definitions"' "$LC_SERVICE_DEFINITIONS_CONTRACT"
check_contains "service definitions file marker" 'service_definitions_file = "definitions.toml"' "$LC_SERVICE_DEFINITIONS_CONTRACT"
check_contains "service definitions validation denied" 'service_definition_validation_allowed = false' "$LC_SERVICE_DEFINITIONS_CONTRACT"
check_contains "service definitions start denied" 'service_start_allowed = false' "$LC_SERVICE_DEFINITIONS_CONTRACT"
check_contains "service definitions host process denied" 'host_process_launch_allowed = false' "$LC_SERVICE_DEFINITIONS_CONTRACT"
check_contains "service plan command surface" 'command_surface = "lc service-plan"' "$LC_SERVICE_PLAN_CONTRACT"
check_contains "service plan file marker" 'service_plan_file = "plan.toml"' "$LC_SERVICE_PLAN_CONTRACT"
check_contains "service plan dependency resolution denied" 'service_dependency_resolution_allowed = false' "$LC_SERVICE_PLAN_CONTRACT"
check_contains "service plan activation denied" 'service_activation_allowed = false' "$LC_SERVICE_PLAN_CONTRACT"
check_contains "service plan host process denied" 'host_process_launch_allowed = false' "$LC_SERVICE_PLAN_CONTRACT"
check_contains "service runtime command surface" 'command_surface = "lc service-runtime"' "$LC_SERVICE_RUNTIME_CONTRACT"
check_contains "service runtime file marker" 'service_runtime_file = "runtime.toml"' "$LC_SERVICE_RUNTIME_CONTRACT"
check_contains "service runtime handoff denied" 'service_runtime_handoff_allowed = false' "$LC_SERVICE_RUNTIME_CONTRACT"
check_contains "service runtime executor denied" 'service_executor_allowed = false' "$LC_SERVICE_RUNTIME_CONTRACT"
check_contains "service runtime supervision denied" 'service_supervision_allowed = false' "$LC_SERVICE_RUNTIME_CONTRACT"
check_contains "service runtime host process denied" 'host_process_launch_allowed = false' "$LC_SERVICE_RUNTIME_CONTRACT"
check_contains "processes command surface" 'command_surface = "lc processes"' "$LC_PROCESSES_CONTRACT"
check_contains "processes table denied" 'process_table_present = false' "$LC_PROCESSES_CONTRACT"
check_contains "processes spawn denied" 'process_spawn_allowed = false' "$LC_PROCESSES_CONTRACT"
check_contains "processes signal denied" 'process_signal_allowed = false' "$LC_PROCESSES_CONTRACT"
check_contains "processes supervision denied" 'process_supervision_allowed = false' "$LC_PROCESSES_CONTRACT"
check_contains "processes host process denied" 'host_process_launch_allowed = false' "$LC_PROCESSES_CONTRACT"
...
if [ -x "$USER_BIN/latticra-lc" ]; then
  if "$USER_BIN/latticra-lc" install-config > "$TMP_DIR/install-config.txt"; then
    ...
    check_contains "LC wrapper service definitions contract present" 'service_definitions_contract_present=1' "$TMP_DIR/install-config.txt"
    check_contains "LC wrapper service plan contract present" 'service_plan_contract_present=1' "$TMP_DIR/install-config.txt"
    check_contains "LC wrapper service runtime contract present" 'service_runtime_contract_present=1' "$TMP_DIR/install-config.txt"
    check_contains "LC wrapper processes contract present" 'processes_contract_present=1' "$TMP_DIR/install-config.txt"
  ...
  if "$USER_BIN/latticra-lc" service-definitions > "$TMP_DIR/service-definitions.txt"; then
    check_contains "LC wrapper service definitions report" 'LATTICRA CONSOLE SERVICE DEFINITIONS CONTRACT' "$TMP_DIR/service-definitions.txt"
    check_contains "LC wrapper service definitions command surface" 'command_surface=lc service-definitions' "$TMP_DIR/service-definitions.txt"
    check_contains "LC wrapper service definitions file" 'service_definitions_file=definitions.toml' "$TMP_DIR/service-definitions.txt"
    check_contains "LC wrapper service definitions stub count" 'service_definition_stub_count=0' "$TMP_DIR/service-definitions.txt"
    check_contains "LC wrapper service definitions validation denied" 'service_definition_validation_allowed=0' "$TMP_DIR/service-definitions.txt"
    check_contains "LC wrapper service definitions host process denied" 'host_process_launch_allowed=0' "$TMP_DIR/service-definitions.txt"
  else
    echo "failed: latticra-lc service-definitions" >&2
    failures=$((failures + 1))
  fi

  if "$USER_BIN/latticra-lc" service-plan > "$TMP_DIR/service-plan.txt"; then
    check_contains "LC wrapper service plan report" 'LATTICRA CONSOLE SERVICE PLAN CONTRACT' "$TMP_DIR/service-plan.txt"
    check_contains "LC wrapper service plan command surface" 'command_surface=lc service-plan' "$TMP_DIR/service-plan.txt"
    check_contains "LC wrapper service plan file" 'service_plan_file=plan.toml' "$TMP_DIR/service-plan.txt"
    check_contains "LC wrapper service plan dependency resolution denied" 'service_dependency_resolution_allowed=0' "$TMP_DIR/service-plan.txt"
    check_contains "LC wrapper service plan activation denied" 'service_activation_allowed=0' "$TMP_DIR/service-plan.txt"
    check_contains "LC wrapper service plan host process denied" 'host_process_launch_allowed=0' "$TMP_DIR/service-plan.txt"
  else
    echo "failed: latticra-lc service-plan" >&2
    failures=$((failures + 1))
  fi

  if "$USER_BIN/latticra-lc" service-runtime > "$TMP_DIR/service-runtime.txt"; then
    check_contains "LC wrapper service runtime report" 'LATTICRA CONSOLE SERVICE RUNTIME CONTRACT' "$TMP_DIR/service-runtime.txt"
    check_contains "LC wrapper service runtime command surface" 'command_surface=lc service-runtime' "$TMP_DIR/service-runtime.txt"
    check_contains "LC wrapper service runtime file" 'service_runtime_file=runtime.toml' "$TMP_DIR/service-runtime.txt"
    check_contains "LC wrapper service runtime handoff denied" 'service_runtime_handoff_allowed=0' "$TMP_DIR/service-runtime.txt"
    check_contains "LC wrapper service runtime executor denied" 'service_executor_allowed=0' "$TMP_DIR/service-runtime.txt"
    check_contains "LC wrapper service runtime supervision denied" 'service_supervision_allowed=0' "$TMP_DIR/service-runtime.txt"
    check_contains "LC wrapper service runtime host process denied" 'host_process_launch_allowed=0' "$TMP_DIR/service-runtime.txt"
  else
    echo "failed: latticra-lc service-runtime" >&2
    failures=$((failures + 1))
  fi

  if "$USER_BIN/latticra-lc" processes > "$TMP_DIR/processes.txt"; then
    check_contains "LC wrapper processes report" 'LATTICRA CONSOLE PROCESSES CONTRACT' "$TMP_DIR/processes.txt"
    check_contains "LC wrapper processes command surface" 'command_surface=lc processes' "$TMP_DIR/processes.txt"
    check_contains "LC wrapper processes table denied" 'process_table_present=0' "$TMP_DIR/processes.txt"
    check_contains "LC wrapper processes spawn denied" 'process_spawn_allowed=0' "$TMP_DIR/processes.txt"
    check_contains "LC wrapper processes signal denied" 'process_signal_allowed=0' "$TMP_DIR/processes.txt"
    check_contains "LC wrapper processes supervision denied" 'process_supervision_allowed=0' "$TMP_DIR/processes.txt"
    check_contains "LC wrapper processes host process denied" 'host_process_launch_allowed=0' "$TMP_DIR/processes.txt"
  else
    echo "failed: latticra-lc processes" >&2
    failures=$((failures + 1))
  fi
fi
