...
require_absent() {
  ...
  file="$2"
  if grep -Fq -- "$pattern" "$file"; then
    printf 'fedora local binary rpm build plan: unexpected pattern in %s: %s\n' "$file" "$pattern" >&2
    exit 1
  fi
}
...
require_contains 'local-only draft' docs/FEDORA_LOCAL_BINARY_RPM_BUILD_PLAN.md
require_contains 'The executable payload is the no-effect CLI built from `src/latticra_cli.c`.' docs/FEDORA_LOCAL_BINARY_RPM_BUILD_PLAN.md
...
require_contains 'archive includes src/latticra_cli.c' docs/FEDORA_LOCAL_BINARY_RPM_BUILD_PLAN.md
...
require_contains 'privileged helper files' docs/FEDORA_LOCAL_BINARY_RPM_BUILD_PLAN.md
require_contains 'network authority files' docs/FEDORA_LOCAL_BINARY_RPM_BUILD_PLAN.md
require_contains 'host-mutation hooks' docs/FEDORA_LOCAL_BINARY_RPM_BUILD_PLAN.md
require_contains 'command/runtime smoke checks only after payload inspection accepts the no-effect entrypoint' docs/FEDORA_LOCAL_BINARY_RPM_BUILD_PLAN.md
require_absent 'No executable command, service' docs/FEDORA_LOCAL_BINARY_RPM_BUILD_PLAN.md
