...
future_runner='scripts/run-fedora-vm-cli-payload-repeatability-lane.sh'
...
require_contains 'future manual disposable Fedora VM repeatability runner' "$doc"
require_contains 'without implementing or running it' "$doc"
require_contains 'This plan does not add the runner.' "$doc"
...
for cmd in awk cat cc find grep gzip id mktemp rpm rpmbuild sha256sum sort sudo tar uname; do
...
require_contains 'fedora_vm_cli_payload_repeatability_runner_present=0' "$doc"
...
require_contains 'This plan is not a runner.' "$doc"
require_contains 'It does not add `scripts/run-fedora-vm-cli-payload-repeatability-lane.sh`.' "$doc"
require_contains 'It does not run `rpmbuild`.' "$doc"
require_contains 'It does not run `rpm`.' "$doc"
require_contains 'Implement manually gated Fedora VM CLI payload repeatability runner' "$doc"
...
require_contains 'fedora_vm_cli_payload_repeatability_runner_present=0' "$status"
require_contains 'sh scripts/test-fedora-vm-cli-payload-repeatability-runner-plan.sh' "$status"
require_contains 'Implement manually gated Fedora VM CLI payload repeatability runner' "$status"
...
require_contains 'fedora_vm_cli_payload_repeatability_runner_present=0' "$index"
require_contains 'Implement manually gated Fedora VM CLI payload repeatability runner' "$index"
...
if [ -f "$future_runner" ]; then
  printf 'fedora vm cli payload repeatability runner plan: future runner already exists: %s\n' "$future_runner" >&2
  exit 1
fi
