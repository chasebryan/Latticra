...
require_file() {
  file="$1"
  if [ ! -f "$file" ]; then
    printf 'latticra panel signed updater state transition denial review: missing file: %s\n' "$file" >&2
    exit 1
  fi
}
...
require_contains() {
  ...
  if ! grep -Fq -- "$pattern" "$file"; then
    printf 'latticra panel signed updater state transition denial review: missing required pattern in %s: %s\n' "$file" "$pattern" >&2
    exit 1
  fi
}
...
require_not_contains() {
  ...
  file="$2"
  if grep -Fq -- "$pattern" "$file"; then
    printf 'latticra panel signed updater state transition denial review: forbidden pattern in %s: %s\n' "$file" "$pattern" >&2
    exit 1
  fi
}
...
require_output_contains() {
  ...
  if ! printf '%s\n' "$output" | grep -Fq -- "$pattern"; then
    printf 'latticra panel signed updater state transition denial review: missing required output pattern: %s\n' "$pattern" >&2
    exit 1
  fi
}

doc='docs/LATTICRA_PANEL_SIGNED_UPDATER_STATE_TRANSITION_DENIAL_REVIEW.md'
status='docs/status/LATTICRA_PANEL_SIGNED_UPDATER_STATE_TRANSITION_DENIAL_REVIEW_STATUS.md'
script='scripts/latticra-panel-signed-updater-state-transition-denial-review.sh'
...
require_contains 'Status: no-effect signed updater state transition denial review' "$doc"
require_contains 'sh scripts/latticra-panel-signed-updater-state-transition-denial-review.sh' "$doc"
require_contains 'signed_updater_state_transition_denial_review_present=1' "$doc"
require_contains 'signed_updater_state_transition_denial_review_stdout_only=1' "$doc"
require_contains 'signed_updater_state_transition_denial_review_file_write_enabled=0' "$doc"
require_contains 'signed_updater_state_transition_denial_review_decision=uphold-deny-state-transition' "$doc"
...
require_contains 'review_write_performed=0' "$doc"
require_contains 'host_mutation_performed=0' "$doc"
require_contains 'not update-state evidence' "$doc"

require_contains 'Status: no-effect signed updater state transition denial review status' "$status"
require_contains 'latticra_panel_signed_updater_state_transition_denial_review_present=1' "$status"
require_contains 'latticra_panel_signed_updater_state_transition_denial_review_guard_present=1' "$status"
...
require_contains 'signed_updater_state_transition_denial_review_state=reviewed-no-effect' "$status"
require_contains 'signed_updater_state_transition_denial_review_stdout_only=1' "$status"
require_contains 'signed_updater_state_transition_denial_review_file_write_enabled=0' "$status"
require_contains 'signed_updater_state_transition_denial_review_decision=uphold-deny-state-transition' "$status"
...
require_contains 'review_write_performed=0' "$status"
require_contains 'host_mutation_performed=0' "$status"
require_contains 'production_update_ready=0' "$status"

require_contains 'LATTICRA PANEL SIGNED UPDATER STATE TRANSITION DENIAL REVIEW' "$script"
require_contains 'signed_updater_state_transition_denial_review_status=ok' "$script"
...
require_contains 'review_write_performed=0' "$script"
...
require_contains 'LATTICRA_PANEL_SIGNED_UPDATER_STATE_TRANSITION_DENIAL_REVIEW_STATUS.md' "$index"
require_contains 'signed updater state transition denial review status' "$index"

require_contains 'signed_updater_state_transition_denial_review_present=1' "$self_update"
require_contains 'signed_updater_state_transition_denial_review_decision=uphold-deny-state-transition' "$self_update"
require_contains 'signed_updater_state_transition_denial_review_file_write_enabled=0' "$self_update"
require_contains 'local signed updater state transition denial review' "$self_update"

require_contains 'Panel signed-updater state transition denial disposition closeout audit review disposition review' "$transcript_doc"
require_contains 'Panel signed-updater state transition denial disposition closeout audit review disposition review' "$transcript_status"
require_contains 'Panel signed-updater state transition denial disposition closeout audit review disposition review' "$state_validation_doc"
require_contains 'Panel signed-updater state transition denial disposition closeout audit review disposition review' "$state_validation_status"
require_contains 'Panel signed-updater state transition denial disposition closeout audit review disposition review' "$state_contract_doc"
require_contains 'Panel signed-updater state transition denial disposition closeout audit review disposition review' "$state_contract_status"
require_contains 'Panel signed-updater state transition denial disposition closeout audit review disposition review' "$manifest_validation_doc"
require_contains 'Panel signed-updater state transition denial disposition closeout audit review disposition review' "$manifest_validation_status"
require_contains 'Panel signed-updater state transition denial disposition closeout audit review disposition review' "$manifest_contract_doc"
require_contains 'Panel signed-updater state transition denial disposition closeout audit review disposition review' "$manifest_contract_status"
...
require_contains 'panel-signed-updater-state-transition-denial-disposition-closeout-audit-review-disposition-review' "$transcript_script"
require_contains 'panel-signed-updater-state-transition-denial-disposition-closeout-audit-review-disposition-review' "$transcript_test"
require_contains 'panel-signed-updater-state-transition-denial-disposition-closeout-audit-review-disposition-review' "$state_validation_script"
require_contains 'panel-signed-updater-state-transition-denial-disposition-closeout-audit-review-disposition-review' "$state_validation_test"
require_contains 'panel-signed-updater-state-transition-denial-disposition-closeout-audit-review-disposition-review' "$state_contract_script"
require_contains 'panel-signed-updater-state-transition-denial-disposition-closeout-audit-review-disposition-review' "$state_contract_test"
require_contains 'panel-signed-updater-state-transition-denial-disposition-closeout-audit-review-disposition-review' "$manifest_validation_script"
require_contains 'panel-signed-updater-state-transition-denial-disposition-closeout-audit-review-disposition-review' "$manifest_validation_test"
require_contains 'panel-signed-updater-state-transition-denial-disposition-closeout-audit-review-disposition-review' "$manifest_contract_script"
require_contains 'panel-signed-updater-state-transition-denial-disposition-closeout-audit-review-disposition-review' "$manifest_contract_test"
...
require_contains 'docs/LATTICRA_PANEL_SIGNED_UPDATER_STATE_TRANSITION_DENIAL_REVIEW.md' "$workflow"
require_contains 'docs/status/LATTICRA_PANEL_SIGNED_UPDATER_STATE_TRANSITION_DENIAL_REVIEW_STATUS.md' "$workflow"
require_contains 'scripts/test-latticra-panel-signed-updater-state-transition-denial-review.sh' "$workflow"
require_contains 'scripts/latticra-panel-signed-updater-state-transition-denial-review.sh' "$workflow"
require_contains 'Validate Latticra Panel signed updater state transition denial review' "$workflow"

sh -n "$script"
sh -n "$0"

output=$(sh "$script")
require_output_contains "$output" 'LATTICRA PANEL SIGNED UPDATER STATE TRANSITION DENIAL REVIEW'
require_output_contains "$output" 'signed_updater_state_transition_denial_review_status=ok'
...
require_output_contains "$output" 'signed_updater_state_transition_denial_review_stdout_only=1'
require_output_contains "$output" 'signed_updater_state_transition_denial_review_file_write_enabled=0'
...
printf 'latticra_panel_signed_updater_state_transition_denial_review: ok\n'
