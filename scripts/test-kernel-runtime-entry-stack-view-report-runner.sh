...
tmpdir="$(mktemp -d "${TMPDIR:-/tmp}/latticra-kernel-runtime-entry-stack-view-report.XXXXXX")"
trap 'rm -rf "$tmpdir"' EXIT INT HUP TERM

report_bin="$tmpdir/latticra-kernel-runtime-entry-stack-view-report"
report_txt="$tmpdir/latticra-kernel-runtime-entry-stack-view-report.txt"
...
grep -Fq 'LATTICRA KERNEL RUNTIME ENTRY STACK VIEW REPORT' "$report_txt"
grep -Fq 'stack_view_status=runtime-entry-stack-view-seed-ready' "$report_txt"
...
grep -Fq 'stack_view_count=4' "$report_txt"
...
grep -Fq 'stack_view[0].process_label=kernel-report-process-metadata' "$report_txt"
grep -Fq 'stack_view[0].stack_view_class=kernel-report-runtime-entry-stack-view' "$report_txt"
grep -Fq 'stack_view[0].stack_view_rank=1' "$report_txt"
grep -Fq 'stack_view[0].stack_slot_count=4' "$report_txt"
grep -Fq 'stack_view[0].stack_window_bytes=4096' "$report_txt"
grep -Fq 'stack_view[0].stack_pointer_declared=1' "$report_txt"
grep -Fq 'stack_view[0].guard_page_declared=1' "$report_txt"
grep -Fq 'stack_view[0].stack_view_planned=0' "$report_txt"
grep -Fq 'stack_view[0].stack_capture_planned=0' "$report_txt"
grep -Fq 'stack_view[0].stack_switch_planned=0' "$report_txt"
grep -Fq 'stack_view[0].runtime_entry_stack_view_allowed=0' "$report_txt"
grep -Fq 'stack_view[0].stack_switch_allowed=0' "$report_txt"
grep -Fq 'stack_view[0].no_effect=1' "$report_txt"

printf 'kernel_runtime_entry_stack_view_report_runner: ok\n'
