...
usage() {
  cat <<'USAGE'
Usage:
  fedora-source-archive-acceptance-gate.sh --transcript-review <path>

Checks a supplied Fedora source archive transcript review report and emits a
no-effect acceptance gate report. It does not create a source archive, run
archive commands, run `rpmbuild`, run `mock`, create RPM artifacts, accept
source archive evidence, open build input, install packages, or mutate the host.
USAGE
}
...
reject_placeholders() {
  ...
  if grep -Eq -- '<required|<placeholder>|TODO|TBD' "$file"; then
    fail "placeholder value is not acceptable in transcript review report"
  fi
}

TRANSCRIPT_REVIEW_REPORT=''

while [ "$#" -gt 0 ]; do
  case "$1" in
    --transcript-review)
      [ "$#" -ge 2 ] || fail "missing value for --transcript-review" 64
      TRANSCRIPT_REVIEW_REPORT="$2"
      shift 2
      ;;
    -h|--help)
      usage
...
[ -n "$TRANSCRIPT_REVIEW_REPORT" ] || fail 'missing --transcript-review path' 64
require_file "$TRANSCRIPT_REVIEW_REPORT"
...
require_file docs/status/FEDORA_SOURCE_ARCHIVE_TRANSCRIPT_REVIEW_VALIDATOR_STATUS.md
...
require_contains 'source_archive_accepted_for_build_by_transcript_validator_alone=0' docs/status/FEDORA_SOURCE_ARCHIVE_TRANSCRIPT_REVIEW_VALIDATOR_STATUS.md
...
require_contains 'Fedora source archive transcript review validator is present' README.md
...
do
  require_contains "$marker" "$TRANSCRIPT_REVIEW_REPORT"
done

reject_placeholders "$TRANSCRIPT_REVIEW_REPORT"

for forbidden_claim in \
  'source_archive_accepted_by_transcript_validator=1' \
  'source_archive_written_by_transcript_validator=1' \
  'source_archive_accepted_for_build_by_transcript_validator_alone=1' \
  'source_archive_accepted_by_acceptance_gate=1' \
  'source_archive_written_by_acceptance_gate=1' \
...
do
  require_absent "$forbidden_claim" "$TRANSCRIPT_REVIEW_REPORT"
done
