...
require_output_not_contains() {
  ...
  if grep -Fq -- "$pattern" "$file"; then
    printf '%s\n' "--- output: $file ---" >&2
    sed -n '1,160p' "$file" >&2
    fail "forbidden output pattern present: $pattern"
  fi
}
...
standalone_verify='installer/scripts/latticra-installer-verify-lc-standalone.sh'
...
require_contains 'standalone/contract.toml' "$apply"
require_contains 'name=lc standalone category=core effect=none capability=lc.standalone.inspect' "$apply"
require_contains 'LATTICRA CONSOLE STANDALONE CONTRACT' "$apply"
require_contains 'Latticra standalone LC verification: ok' "$standalone_verify"
...
require_contains 'verify-lc-standalone' installer/Makefile
require_contains 'Validate Latticra Panel LC install configuration' "$workflow"
require_contains 'Validate standalone LC local install verifier' "$workflow"
...
require_output_contains '[dry-run] would skip Latticra Panel build/copy' "$tmp/standalone-dry-run.out"
require_output_contains '[dry-run] would write latticra, latticra-lc commands' "$tmp/standalone-dry-run.out"
require_output_not_contains '[dry-run] would build/copy Latticra Panel when cargo is available' "$tmp/standalone-dry-run.out"
require_output_not_contains 'latticra-panel commands' "$tmp/standalone-dry-run.out"
...
require_contains 'standalone_requires_panel=0' "$tmp/standalone-plan.txt"
