...
tmpdir="$(mktemp -d "${TMPDIR:-/tmp}/latticra-kernel-scheduler-run-entry-report.XXXXXX")"
trap 'rm -rf "$tmpdir"' EXIT INT HUP TERM

report_bin="$tmpdir/latticra-kernel-scheduler-run-entry-report"
report_txt="$tmpdir/latticra-kernel-scheduler-run-entry-report.txt"
...
grep -Fq 'LATTICRA KERNEL SCHEDULER RUN ENTRY REPORT' "$report_txt"
grep -Fq 'run_entry_status=scheduler-run-entry-seed-ready' "$report_txt"
...
grep -Fq 'run_entry_count=4' "$report_txt"
...
grep -Fq 'run_entry[0].process_label=kernel-report-process-metadata' "$report_txt"
grep -Fq 'run_entry[0].run_entry_class=kernel-report-scheduler-run-entry' "$report_txt"
grep -Fq 'run_entry[0].run_entry_rank=1' "$report_txt"
grep -Fq 'run_entry[0].run_entry_declared=1' "$report_txt"
grep -Fq 'run_entry[0].run_entry_planned=0' "$report_txt"
grep -Fq 'run_entry[0].run_entry_prepared=0' "$report_txt"
grep -Fq 'run_entry[0].execution_entered=0' "$report_txt"
grep -Fq 'run_entry[0].no_effect=1' "$report_txt"

printf 'kernel_scheduler_run_entry_report_runner: ok\n'
