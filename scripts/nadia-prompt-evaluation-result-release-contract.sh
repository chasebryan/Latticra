...
usage() {
  cat <<'USAGE'
Usage:
  nadia-prompt-evaluation-result-release-contract.sh [options]

Options:
  --prompt-evaluation-result-disposition <file>  Stage-33 Nadia prompt-evaluation result disposition contract
  --request-class <label>                        Operator request classification label
  --release-family <label>                       Planned prompt-evaluation result release family label
  --release-format <label>                       Planned prompt-evaluation result release format label
  --output <dir>                                 Output directory for prompt-evaluation result release contracts

Creates Nadia Stage-34 prompt-evaluation result release contract metadata only.
It verifies the prompt-evaluation result disposition contract and records
planned release requirements before any release record, release receipt,
publication, disposition record, result-review record, result record,
model-output access, runtime invocation, prompt evaluation, dialogue
generation, token generation, or inference boundary. It does not create
release records, create release receipts, publish output, apply dispositions,
create result-review records, create result records, read or record model
output, record generated text, invoke a runtime, evaluate prompts, generate
...
fail() {
  printf 'nadia prompt evaluation result release contract: %s\n' "$1" >&2
  exit "${2:-1}"
}
...
require_field() {
  ...
  expected="$2"
  message="$3"
  actual=$(field_or_unknown "$key" "$PROMPT_EVALUATION_RESULT_DISPOSITION_ABS")
...
reject_boundary_label() {
  ...
  case "$normalized" in
    *sex*|*sexual*|*porn*|*erotic*|*nude*|*nudity*|*nsfw*|*fetish*|*adult-content*)
      fail "$name is outside Nadia prompt-evaluation-result-release boundary" 65
      ;;
  esac
}
...
RELEASE_FAMILY="operator-reviewed-prompt-evaluation-result-release"
RELEASE_FORMAT="contract-only-offline-evaluation-result-release"
OUTPUT=""

while [ "$#" -gt 0 ]; do
  case "$1" in
    --prompt-evaluation-result-disposition)
      [ "$#" -ge 2 ] || fail "missing value for --prompt-evaluation-result-disposition" 64
      PROMPT_EVALUATION_RESULT_DISPOSITION="$2"
      shift 2
      ;;
    --request-class)
...
if [ -z "$PROMPT_EVALUATION_RESULT_DISPOSITION" ] && [ -f "reports/nadia/prompt-evaluation-result-disposition/latest-prompt-evaluation-result-disposition-contract.txt" ]; then
  PROMPT_EVALUATION_RESULT_DISPOSITION="reports/nadia/prompt-evaluation-result-disposition/latest-prompt-evaluation-result-disposition-contract.txt"
fi

[ -n "$PROMPT_EVALUATION_RESULT_DISPOSITION" ] || fail "prompt evaluation result disposition contract is required" 64
[ -f "$PROMPT_EVALUATION_RESULT_DISPOSITION" ] || fail "prompt evaluation result disposition contract not found: $PROMPT_EVALUATION_RESULT_DISPOSITION" 66
...
if [ -z "$OUTPUT" ]; then
  OUTPUT="reports/nadia/prompt-evaluation-result-release"
fi
mkdir -p "$OUTPUT"
OUT_DIR=$(CDPATH= cd -- "$OUTPUT" && pwd -P)

TS=${NADIA_PROMPT_EVALUATION_RESULT_RELEASE_TIMESTAMP:-$(date -u +"%Y%m%dT%H%M%SZ")}
REPORT="$OUT_DIR/nadia-prompt-evaluation-result-release-contract-$TS.txt"

PROMPT_EVALUATION_RESULT_DISPOSITION_ABS=$(cd "$(dirname -- "$PROMPT_EVALUATION_RESULT_DISPOSITION")" && printf '%s/%s\n' "$(pwd -P)" "$(basename -- "$PROMPT_EVALUATION_RESULT_DISPOSITION")")
PROMPT_EVALUATION_RESULT_DISPOSITION_MEASUREMENT=$(measure_file "$PROMPT_EVALUATION_RESULT_DISPOSITION_ABS")

require_field stage 33-prompt-evaluation-result-disposition-contract "prompt evaluation result disposition stage must be 33-prompt-evaluation-result-disposition-contract"
require_field prompt_evaluation_result_disposition_contract_status contract_only "prompt evaluation result disposition contract must remain contract-only"
require_field prompt_evaluation_result_disposition_stage contract-only "prompt evaluation result disposition stage must be contract-only"
require_field prompt_evaluation_result_disposition_authority 0 "prompt evaluation result disposition authority must remain denied"
require_field prompt_evaluation_result_disposition_allowed 0 "prompt evaluation result disposition must remain disallowed"
require_field prompt_evaluation_result_disposition_recorded 0 "prompt evaluation result disposition must not be recorded"
require_field prompt_evaluation_result_disposition_created 0 "prompt evaluation result disposition must not be created"
require_field prompt_evaluation_result_disposition_performed 0 "prompt evaluation result disposition must not be performed"
require_field prompt_evaluation_result_disposition_metadata_present 1 "prompt evaluation result disposition metadata must be present"
require_field prompt_evaluation_result_disposition_decision blocked_contract_only "prompt evaluation result disposition decision must be blocked_contract_only"
require_field prompt_evaluation_result_disposition_evidence_present 1 "prompt evaluation result disposition evidence must be present"
require_field prompt_evaluation_result_disposition_source_policy operator-reviewed-offline "prompt evaluation result disposition source policy must be operator-reviewed-offline"
require_field prompt_evaluation_result_disposition_plan_recorded 1 "prompt evaluation result disposition plan must be recorded"
require_field prompt_evaluation_result_disposition_result_recorded 0 "prompt evaluation result disposition result must remain absent"
require_field prompt_evaluation_result_disposition_runtime_invoked 0 "prompt evaluation result disposition runtime must not be invoked"
...
require_field requires_future_prompt_evaluation_result_release_contract 1 "future prompt evaluation result release contract must be required"
require_field prompt_evaluation_result_disposition_promotion_allowed 0 "prompt evaluation result disposition must not allow promotion"
...
require_field requires_prompt_evaluation_result_disposition_schema_policy 1 "prompt evaluation result disposition schema policy must be required"
require_field requires_prompt_evaluation_result_disposition_denial_policy 1 "prompt evaluation result disposition denial policy must be required"
require_field requires_prompt_evaluation_result_release_policy 1 "prompt evaluation result release policy must be required"
...
require_field prompt_evaluation_result_review_findings_recorded 0 "prompt evaluation result review findings must not be recorded"
...
PROMPT_EVALUATION_RESULT_DISPOSITION_CONTRACT_STATUS=$(field_or_unknown prompt_evaluation_result_disposition_contract_status "$PROMPT_EVALUATION_RESULT_DISPOSITION_ABS")
PROMPT_EVALUATION_RESULT_DISPOSITION_STAGE=$(field_or_unknown prompt_evaluation_result_disposition_stage "$PROMPT_EVALUATION_RESULT_DISPOSITION_ABS")
PROMPT_EVALUATION_RESULT_DISPOSITION_FAMILY=$(field_or_unknown prompt_evaluation_result_disposition_family "$PROMPT_EVALUATION_RESULT_DISPOSITION_ABS")
PROMPT_EVALUATION_RESULT_DISPOSITION_FORMAT=$(field_or_unknown prompt_evaluation_result_disposition_format "$PROMPT_EVALUATION_RESULT_DISPOSITION_ABS")
PROMPT_EVALUATION_RESULT_REVIEW_STAGE=$(field_or_unknown prompt_evaluation_result_review_stage "$PROMPT_EVALUATION_RESULT_DISPOSITION_ABS")
PROMPT_EVALUATION_RESULT_STAGE=$(field_or_unknown prompt_evaluation_result_stage "$PROMPT_EVALUATION_RESULT_DISPOSITION_ABS")
PROMPT_EVALUATION_INVOCATION_STAGE=$(field_or_unknown prompt_evaluation_invocation_stage "$PROMPT_EVALUATION_RESULT_DISPOSITION_ABS")
PROMPT_EVALUATION_RUNTIME_HANDOFF_STAGE=$(field_or_unknown prompt_evaluation_runtime_handoff_stage "$PROMPT_EVALUATION_RESULT_DISPOSITION_ABS")
PROMPT_EVALUATION_INPUT_STAGE=$(field_or_unknown prompt_evaluation_input_stage "$PROMPT_EVALUATION_RESULT_DISPOSITION_ABS")
CONTEXT_WINDOW_ASSEMBLY_STAGE=$(field_or_unknown context_window_assembly_stage "$PROMPT_EVALUATION_RESULT_DISPOSITION_ABS")
PROMPT_TOKEN_SEQUENCE_STAGE=$(field_or_unknown prompt_token_sequence_stage "$PROMPT_EVALUATION_RESULT_DISPOSITION_ABS")
PROMPT_TOKENIZATION_STAGE=$(field_or_unknown prompt_tokenization_stage "$PROMPT_EVALUATION_RESULT_DISPOSITION_ABS")
...
prompt_evaluation_result_release_family=$RELEASE_FAMILY
prompt_evaluation_result_release_format=$RELEASE_FORMAT
prompt_evaluation_result_release_schema_planned=1
prompt_evaluation_result_release_schema_validated=0
...
[inputs]
prompt_evaluation_result_disposition=$PROMPT_EVALUATION_RESULT_DISPOSITION_ABS
prompt_evaluation_result_disposition_measurement=$PROMPT_EVALUATION_RESULT_DISPOSITION_MEASUREMENT
prompt_evaluation_result_disposition_stage_required=33-prompt-evaluation-result-disposition-contract
prompt_evaluation_result_disposition_contract_status=$PROMPT_EVALUATION_RESULT_DISPOSITION_CONTRACT_STATUS
prompt_evaluation_result_disposition_stage=$PROMPT_EVALUATION_RESULT_DISPOSITION_STAGE
prompt_evaluation_result_disposition_family=$PROMPT_EVALUATION_RESULT_DISPOSITION_FAMILY
prompt_evaluation_result_disposition_format=$PROMPT_EVALUATION_RESULT_DISPOSITION_FORMAT
...
[prompt_evaluation_result_release_contract]
...
[prompt_evaluation_result_release_requirements]
...
prompt_evaluation_result_release_performed=0
prompt_evaluation_result_release_created=0
prompt_evaluation_result_release_materialized=0
...
cp "$REPORT" "$OUT_DIR/latest-prompt-evaluation-result-release-contract.txt"

printf 'NADIA_PROMPT_EVALUATION_RESULT_RELEASE_CONTRACT=%s\n' "$REPORT"
printf 'NADIA_PROMPT_EVALUATION_RESULT_RELEASE_RECORDED=0\n'
