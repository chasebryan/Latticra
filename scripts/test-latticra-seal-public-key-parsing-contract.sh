...
require_file() {
  file="$1"
  if [ ! -f "$file" ]; then
    printf 'seal public-key parsing contract: missing file: %s\n' "$file" >&2
    exit 1
  fi
}
...
require_contains() {
  ...
  if ! grep -Fq -- "$pattern" "$file"; then
    printf 'seal public-key parsing contract: missing required pattern in %s: %s\n' "$file" "$pattern" >&2
    exit 1
  fi
}

contract="docs/LATTICRA_SEAL_PUBLIC_KEY_PARSING_CONTRACT.md"
...
require_contains 'Status: Latticra Seal public-key parsing boundary contract' "$contract"
require_contains 'The public-key parsing surface is public-key parsing path classification, not public-key parsing, not key material handling, not signing, not verification, and not trust-store behavior.' "$contract"
...
require_contains 'bounded no-effect key parsing implementation' "$contract"
...
require_contains 'public_key_parsing_state=public-key-parsing-metadata-only' docs/LATTICRA_SEAL_PUBLIC_KEY_PARSING_IMPLEMENTATION.md
require_contains 'public_key_parsing_ready=1' docs/LATTICRA_SEAL_PUBLIC_KEY_PARSING_IMPLEMENTATION.md
require_contains 'public_key_parsed=0' docs/LATTICRA_SEAL_PUBLIC_KEY_PARSING_IMPLEMENTATION.md
require_contains 'key_material_loaded=0' docs/LATTICRA_SEAL_PUBLIC_KEY_PARSING_IMPLEMENTATION.md
require_contains 'hardware_key_used=0' docs/LATTICRA_SEAL_PUBLIC_KEY_PARSING_IMPLEMENTATION.md
...
require_contains 'key_material_state=key-material-metadata-only' docs/status/SEAL_KEY_MATERIAL_STATUS.md
require_contains 'key_material_ready=1' docs/status/SEAL_KEY_MATERIAL_STATUS.md
require_contains 'public_key_parsed=0' docs/status/SEAL_KEY_MATERIAL_STATUS.md
require_contains 'key_material_loaded=0' docs/status/SEAL_KEY_MATERIAL_STATUS.md
