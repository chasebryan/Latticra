...
require_file() {
  file="$1"
  if [ ! -f "$file" ]; then
    printf 'kernel_runtime_entry_address_space_view: missing file: %s\n' "$file" >&2
    exit 1
  fi
}
...
tmpdir="$(mktemp -d "${TMPDIR:-/tmp}/latticra-kernel-runtime-entry-address-space-view.XXXXXX")"
trap 'rm -rf "$tmpdir"' EXIT INT HUP TERM
bin="$tmpdir/latticra-kernel-runtime-entry-address-space-view"
