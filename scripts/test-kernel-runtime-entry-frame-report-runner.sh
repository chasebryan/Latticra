...
tmpdir="$(mktemp -d "${TMPDIR:-/tmp}/latticra-kernel-runtime-entry-frame-report.XXXXXX")"
trap 'rm -rf "$tmpdir"' EXIT INT HUP TERM

report_bin="$tmpdir/latticra-kernel-runtime-entry-frame-report"
report_txt="$tmpdir/latticra-kernel-runtime-entry-frame-report.txt"
...
grep -Fq 'LATTICRA KERNEL RUNTIME ENTRY FRAME REPORT' "$report_txt"
grep -Fq 'frame_status=runtime-entry-frame-seed-ready' "$report_txt"
...
grep -Fq 'frame_count=4' "$report_txt"
...
grep -Fq 'frame[0].process_label=kernel-report-process-metadata' "$report_txt"
grep -Fq 'frame[0].frame_class=kernel-report-runtime-entry-frame' "$report_txt"
grep -Fq 'frame[0].admission_rank=1' "$report_txt"
grep -Fq 'frame[0].frame_rank=1' "$report_txt"
grep -Fq 'frame[0].frame_declared=1' "$report_txt"
grep -Fq 'frame[0].frame_planned=0' "$report_txt"
grep -Fq 'frame[0].frame_built=0' "$report_txt"
grep -Fq 'frame[0].frame_installed=0' "$report_txt"
grep -Fq 'frame[0].runtime_entry_entered=0' "$report_txt"
grep -Fq 'frame[0].execution_entered=0' "$report_txt"
grep -Fq 'frame[0].no_effect=1' "$report_txt"

printf 'kernel_runtime_entry_frame_report_runner: ok\n'
