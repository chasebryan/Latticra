...
for file in "$doc" "$status" "$script" "$test_script" "$readiness" \
  "$promotion_gate" "$evidence_intake" "$ledger" "$workflow" Makefile
do
  require_file "$file"
done
...
require_contains 'release_artifact_candidate_parent_dir_exists=<observed>' "$doc"
...
require_output_contains 'release_artifact_candidate_parent_dir_exists=' "$output"
