...
usage() {
  cat <<'USAGE'
Usage:
  latticra-panel-signed-updater-state-transition-denial-disposition-closeout-audit.sh

Emits the no-effect audit for the Latticra Panel signed updater state
transition denial disposition closeout. It does not transition update state,
stage updates, write audit files, write receipts, mutate host state, or apply
signed updates.
USAGE
}
...
signed_updater_state_transition_denial_disposition_closeout_audit_status=ok
repo_root=$ROOT
