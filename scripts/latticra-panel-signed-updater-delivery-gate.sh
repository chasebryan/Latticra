...
usage() {
  cat <<'USAGE'
Usage:
  latticra-panel-signed-updater-delivery-gate.sh

Emits the closed signed-updater delivery gate for Latticra Panel. It does not
fetch updates, trust a remote repository, verify artifacts, stage updates,
write receipts, mutate host state, or apply signed updates.
USAGE
}
...
signed_updater_delivery_gate_status=ok
repo_root=$ROOT
