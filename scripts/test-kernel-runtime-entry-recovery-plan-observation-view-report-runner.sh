...
tmpdir="$(mktemp -d "${TMPDIR:-/tmp}/latticra-kernel-runtime-entry-recovery-plan-observation-view-report.XXXXXX")"
trap 'rm -rf "$tmpdir"' EXIT INT HUP TERM

report_bin="$tmpdir/latticra-kernel-runtime-entry-recovery-plan-observation-view-report"
report_txt="$tmpdir/latticra-kernel-runtime-entry-recovery-plan-observation-view-report.txt"
...
cc $CFLAGS \
  ...
  tools/kernel_runtime_entry_recovery_plan_observation_view_report.c \
  -o "$report_bin"

"$report_bin" > "$report_txt"

grep -Fq 'LATTICRA KERNEL RUNTIME ENTRY RECOVERY PLAN OBSERVATION VIEW REPORT' "$report_txt"
grep -Fq 'recovery_plan_observation_view_status=runtime-entry-recovery-plan-observation-view-seed-ready' "$report_txt"
...
grep -Fq 'evidence_level=45' "$report_txt"
grep -Fq 'recovery_plan_observation_view[0].process_label=kernel-report-process-metadata' "$report_txt"
grep -Fq 'recovery_plan_observation_view[0].recovery_plan_observation_view_class=kernel-report-runtime-entry-recovery-plan-observation-view' "$report_txt"
grep -Fq 'recovery_plan_observation_view[0].recovery_boundary_observation_view_token=0' "$report_txt"
grep -Fq 'recovery_plan_observation_view[0].persistence_boundary_slot=10240' "$report_txt"
grep -Fq 'recovery_plan_observation_view[0].recovery_boundary_slot=11264' "$report_txt"
grep -Fq 'recovery_plan_observation_view[0].recovery_plan_observation_slot=12288' "$report_txt"
grep -Fq 'recovery_plan_observation_view[0].recovery_epoch_slot=30' "$report_txt"
grep -Fq 'recovery_plan_observation_view[0].recovery_reason_slot=31' "$report_txt"
grep -Fq 'recovery_plan_observation_view[0].recovery_plan_slot=32' "$report_txt"
grep -Fq 'recovery_plan_observation_view[0].recovery_authority_slot=33' "$report_txt"
grep -Fq 'recovery_plan_observation_view[0].runtime_entry_recovery_plan_observation_view_allowed=0' "$report_txt"
grep -Fq 'recovery_plan_observation_view[0].runtime_entry_recovery_boundary_observation_view_allowed=0' "$report_txt"
grep -Fq 'recovery_plan_observation_view[0].runtime_entry_persistence_boundary_observation_view_allowed=0' "$report_txt"
grep -Fq 'recovery_plan_observation_view[0].recovery_plan_observation_allowed=0' "$report_txt"
grep -Fq 'recovery_plan_observation_view[0].recovery_boundary_allowed=0' "$report_txt"
grep -Fq 'recovery_plan_observation_view[0].recovery_plan_allowed=0' "$report_txt"
grep -Fq 'recovery_plan_observation_view[0].persistence_boundary_allowed=0' "$report_txt"
grep -Fq 'recovery_plan_observation_view[0].persistence_commit_allowed=0' "$report_txt"
grep -Fq 'recovery_plan_observation_view[0].persistence_allowed=0' "$report_txt"
grep -Fq 'recovery_plan_observation_view[0].recovery_authority_allowed=0' "$report_txt"
grep -Fq 'recovery_plan_observation_view[0].no_effect=1' "$report_txt"

printf 'kernel_runtime_entry_recovery_plan_observation_view_report_runner: ok\n'
