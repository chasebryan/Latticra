...
tmpdir="$(mktemp -d "${TMPDIR:-/tmp}/latticra-kernel-runtime-entry-syscall-gate-view-report.XXXXXX")"
trap 'rm -rf "$tmpdir"' EXIT INT HUP TERM

report_bin="$tmpdir/latticra-kernel-runtime-entry-syscall-gate-view-report"
report_txt="$tmpdir/latticra-kernel-runtime-entry-syscall-gate-view-report.txt"
...
grep -Fq 'LATTICRA KERNEL RUNTIME ENTRY SYSCALL GATE VIEW REPORT' "$report_txt"
grep -Fq 'syscall_gate_view_status=runtime-entry-syscall-gate-view-seed-ready' "$report_txt"
...
grep -Fq 'syscall_gate_view_count=4' "$report_txt"
...
grep -Fq 'syscall_gate_view[0].process_label=kernel-report-process-metadata' "$report_txt"
grep -Fq 'syscall_gate_view[0].syscall_gate_view_class=kernel-report-runtime-entry-syscall-gate-view' "$report_txt"
grep -Fq 'syscall_gate_view[0].privilege_level=0' "$report_txt"
grep -Fq 'syscall_gate_view[0].ring_number=0' "$report_txt"
grep -Fq 'syscall_gate_view[0].syscall_number=512' "$report_txt"
grep -Fq 'syscall_gate_view[0].gate_vector=128' "$report_txt"
grep -Fq 'syscall_gate_view[0].handler_slot=0' "$report_txt"
grep -Fq 'syscall_gate_view[0].syscall_gate_view_declared=1' "$report_txt"
grep -Fq 'syscall_gate_view[0].syscall_number_declared=1' "$report_txt"
grep -Fq 'syscall_gate_view[0].syscall_vector_declared=1' "$report_txt"
grep -Fq 'syscall_gate_view[0].syscall_handler_declared=1' "$report_txt"
grep -Fq 'syscall_gate_view[0].syscall_gate_view_planned=0' "$report_txt"
grep -Fq 'syscall_gate_view[0].syscall_dispatch_planned=0' "$report_txt"
grep -Fq 'syscall_gate_view[0].syscall_instruction_planned=0' "$report_txt"
grep -Fq 'syscall_gate_view[0].syscall_handler_planned=0' "$report_txt"
grep -Fq 'syscall_gate_view[0].syscall_return_planned=0' "$report_txt"
grep -Fq 'syscall_gate_view[0].runtime_entry_syscall_gate_view_allowed=0' "$report_txt"
grep -Fq 'syscall_gate_view[0].syscall_gate_allowed=0' "$report_txt"
grep -Fq 'syscall_gate_view[0].no_effect=1' "$report_txt"

printf 'kernel_runtime_entry_syscall_gate_view_report_runner: ok\n'
