...
require_file() {
  file="$1"
  if [ ! -f "$file" ]; then
    printf 'opensuse rpm build-evidence intake denial disposition closeout archive gate review contract: missing file: %s\n' "$file" >&2
    exit 1
  fi
}
...
require_contains() {
  ...
  if ! grep -Fq -- "$pattern" "$file"; then
    printf 'opensuse rpm build-evidence intake denial disposition closeout archive gate review contract: missing required pattern in %s: %s\n' "$file" "$pattern" >&2
    exit 1
  fi
}
...
require_no_rpm_artifacts() {
  ...
  if find "$dir" \( -name '*.rpm' -o -name '*.src.rpm' \) -print | grep -q .; then
    printf 'opensuse rpm build-evidence intake denial disposition closeout archive gate review contract: unexpected RPM artifact under %s\n' "$dir" >&2
    exit 1
  fi
}

contract='docs/OPENSUSE_RPM_BUILD_EVIDENCE_INTAKE_DENIAL_DISPOSITION_CLOSEOUT_ARCHIVE_GATE_REVIEW_CONTRACT.md'
...
require_contains 'Status: active RPM build-evidence intake denial disposition closeout archive gate review contract' "$contract"
require_contains 'documentation-only and static' "$contract"
require_contains 'opensuse_rpm_build_evidence_intake_denial_disposition_closeout_archive_gate_review_contract_present=1' "$contract"
require_contains 'rpm_build_evidence_intake_denial_disposition_closeout_archive_gate_review_contract_present=1' "$contract"
...
require_contains 'opensuse_build_evidence_intake_denial_archive_gate_present=1' "$contract"
require_contains 'opensuse_build_evidence_intake_denial_archive_gate_closed=1' "$contract"
require_contains 'opensuse_build_evidence_intake_denial_disposition_closeout_present=1' "$contract"
require_contains 'opensuse_build_evidence_intake_denial_closed_out=1' "$contract"
...
require_contains 'Add openSUSE RPM build-evidence intake denial disposition closeout archive gate review disposition contract' "$contract"
require_contains 'opensuse_rpm_build_evidence_intake_denial_disposition_closeout_archive_gate_review_contract: ok' "$contract"
...
require_contains 'OPENSUSE_RPM_BUILD_EVIDENCE_INTAKE_DENIAL_DISPOSITION_CLOSEOUT_ARCHIVE_GATE_REVIEW_CONTRACT.md' docs/README.md
require_contains 'OPENSUSE_RPM_BUILD_EVIDENCE_INTAKE_DENIAL_DISPOSITION_CLOSEOUT_ARCHIVE_GATE_REVIEW_CONTRACT.md' docs/status/README.md

for file in \
  docs/status/OPENSUSE_ECOSYSTEM_INTEGRATION_STATUS.md \
  packaging/opensuse/README.md
do
  require_contains 'opensuse_rpm_build_evidence_intake_denial_disposition_closeout_archive_gate_review_contract_present=1' "$file"
  require_contains 'rpm_build_evidence_intake_denial_disposition_closeout_archive_gate_review_contract_present=1' "$file"
...
require_contains 'opensuse_build_evidence_intake_denial_archive_gate_review_present=1' docs/status/OPENSUSE_ECOSYSTEM_INTEGRATION_STATUS.md
require_contains 'opensuse_build_evidence_intake_denial_archive_gate_reviewed=1' docs/status/OPENSUSE_ECOSYSTEM_INTEGRATION_STATUS.md
require_contains 'opensuse_denial_archive_record_write_allowed=0' docs/status/OPENSUSE_ECOSYSTEM_INTEGRATION_STATUS.md
require_contains 'opensuse_build_evidence_intake_denial_archive_gate_review_present=1' packaging/opensuse/README.md
require_contains 'opensuse_build_evidence_intake_denial_archive_gate_reviewed=1' packaging/opensuse/README.md
...
require_contains 'Run openSUSE RPM build evidence intake denial disposition closeout archive gate review contract guard' .github/workflows/opensuse-rpm-build-evidence-intake-denial-disposition-closeout-archive-gate-review-contract.yml
require_contains 'sh scripts/test-opensuse-rpm-build-evidence-intake-denial-disposition-closeout-archive-gate-review-contract.sh' .github/workflows/opensuse-rpm-build-evidence-intake-denial-disposition-closeout-archive-gate-review-contract.yml

sh -n "$0"

printf 'opensuse_rpm_build_evidence_intake_denial_disposition_closeout_archive_gate_review_contract: ok\n'
