...
usage() {
  cat <<'USAGE'
Usage:
  latticra-panel-signed-updater-state-transition-denial-review.sh

Emits the no-effect review for the Latticra Panel signed updater state
transition denial transcript. It does not transition update state, stage
updates, write review files, write receipts, mutate host state, or apply signed
updates.
USAGE
}
...
signed_updater_state_transition_denial_review_status=ok
repo_root=$ROOT
