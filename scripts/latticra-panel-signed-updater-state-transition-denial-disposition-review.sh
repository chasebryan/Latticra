...
usage() {
  cat <<'USAGE'
Usage:
  latticra-panel-signed-updater-state-transition-denial-disposition-review.sh

Emits the no-effect review for the Latticra Panel signed updater state
transition denial disposition. It does not transition update state, stage
...
signed_updater_state_transition_denial_disposition_review_status=ok
repo_root=$ROOT
