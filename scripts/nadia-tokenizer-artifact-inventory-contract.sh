...
usage() {
  cat <<'USAGE'
Usage:
  nadia-tokenizer-artifact-inventory-contract.sh [options]

Options:
  --tokenizer-manifest <file>  Stage-19 Nadia tokenizer-manifest contract
  --request-class <label>      Operator request classification label
  --inventory-family <label>   Planned tokenizer artifact inventory family label
  --inventory-format <label>   Planned tokenizer artifact inventory format label
  --output <dir>               Output directory for tokenizer-artifact-inventory contracts

Creates Nadia Stage-20 tokenizer-artifact-inventory contract metadata only. It
verifies the tokenizer-manifest contract and records planned tokenizer artifact
inventory requirements before any tokenizer artifact path resolution, artifact
scan, artifact stat, artifact hashing, tokenizer manifest loading, tokenizer
file access, tokenizer vocabulary loading, prompt tokenization, or prompt
evaluation boundary. It does not open tokenizer artifacts, read tokenizer
artifacts, hash tokenizer artifacts, load tokenizer manifests, tokenize
...
fail() {
  printf 'nadia tokenizer artifact inventory contract: %s\n' "$1" >&2
  exit "${2:-1}"
}
...
require_field() {
  ...
  expected="$2"
  message="$3"
  actual=$(field_or_unknown "$key" "$TOKENIZER_MANIFEST_ABS")
...
reject_boundary_label() {
  ...
  case "$normalized" in
    *sex*|*sexual*|*porn*|*erotic*|*nude*|*nudity*|*nsfw*|*fetish*|*adult-content*)
      fail "$name is outside Nadia tokenizer-artifact-inventory boundary" 65
      ;;
  esac
}
...
INVENTORY_FAMILY="operator-reviewed-tokenizer-artifact-inventory"
INVENTORY_FORMAT="contract-only-offline-inventory"
OUTPUT=""

while [ "$#" -gt 0 ]; do
  case "$1" in
    --tokenizer-manifest)
      [ "$#" -ge 2 ] || fail "missing value for --tokenizer-manifest" 64
      TOKENIZER_MANIFEST="$2"
      shift 2
      ;;
    --request-class)
      ...
      ;;
    --inventory-family)
      [ "$#" -ge 2 ] || fail "missing value for --inventory-family" 64
      INVENTORY_FAMILY="$2"
      shift 2
      ;;
    --inventory-format)
      [ "$#" -ge 2 ] || fail "missing value for --inventory-format" 64
      INVENTORY_FORMAT="$2"
      shift 2
      ;;
    --output)
...
if [ -z "$TOKENIZER_MANIFEST" ] && [ -f "reports/nadia/tokenizer-manifest/latest-tokenizer-manifest-contract.txt" ]; then
  TOKENIZER_MANIFEST="reports/nadia/tokenizer-manifest/latest-tokenizer-manifest-contract.txt"
fi

[ -n "$TOKENIZER_MANIFEST" ] || fail "tokenizer manifest contract is required" 64
[ -f "$TOKENIZER_MANIFEST" ] || fail "tokenizer manifest contract not found: $TOKENIZER_MANIFEST" 66

reject_boundary_label "request class" "$REQUEST_CLASS"
reject_boundary_label "inventory family" "$INVENTORY_FAMILY"
reject_boundary_label "inventory format" "$INVENTORY_FORMAT"

if [ -z "$OUTPUT" ]; then
  OUTPUT="reports/nadia/tokenizer-artifact-inventory"
fi
mkdir -p "$OUTPUT"
OUT_DIR=$(CDPATH= cd -- "$OUTPUT" && pwd -P)

TS=${NADIA_TOKENIZER_ARTIFACT_INVENTORY_TIMESTAMP:-$(date -u +"%Y%m%dT%H%M%SZ")}
REPORT="$OUT_DIR/nadia-tokenizer-artifact-inventory-contract-$TS.txt"

TOKENIZER_MANIFEST_ABS=$(cd "$(dirname -- "$TOKENIZER_MANIFEST")" && printf '%s/%s\n' "$(pwd -P)" "$(basename -- "$TOKENIZER_MANIFEST")")
TOKENIZER_MANIFEST_MEASUREMENT=$(measure_file "$TOKENIZER_MANIFEST_ABS")

require_field stage 19-tokenizer-manifest-contract "tokenizer manifest stage must be 19-tokenizer-manifest-contract"
require_field tokenizer_manifest_contract_status contract_only "tokenizer manifest contract must remain contract-only"
require_field tokenizer_manifest_stage contract-only "tokenizer manifest stage must be contract-only"
require_field tokenizer_manifest_authority 0 "tokenizer manifest authority must remain denied"
require_field tokenizer_manifest_allowed 0 "tokenizer manifest must remain disallowed"
require_field tokenizer_manifest_performed 0 "tokenizer manifest must not be performed"
require_field tokenizer_manifest_metadata_present 1 "tokenizer manifest metadata must be present"
require_field tokenizer_manifest_decision blocked_contract_only "tokenizer manifest decision must be blocked_contract_only"
require_field tokenizer_manifest_evidence_present 1 "tokenizer manifest evidence must be present"
require_field tokenizer_manifest_source_policy operator-reviewed-offline "tokenizer manifest source policy must be operator-reviewed-offline"
require_field tokenizer_manifest_path_recorded 0 "tokenizer manifest path must not be recorded"
require_field tokenizer_manifest_schema_planned 1 "tokenizer manifest schema must only be planned"
...
require_field tokenizer_manifest_validated 0 "tokenizer manifest must not be validated"
require_field requires_tokenizer_specification_contract 1 "tokenizer manifest must require tokenizer specification"
require_field requires_tokenization_boundary_contract 1 "tokenizer manifest must require tokenization boundary"
require_field requires_prompt_evaluation_handoff_contract 1 "tokenizer manifest must require prompt evaluation handoff"
require_field requires_awareness_dialogue_contract 1 "tokenizer manifest must require awareness dialogue"
require_field requires_prompt_materialization_contract 1 "tokenizer manifest must require prompt materialization"
require_field requires_prompt_receipt_contract 1 "tokenizer manifest must require prompt receipt"
require_field requires_prompt_buffer_boundary 1 "tokenizer manifest must require prompt buffer boundary"
require_field requires_protective_safety_boundary 1 "tokenizer manifest must require protective safety"
require_field requires_operator_review 1 "tokenizer manifest must require operator review"
require_field requires_official_source_snapshot 1 "tokenizer manifest must require official source snapshot"
require_field requires_future_tokenizer_artifact_inventory_contract 1 "tokenizer manifest must require future tokenizer artifact inventory contract"
require_field tokenizer_manifest_promotion_allowed 0 "tokenizer manifest must not allow promotion"
require_field requires_manifest_identity 1 "manifest identity must be required"
require_field requires_manifest_schema_review 1 "manifest schema review must be required"
require_field requires_model_tokenizer_compatibility_reference 1 "model-tokenizer compatibility reference must be required"
require_field requires_tokenizer_family_match 1 "tokenizer family match must be required"
require_field requires_tokenizer_format_match 1 "tokenizer format match must be required"
require_field requires_tokenizer_file_inventory 1 "tokenizer file inventory must be required"
require_field requires_vocabulary_file_entry 1 "vocabulary file entry must be required"
require_field requires_merges_file_entry_review 1 "merges file entry review must be required"
require_field requires_special_tokens_entry 1 "special tokens entry must be required"
require_field requires_bos_eos_policy_entry 1 "BOS/EOS policy entry must be required"
require_field requires_chat_template_entry 1 "chat template entry must be required"
require_field requires_prompt_template_boundary_entry 1 "prompt template boundary entry must be required"
require_field requires_unicode_policy_entry 1 "unicode policy entry must be required"
require_field requires_normalization_policy_entry 1 "normalization policy entry must be required"
require_field requires_context_window_entry 1 "context window entry must be required"
require_field requires_stop_sequence_entry 1 "stop sequence entry must be required"
require_field requires_license_and_source_entry 1 "license/source entry must be required"
...
require_field tokenizer_manifest_validation_authority 0 "tokenizer manifest validation authority must remain denied"
require_field tokenizer_manifest_load_authority 0 "tokenizer manifest load authority must remain denied"
require_field tokenizer_manifest_bytes_read 0 "tokenizer manifest bytes must not be read"
require_field tokenizer_manifest_hash_computed 0 "tokenizer manifest hash must not be computed"
require_field tokenizer_manifest_entries_loaded 0 "tokenizer manifest entries must not be loaded"
require_field tokenizer_manifest_file_count 0 "tokenizer manifest file count must remain zero"
require_field tokenizer_manifest_vocab_entry_loaded 0 "tokenizer manifest vocab entry must not be loaded"
require_field tokenizer_manifest_merges_entry_loaded 0 "tokenizer manifest merges entry must not be loaded"
require_field tokenizer_manifest_special_tokens_entry_loaded 0 "tokenizer manifest special tokens entry must not be loaded"
require_field tokenizer_file_path_resolved 0 "tokenizer file path must not be resolved"
require_field tokenizer_vocab_path_resolved 0 "tokenizer vocab path must not be resolved"
...
TOKENIZER_MANIFEST_CONTRACT_STATUS=$(field_or_unknown tokenizer_manifest_contract_status "$TOKENIZER_MANIFEST_ABS")
TOKENIZER_MANIFEST_STAGE=$(field_or_unknown tokenizer_manifest_stage "$TOKENIZER_MANIFEST_ABS")
TOKENIZER_SPECIFICATION_STAGE=$(field_or_unknown tokenizer_specification_stage "$TOKENIZER_MANIFEST_ABS")
TOKENIZATION_BOUNDARY_STAGE=$(field_or_unknown tokenization_boundary_stage "$TOKENIZER_MANIFEST_ABS")
PROMPT_EVALUATION_HANDOFF_STAGE=$(field_or_unknown prompt_evaluation_handoff_stage "$TOKENIZER_MANIFEST_ABS")
AWARENESS_DIALOGUE_STAGE=$(field_or_unknown awareness_dialogue_stage "$TOKENIZER_MANIFEST_ABS")
TOKENIZER_MANIFEST_FAMILY=$(field_or_unknown tokenizer_manifest_family "$TOKENIZER_MANIFEST_ABS")
TOKENIZER_MANIFEST_FORMAT=$(field_or_unknown tokenizer_manifest_format "$TOKENIZER_MANIFEST_ABS")

cat > "$REPORT" <<REPORT
NADIA TOKENIZER ARTIFACT INVENTORY CONTRACT
...
tokenizer_artifact_inventory_family=$INVENTORY_FAMILY
tokenizer_artifact_inventory_format=$INVENTORY_FORMAT
...
[inputs]
tokenizer_manifest=$TOKENIZER_MANIFEST_ABS
tokenizer_manifest_measurement=$TOKENIZER_MANIFEST_MEASUREMENT
tokenizer_manifest_stage_required=19-tokenizer-manifest-contract
tokenizer_manifest_contract_status=$TOKENIZER_MANIFEST_CONTRACT_STATUS
...
tokenizer_manifest_family=$TOKENIZER_MANIFEST_FAMILY
tokenizer_manifest_format=$TOKENIZER_MANIFEST_FORMAT

[tokenizer_artifact_inventory_contract]
...
[tokenizer_artifact_inventory_requirements]
...
cp "$REPORT" "$OUT_DIR/latest-tokenizer-artifact-inventory-contract.txt"

printf 'NADIA_TOKENIZER_ARTIFACT_INVENTORY_CONTRACT=%s\n' "$REPORT"
printf 'NADIA_TOKENIZER_ARTIFACT_FILE_OPENED=0\n'
