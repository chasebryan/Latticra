...
fail() {
  printf 'fedora rpm build-evidence intake denial review contract: %s\n' "$1" >&2
  exit 1
}
...
doc='docs/FEDORA_RPM_BUILD_EVIDENCE_INTAKE_DENIAL_REVIEW_CONTRACT.md'
status='docs/status/FEDORA_RPM_BUILD_EVIDENCE_INTAKE_DENIAL_REVIEW_CONTRACT_STATUS.md'
index='docs/status/README.md'
docs_readme='docs/README.md'
readme='README.md'
packaging_readme='packaging/fedora/README.md'
test_script='scripts/test-fedora-rpm-build-evidence-intake-denial-review-contract.sh'
workflow='.github/workflows/fedora-rpm-build-evidence-intake-denial-review-contract.yml'
...
denial_status='docs/status/FEDORA_RPM_BUILD_EVIDENCE_INTAKE_DENIAL_CONTRACT_STATUS.md'
...
require_contains 'Status: active RPM build-evidence intake denial review contract' "$doc"
...
require_contains 'rpm_build_evidence_intake_denial_review_contract_present=1' "$doc"
...
require_contains 'denial_review_required_before_re_request=1' "$doc"
...
require_contains 'Status: rpm-build-evidence-intake-denial-review/status alignment' "$status"
...
require_contains "$disposition_doc" "$doc"
require_contains "$disposition_test" "$doc"
require_contains "$disposition_workflow" "$doc"
require_contains "$disposition_doc" "$status"
require_contains "$disposition_test" "$status"
require_contains "$disposition_workflow" "$status"

require_contains 'FEDORA_RPM_BUILD_EVIDENCE_INTAKE_DENIAL_REVIEW_CONTRACT_STATUS.md' "$index"
require_contains 'Current Fedora RPM build-evidence intake denial review contract checkpoint' "$index"
require_contains 'The Fedora RPM build-evidence intake denial review contract is present and upholds the denied intake state while re-request authority remains closed' "$index"

require_contains 'FEDORA_RPM_BUILD_EVIDENCE_INTAKE_DENIAL_REVIEW_CONTRACT.md' "$docs_readme"
require_contains 'docs/FEDORA_RPM_BUILD_EVIDENCE_INTAKE_DENIAL_REVIEW_CONTRACT.md' "$readme"
require_contains 'docs/status/FEDORA_RPM_BUILD_EVIDENCE_INTAKE_DENIAL_REVIEW_CONTRACT_STATUS.md' "$readme"
require_contains 'The Fedora RPM build-evidence intake denial review contract is present' "$readme"

require_contains 'docs/FEDORA_RPM_BUILD_EVIDENCE_INTAKE_DENIAL_REVIEW_CONTRACT.md' "$packaging_readme"
require_contains 'scripts/test-fedora-rpm-build-evidence-intake-denial-review-contract.sh' "$packaging_readme"
require_contains 'fedora_rpm_build_evidence_intake_denial_review_contract_present=1' "$packaging_readme"
require_contains 'build_evidence_intake_denial_review_state=reviewed-upheld-no-effect' "$packaging_readme"

require_contains "$doc" "$denial_doc"
require_contains "$doc" "$denial_status"
require_contains 'Add a Fedora RPM build-evidence intake denial disposition closeout archive gate review disposition closeout archive gate review contract before any reviewed archive gate disposition closeout archive gate can be relaxed or re-requested.' "$denial_doc"
require_contains 'Add a Fedora RPM build-evidence intake denial disposition closeout archive gate review disposition closeout archive gate review contract before any reviewed archive gate disposition closeout archive gate can be relaxed or re-requested.' "$denial_status"
require_contains 'fedora_rpm_build_evidence_intake_denial_review_contract_present=1' "$matrix_script"
require_contains 'fedora_rpm_build_evidence_intake_denial_review_contract_present=1' "$matrix_status"
require_contains 'fedora_rpm_build_evidence_intake_denial_review_contract_present=1' "$matrix_test"
...
require_contains 'name: Fedora RPM Build Evidence Intake Denial Review Contract' "$workflow"
...
require_contains 'sh scripts/test-fedora-rpm-build-evidence-intake-denial-review-contract.sh' "$workflow"
...
printf 'fedora_rpm_build_evidence_intake_denial_review_contract: ok\n'
