...
require_contains 'require_file scripts/test-latticra-prevention-surface-consistency.sh' "$runner"
...
require_contains 'sh scripts/test-latticra-prevention-surface-consistency.sh' "$runner"
...
require_contains '/usr/bin/latticra --prevention-evidence' "$runner"
require_contains '/usr/bin/latticra --prevention-gate' "$runner"
require_contains '/usr/bin/latticra --prevention-fixtures' "$runner"
...
require_contains 'usage: latticra [--status|--version|--report|--prevention-research|--prevention-boundary|--prevention-evidence|--prevention-gate|--prevention-fixtures|--prevention-method <id>]' "$runner"
require_contains 'LATTICRA PREVENTION RESEARCH REPORT' "$runner"
require_contains 'LATTICRA PREVENTION BOUNDARY REPORT' "$runner"
require_contains 'LATTICRA PREVENTION EVIDENCE REPORT' "$runner"
require_contains 'LATTICRA PREVENTION GATE REPORT' "$runner"
require_contains 'LATTICRA PREVENTION FIXTURE REPORT' "$runner"
...
require_contains 'source_owasp_input_validation=https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html' "$runner"
require_contains 'source_owasp_asvs=https://owasp.org/www-project-application-security-verification-standard/' "$runner"
require_contains 'input_validation_position=early-syntactic-and-semantic-gate' "$runner"
require_contains 'input_validation_not_primary_sql_xss_defense=1' "$runner"
require_contains 'allowlist_validation_primary_required=1' "$runner"
require_contains 'server_side_validation_required=1' "$runner"
...
require_contains 'evidence_schema_version=1' "$runner"
require_contains 'evidence_source_sink_map_required=1' "$runner"
require_contains 'evidence_safe_api_trace_required=1' "$runner"
require_contains 'gate_unmapped_boundary_blocks_release=1' "$runner"
require_contains 'gate_missing_negative_fixture_blocks_release=1' "$runner"
require_contains 'gate_secret_capture_blocks_release=1' "$runner"
require_contains 'gate_schema_version=1' "$runner"
require_contains 'gate_default=block-until-evidence-complete' "$runner"
require_contains 'gate_fixture_coverage_required=1' "$runner"
require_contains 'gate_runtime_evidence_required_for_protection_claim=1' "$runner"
require_contains 'release_without_complete_evidence_allowed=0' "$runner"
require_contains 'evidence_fail_closed_result_required=1' "$runner"
require_contains 'fixture_schema_version=1' "$runner"
require_contains 'fixture_set_count=16' "$runner"
require_contains 'fixture_payload_strings_in_report=0' "$runner"
require_contains 'fixture_safe_harness_required=1' "$runner"
...
require_contains 'cli_prevention_evidence_output_recorded=1' "$runner"
require_contains 'cli_prevention_gate_output_recorded=1' "$runner"
require_contains 'cli_prevention_fixtures_output_recorded=1' "$runner"
require_contains 'cli_prevention_method_output_recorded=1' "$runner"
require_contains 'cli_prevention_surface_consistency_guard_passed=1' "$runner"
