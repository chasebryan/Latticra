...
tmpdir="$(mktemp -d "${TMPDIR:-/tmp}/latticra-kernel-runtime-entry-recovery-audit-observation-view-report.XXXXXX")"
trap 'rm -rf "$tmpdir"' EXIT INT HUP TERM

report_bin="$tmpdir/latticra-kernel-runtime-entry-recovery-audit-observation-view-report"
report_txt="$tmpdir/latticra-kernel-runtime-entry-recovery-audit-observation-view-report.txt"
...
cc $CFLAGS \
  ...
  tools/kernel_runtime_entry_recovery_audit_observation_view_report.c \
  -o "$report_bin"

"$report_bin" > "$report_txt"

grep -Fq 'LATTICRA KERNEL RUNTIME ENTRY RECOVERY AUDIT OBSERVATION VIEW REPORT' "$report_txt"
grep -Fq 'recovery_audit_observation_view_status=runtime-entry-recovery-audit-observation-view-seed-ready' "$report_txt"
...
grep -Fq 'runtime_entry_recovery_outcome_observation_view_status=runtime-entry-recovery-outcome-observation-view-seed-ready' "$report_txt"
...
grep -Fq 'recovery_audit_observation_view[0].process_label=kernel-report-process-metadata' "$report_txt"
grep -Fq 'recovery_audit_observation_view[0].recovery_audit_observation_view_class=kernel-report-runtime-entry-recovery-audit-observation-view' "$report_txt"
grep -Fq 'recovery_audit_observation_view[0].recovery_audit_observation_view_token=0' "$report_txt"
grep -Fq 'recovery_audit_observation_view[0].recovery_closeout_observation_view_token=0' "$report_txt"
grep -Fq 'recovery_audit_observation_view[0].recovery_outcome_observation_view_token=0' "$report_txt"
grep -Fq 'recovery_audit_observation_view[0].recovery_audit_observation_slot=16384' "$report_txt"
grep -Fq 'recovery_audit_observation_view[0].recovery_closeout_observation_slot=15360' "$report_txt"
grep -Fq 'recovery_audit_observation_view[0].recovery_audit_slot=37' "$report_txt"
grep -Fq 'recovery_audit_observation_view[0].recovery_closeout_slot=36' "$report_txt"
grep -Fq 'recovery_audit_observation_view[0].recovery_audit_observation_declared=1' "$report_txt"
grep -Fq 'recovery_audit_observation_view[0].recovery_audit_declared=1' "$report_txt"
grep -Fq 'recovery_audit_observation_view[0].recovery_audit_observation_planned=0' "$report_txt"
grep -Fq 'recovery_audit_observation_view[0].runtime_entry_recovery_audit_observation_view_allowed=0' "$report_txt"
grep -Fq 'recovery_audit_observation_view[0].runtime_entry_recovery_closeout_observation_view_allowed=0' "$report_txt"
grep -Fq 'recovery_audit_observation_view[0].recovery_audit_observation_allowed=0' "$report_txt"
grep -Fq 'recovery_audit_observation_view[0].recovery_audit_allowed=0' "$report_txt"
grep -Fq 'recovery_audit_observation_view[0].recovery_closeout_observation_allowed=0' "$report_txt"
grep -Fq 'recovery_audit_observation_view[0].recovery_closeout_allowed=0' "$report_txt"
grep -Fq 'recovery_audit_observation_view[0].no_effect=1' "$report_txt"

printf 'kernel_runtime_entry_recovery_audit_observation_view_report_runner: ok\n'
