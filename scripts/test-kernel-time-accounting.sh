...
require_file() {
  file="$1"
  if [ ! -f "$file" ]; then
    printf 'kernel_time_accounting: missing file: %s\n' "$file" >&2
    exit 1
  fi
}

require_file docs/KERNEL_TIME_ACCOUNTING_SEED.md
...
tmpdir="$(mktemp -d "${TMPDIR:-/tmp}/latticra-kernel-time-accounting.XXXXXX")"
trap 'rm -rf "$tmpdir"' EXIT INT HUP TERM
bin="$tmpdir/latticra-kernel-time-accounting"
