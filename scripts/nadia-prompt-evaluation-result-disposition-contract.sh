...
usage() {
  cat <<'USAGE'
Usage:
  nadia-prompt-evaluation-result-disposition-contract.sh [options]

Options:
  --prompt-evaluation-result-review <file>  Stage-32 Nadia prompt-evaluation result review contract
  --request-class <label>                   Operator request classification label
  --disposition-family <label>              Planned prompt-evaluation result disposition family label
  --disposition-format <label>              Planned prompt-evaluation result disposition format label
  --output <dir>                            Output directory for prompt-evaluation result disposition contracts

Creates Nadia Stage-33 prompt-evaluation result disposition contract metadata
only. It verifies the prompt-evaluation result review contract and records
planned disposition requirements before any disposition record, release record,
result-review record, result record, model-output access, runtime invocation,
prompt evaluation, dialogue generation, token generation, or inference
boundary. It does not create disposition records, create release records,
...
fail() {
  printf 'nadia prompt evaluation result disposition contract: %s\n' "$1" >&2
  exit "${2:-1}"
}
...
require_field() {
  ...
  expected="$2"
  message="$3"
  actual=$(field_or_unknown "$key" "$PROMPT_EVALUATION_RESULT_REVIEW_ABS")
...
reject_boundary_label() {
  ...
  case "$normalized" in
    *sex*|*sexual*|*porn*|*erotic*|*nude*|*nudity*|*nsfw*|*fetish*|*adult-content*)
      fail "$name is outside Nadia prompt-evaluation-result-disposition boundary" 65
      ;;
  esac
}

PROMPT_EVALUATION_RESULT_REVIEW=""
REQUEST_CLASS="awareness-education"
DISPOSITION_FAMILY="operator-reviewed-prompt-evaluation-result-disposition"
DISPOSITION_FORMAT="contract-only-offline-evaluation-result-disposition"
OUTPUT=""

while [ "$#" -gt 0 ]; do
  case "$1" in
    --prompt-evaluation-result-review)
      [ "$#" -ge 2 ] || fail "missing value for --prompt-evaluation-result-review" 64
      PROMPT_EVALUATION_RESULT_REVIEW="$2"
      shift 2
      ;;
    --request-class)
...
if [ -z "$PROMPT_EVALUATION_RESULT_REVIEW" ] && [ -f "reports/nadia/prompt-evaluation-result-review/latest-prompt-evaluation-result-review-contract.txt" ]; then
  PROMPT_EVALUATION_RESULT_REVIEW="reports/nadia/prompt-evaluation-result-review/latest-prompt-evaluation-result-review-contract.txt"
fi

[ -n "$PROMPT_EVALUATION_RESULT_REVIEW" ] || fail "prompt evaluation result review contract is required" 64
[ -f "$PROMPT_EVALUATION_RESULT_REVIEW" ] || fail "prompt evaluation result review contract not found: $PROMPT_EVALUATION_RESULT_REVIEW" 66
...
if [ -z "$OUTPUT" ]; then
  OUTPUT="reports/nadia/prompt-evaluation-result-disposition"
fi
mkdir -p "$OUTPUT"
OUT_DIR=$(CDPATH= cd -- "$OUTPUT" && pwd -P)

TS=${NADIA_PROMPT_EVALUATION_RESULT_DISPOSITION_TIMESTAMP:-$(date -u +"%Y%m%dT%H%M%SZ")}
REPORT="$OUT_DIR/nadia-prompt-evaluation-result-disposition-contract-$TS.txt"

PROMPT_EVALUATION_RESULT_REVIEW_ABS=$(cd "$(dirname -- "$PROMPT_EVALUATION_RESULT_REVIEW")" && printf '%s/%s\n' "$(pwd -P)" "$(basename -- "$PROMPT_EVALUATION_RESULT_REVIEW")")
PROMPT_EVALUATION_RESULT_REVIEW_MEASUREMENT=$(measure_file "$PROMPT_EVALUATION_RESULT_REVIEW_ABS")

require_field stage 32-prompt-evaluation-result-review-contract "prompt evaluation result review stage must be 32-prompt-evaluation-result-review-contract"
require_field prompt_evaluation_result_review_contract_status contract_only "prompt evaluation result review contract must remain contract-only"
require_field prompt_evaluation_result_review_stage contract-only "prompt evaluation result review stage must be contract-only"
require_field prompt_evaluation_result_review_authority 0 "prompt evaluation result review authority must remain denied"
require_field prompt_evaluation_result_review_allowed 0 "prompt evaluation result review must remain disallowed"
require_field prompt_evaluation_result_review_recorded 0 "prompt evaluation result review must not be recorded"
require_field prompt_evaluation_result_review_created 0 "prompt evaluation result review must not be created"
require_field prompt_evaluation_result_review_performed 0 "prompt evaluation result review must not be performed"
require_field prompt_evaluation_result_review_metadata_present 1 "prompt evaluation result review metadata must be present"
require_field prompt_evaluation_result_review_decision blocked_contract_only "prompt evaluation result review decision must be blocked_contract_only"
require_field prompt_evaluation_result_review_evidence_present 1 "prompt evaluation result review evidence must be present"
require_field prompt_evaluation_result_review_source_policy operator-reviewed-offline "prompt evaluation result review source policy must be operator-reviewed-offline"
require_field prompt_evaluation_result_review_plan_recorded 1 "prompt evaluation result review plan must be recorded"
require_field prompt_evaluation_result_review_result_recorded 0 "prompt evaluation result review result must remain absent"
require_field prompt_evaluation_result_review_runtime_invoked 0 "prompt evaluation result review runtime must not be invoked"
require_field requires_prompt_evaluation_result_contract 1 "prompt evaluation result contract must be required"
require_field requires_future_prompt_evaluation_result_disposition_contract 1 "future prompt evaluation result disposition contract must be required"
require_field prompt_evaluation_result_review_promotion_allowed 0 "prompt evaluation result review must not allow promotion"
...
require_field requires_prompt_evaluation_result_review_schema_policy 1 "prompt evaluation result review schema policy must be required"
require_field requires_prompt_evaluation_result_review_denial_policy 1 "prompt evaluation result review denial policy must be required"
require_field requires_prompt_evaluation_result_disposition_policy 1 "prompt evaluation result disposition policy must be required"
...
PROMPT_EVALUATION_RESULT_REVIEW_CONTRACT_STATUS=$(field_or_unknown prompt_evaluation_result_review_contract_status "$PROMPT_EVALUATION_RESULT_REVIEW_ABS")
PROMPT_EVALUATION_RESULT_REVIEW_STAGE=$(field_or_unknown prompt_evaluation_result_review_stage "$PROMPT_EVALUATION_RESULT_REVIEW_ABS")
PROMPT_EVALUATION_RESULT_REVIEW_FAMILY=$(field_or_unknown prompt_evaluation_result_review_family "$PROMPT_EVALUATION_RESULT_REVIEW_ABS")
PROMPT_EVALUATION_RESULT_REVIEW_FORMAT=$(field_or_unknown prompt_evaluation_result_review_format "$PROMPT_EVALUATION_RESULT_REVIEW_ABS")
PROMPT_EVALUATION_RESULT_STAGE=$(field_or_unknown prompt_evaluation_result_stage "$PROMPT_EVALUATION_RESULT_REVIEW_ABS")
PROMPT_EVALUATION_INVOCATION_STAGE=$(field_or_unknown prompt_evaluation_invocation_stage "$PROMPT_EVALUATION_RESULT_REVIEW_ABS")
PROMPT_EVALUATION_RUNTIME_HANDOFF_STAGE=$(field_or_unknown prompt_evaluation_runtime_handoff_stage "$PROMPT_EVALUATION_RESULT_REVIEW_ABS")
PROMPT_EVALUATION_INPUT_STAGE=$(field_or_unknown prompt_evaluation_input_stage "$PROMPT_EVALUATION_RESULT_REVIEW_ABS")
CONTEXT_WINDOW_ASSEMBLY_STAGE=$(field_or_unknown context_window_assembly_stage "$PROMPT_EVALUATION_RESULT_REVIEW_ABS")
PROMPT_TOKEN_SEQUENCE_STAGE=$(field_or_unknown prompt_token_sequence_stage "$PROMPT_EVALUATION_RESULT_REVIEW_ABS")
PROMPT_TOKENIZATION_STAGE=$(field_or_unknown prompt_tokenization_stage "$PROMPT_EVALUATION_RESULT_REVIEW_ABS")
...
prompt_evaluation_result_disposition_family=$DISPOSITION_FAMILY
prompt_evaluation_result_disposition_format=$DISPOSITION_FORMAT
prompt_evaluation_result_disposition_schema_planned=1
prompt_evaluation_result_disposition_schema_validated=0
...
[inputs]
prompt_evaluation_result_review=$PROMPT_EVALUATION_RESULT_REVIEW_ABS
prompt_evaluation_result_review_measurement=$PROMPT_EVALUATION_RESULT_REVIEW_MEASUREMENT
prompt_evaluation_result_review_stage_required=32-prompt-evaluation-result-review-contract
prompt_evaluation_result_review_contract_status=$PROMPT_EVALUATION_RESULT_REVIEW_CONTRACT_STATUS
prompt_evaluation_result_review_stage=$PROMPT_EVALUATION_RESULT_REVIEW_STAGE
prompt_evaluation_result_review_family=$PROMPT_EVALUATION_RESULT_REVIEW_FAMILY
prompt_evaluation_result_review_format=$PROMPT_EVALUATION_RESULT_REVIEW_FORMAT
...
[prompt_evaluation_result_disposition_contract]
...
[prompt_evaluation_result_disposition_requirements]
...
prompt_evaluation_result_disposition_performed=0
prompt_evaluation_result_disposition_created=0
prompt_evaluation_result_disposition_materialized=0
...
cp "$REPORT" "$OUT_DIR/latest-prompt-evaluation-result-disposition-contract.txt"

printf 'NADIA_PROMPT_EVALUATION_RESULT_DISPOSITION_CONTRACT=%s\n' "$REPORT"
printf 'NADIA_PROMPT_EVALUATION_RESULT_DISPOSITION_RECORDED=0\n'
