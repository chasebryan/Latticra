...
usage() {
  cat <<'USAGE'
Usage:
  fedora-vm-cli-payload-repeatability-publication-receipt-acceptance.sh --acceptance <path>

Validates a supplied Fedora VM CLI payload repeatability publication receipt
acceptance candidate. It reads the provided file, rejects missing required
markers and placeholder values, and prints a no-effect validation report. It
...
RPM, remove an RPM, or mutate the host.
USAGE
}

fail() {
  printf 'fedora vm cli payload repeatability publication receipt acceptance: %s\n' "$1" >&2
  exit "${2:-1}"
}
...
reject_placeholders() {
  ...
  if grep -Eq -- '<required|<placeholder>|TODO|TBD' "$file"; then
    fail "placeholder value is not acceptable in acceptance candidate"
  fi
}
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    --acceptance)
      [ "$#" -ge 2 ] || fail "missing value for --acceptance" 64
      ACCEPTANCE_CANDIDATE="$2"
      shift 2
      ;;
    -h|--help)
      usage
...
[ -n "$ACCEPTANCE_CANDIDATE" ] || fail 'missing --acceptance path' 64
require_file "$ACCEPTANCE_CANDIDATE"
require_file docs/FEDORA_VM_CLI_PAYLOAD_REPEATABILITY_PUBLICATION_RECEIPT_ACCEPTANCE_TEMPLATE.md
require_file docs/status/FEDORA_VM_CLI_PAYLOAD_REPEATABILITY_PUBLICATION_RECEIPT_ACCEPTANCE_TEMPLATE_STATUS.md
...
require_file scripts/fedora-vm-cli-payload-repeatability-publication-receipt-acceptance-template.sh
require_file scripts/fedora-vm-cli-payload-repeatability-publication-receipt.sh

require_contains 'FEDORA VM CLI PAYLOAD REPEATABILITY PUBLICATION RECEIPT ACCEPTANCE' docs/FEDORA_VM_CLI_PAYLOAD_REPEATABILITY_PUBLICATION_RECEIPT_ACCEPTANCE_TEMPLATE.md
require_contains 'publication_receipt_acceptance_template_complete=0' docs/status/FEDORA_VM_CLI_PAYLOAD_REPEATABILITY_PUBLICATION_RECEIPT_ACCEPTANCE_TEMPLATE_STATUS.md
...
require_contains 'FEDORA VM CLI PAYLOAD REPEATABILITY PUBLICATION RECEIPT VALIDATION' docs/FEDORA_VM_CLI_PAYLOAD_REPEATABILITY_PUBLICATION_RECEIPT_VALIDATOR.md
require_contains 'FEDORA VM CLI PAYLOAD REPEATABILITY PUBLICATION RECEIPT ACCEPTANCE TEMPLATE' scripts/fedora-vm-cli-payload-repeatability-publication-receipt-acceptance-template.sh
...
do
  require_contains "$marker" "$ACCEPTANCE_CANDIDATE"
done

reject_placeholders "$ACCEPTANCE_CANDIDATE"
...
do
  require_absent "$forbidden_claim" "$ACCEPTANCE_CANDIDATE"
done
