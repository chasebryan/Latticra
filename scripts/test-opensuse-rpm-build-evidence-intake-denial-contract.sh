...
require_file() {
  file="$1"
  if [ ! -f "$file" ]; then
    printf 'opensuse rpm build-evidence intake denial contract: missing file: %s\n' "$file" >&2
    exit 1
  fi
}
...
require_contains() {
  ...
  if ! grep -Fq -- "$pattern" "$file"; then
    printf 'opensuse rpm build-evidence intake denial contract: missing required pattern in %s: %s\n' "$file" "$pattern" >&2
    exit 1
  fi
}
...
require_no_rpm_artifacts() {
  ...
  if find "$dir" \( -name '*.rpm' -o -name '*.src.rpm' \) -print | grep -q .; then
    printf 'opensuse rpm build-evidence intake denial contract: unexpected RPM artifact under %s\n' "$dir" >&2
    exit 1
  fi
}

contract='docs/OPENSUSE_RPM_BUILD_EVIDENCE_INTAKE_DENIAL_CONTRACT.md'
...
require_contains 'Status: active RPM build-evidence intake denial contract' "$contract"
require_contains 'documentation-only and static' "$contract"
require_contains 'opensuse_rpm_build_evidence_intake_denial_contract_present=1' "$contract"
require_contains 'rpm_build_evidence_intake_denial_contract_present=1' "$contract"
...
require_contains 'opensuse_build_evidence_intake_requested=0' "$contract"
...
require_contains 'rpmbuild_evidence_accepted=0' "$contract"
require_contains 'osc_build_evidence_accepted=0' "$contract"
require_contains 'rpmlint_evidence_accepted=0' "$contract"
require_contains 'source_rpm_evidence_accepted=0' "$contract"
require_contains 'binary_rpm_evidence_accepted=0' "$contract"
...
require_contains 'rpmlint_transcript_reference' "$contract"
...
require_contains 'OPENSUSE_RPM_BUILD_EVIDENCE_INTAKE_DENIAL_CONTRACT.md' docs/README.md
require_contains 'OPENSUSE_RPM_BUILD_EVIDENCE_INTAKE_DENIAL_CONTRACT.md' docs/status/README.md

require_contains 'opensuse_rpm_build_evidence_intake_denial_contract_present=1' docs/status/OPENSUSE_ECOSYSTEM_INTEGRATION_STATUS.md
require_contains 'rpm_build_evidence_intake_denial_contract_present=1' docs/status/OPENSUSE_ECOSYSTEM_INTEGRATION_STATUS.md
require_contains 'build_evidence_intake_denial_state=denied-no-effect' docs/status/OPENSUSE_ECOSYSTEM_INTEGRATION_STATUS.md
require_contains 'opensuse_build_evidence_intake_denied=1' docs/status/OPENSUSE_ECOSYSTEM_INTEGRATION_STATUS.md
require_contains 'opensuse_build_transcript_intake_accepted=0' docs/status/OPENSUSE_ECOSYSTEM_INTEGRATION_STATUS.md
require_contains 'opensuse_rpmbuild_evidence_accepted=0' docs/status/OPENSUSE_ECOSYSTEM_INTEGRATION_STATUS.md
require_contains 'opensuse_osc_build_evidence_accepted=0' docs/status/OPENSUSE_ECOSYSTEM_INTEGRATION_STATUS.md
require_contains 'opensuse_rpmlint_evidence_accepted=0' docs/status/OPENSUSE_ECOSYSTEM_INTEGRATION_STATUS.md

require_contains 'opensuse_rpm_build_evidence_intake_denial_contract_present=1' packaging/opensuse/README.md
require_contains 'rpm_build_evidence_intake_denial_contract_present=1' packaging/opensuse/README.md
...
require_contains 'opensuse_build_transcript_intake_accepted=0' packaging/opensuse/README.md
require_contains 'opensuse_rpmbuild_evidence_accepted=0' packaging/opensuse/README.md
require_contains 'opensuse_osc_build_evidence_accepted=0' packaging/opensuse/README.md
require_contains 'opensuse_rpmlint_evidence_accepted=0' packaging/opensuse/README.md

require_no_rpm_artifacts packaging/opensuse

require_contains 'Run openSUSE RPM build evidence intake denial contract guard' .github/workflows/opensuse-rpm-build-evidence-intake-denial-contract.yml
require_contains 'sh scripts/test-opensuse-rpm-build-evidence-intake-denial-contract.sh' .github/workflows/opensuse-rpm-build-evidence-intake-denial-contract.yml

printf 'opensuse_rpm_build_evidence_intake_denial_contract: ok\n'
