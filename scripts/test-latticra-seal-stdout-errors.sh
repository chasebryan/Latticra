...
require_contains "commitment_output=redacted" "$tmpdir/hybrid.out"
...
require_contains "detached_commitment_checked_before_decrypt=1" "$tmpdir/hybrid.out"
require_contains "detached_commitment_tampering_rejected=1" "$tmpdir/hybrid.out"
require_contains "cli_record_buffer_zeroized=1" "$tmpdir/hybrid.out"
require_contains "cli_committed_ciphertext_buffer_zeroized=1" "$tmpdir/hybrid.out"
require_contains "cli_recovered_plaintext_buffer_zeroized=1" "$tmpdir/hybrid.out"
require_contains "cli_committed_recovered_plaintext_buffer_zeroized=1" "$tmpdir/hybrid.out"
require_contains "committed_detached_envelope_self_check=pass" "$tmpdir/hybrid.out"
