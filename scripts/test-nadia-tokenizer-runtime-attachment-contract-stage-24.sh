...
if [ -n "${NADIA_TEST_TMP_ROOT:-}" ]; then
  ...
  tmp_base="${TMPDIR:-/tmp}"
  tmp_root=$(mktemp -d "$tmp_base/latticra-nadia-stage24-test.XXXXXX")
  trap 'rm -rf "$tmp_root"' EXIT INT HUP TERM
fi

log_out="$tmp_root/stage24/logs"
mkdir -p "$log_out"
NADIA_TEST_TMP_ROOT="$tmp_root" sh "$stage23_guard" >"$log_out/prereq-stage23.out"

out="$tmp_root/stage24/tokenizer-runtime-attachment"
binding="$tmp_root/stage23/tokenizer-artifact-binding/nadia-tokenizer-artifact-binding-contract-stage23-test.txt"
rm -rf "$out"
mkdir -p "$out"
...
NADIA_TOKENIZER_RUNTIME_ATTACHMENT_TIMESTAMP=stage24-test sh "$attachment_script" \
  ...
  --output "$out" >"$log_out/attachment.out"
...
require_contains 'outside Nadia tokenizer-runtime-attachment boundary' "$log_out/reject.err"
