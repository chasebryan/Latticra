...
require_file() {
  file="$1"
  if [ ! -f "$file" ]; then
    printf 'ubuntu ppa/archive publication evidence intake denial disposition closeout archive gate contract: missing file: %s\n' "$file" >&2
    exit 1
  fi
}
...
require_contains() {
  ...
  if ! grep -Fq -- "$pattern" "$file"; then
    printf 'ubuntu ppa/archive publication evidence intake denial disposition closeout archive gate contract: missing required pattern in %s: %s\n' "$file" "$pattern" >&2
    exit 1
  fi
}
...
require_no_ubuntu_artifacts() {
  ...
  if find "$dir" \( -name '*.deb' -o -name '*.dsc' -o -name '*.changes' -o -name '*.build' -o -name '*.buildinfo' \) -print | grep -q .; then
    printf 'ubuntu ppa/archive publication evidence intake denial disposition closeout archive gate contract: unexpected Ubuntu package artifact under %s\n' "$dir" >&2
    exit 1
  fi
}

contract='docs/UBUNTU_PPA_ARCHIVE_PUBLICATION_EVIDENCE_INTAKE_DENIAL_DISPOSITION_CLOSEOUT_ARCHIVE_GATE_CONTRACT.md'
...
workflow='.github/workflows/ubuntu-ppa-archive-publication-evidence-intake-denial-disposition-closeout-archive-gate-contract.yml'
archive_gate_review_workflow='.github/workflows/ubuntu-ppa-archive-publication-evidence-intake-denial-disposition-closeout-archive-gate-review-contract.yml'
...
require_file "$archive_gate_review_workflow"
require_file README.md
require_file docs/QUICK_START_CHEATSHEET.md
require_file packaging/ubuntu/README.md

require_contains 'Status: no-effect publication evidence intake denial disposition closeout archive gate contract' "$contract"
...
require_contains 'publication_evidence_denial_archive_gate_identifier' "$contract"
...
require_contains 'publication_evidence_denial_archive_gate_decision' "$contract"
require_contains 'publication_evidence_denial_archive_gate_reason' "$contract"
require_contains 'publication_evidence_denial_archive_gate_reviewer' "$contract"
...
require_contains 'docs/UBUNTU_PPA_ARCHIVE_PUBLICATION_EVIDENCE_INTAKE_DENIAL_DISPOSITION_CLOSEOUT_ARCHIVE_GATE_REVIEW_CONTRACT.md' "$contract"
require_contains 'scripts/test-ubuntu-ppa-archive-publication-evidence-intake-denial-disposition-closeout-archive-gate-review-contract.sh' "$contract"
require_contains '.github/workflows/ubuntu-ppa-archive-publication-evidence-intake-denial-disposition-closeout-archive-gate-review-contract.yml' "$contract"
...
require_contains 'ubuntu_ppa_archive_publication_evidence_intake_denial_disposition_closeout_archive_gate_contract: ok' "$contract"

require_contains "$contract" "$archive_gate_review_contract"
require_contains 'publication_evidence_intake_denial_disposition_closeout_archive_gate_review_state=reviewed-upheld-no-effect' "$archive_gate_review_contract"
require_contains 'publication_evidence_denial_archive_gate_review_decision=uphold-closed-archive-gate' "$archive_gate_review_contract"
...
require_contains "$archive_gate_review_contract" "$closeout_contract"
require_contains "$archive_gate_review_contract" "$disposition_contract"
require_contains "$archive_gate_review_contract" "$review_contract"
require_contains "$archive_gate_review_contract" "$denial_contract"
require_contains "$archive_gate_review_contract" "$matrix_contract"
require_contains "$archive_gate_review_contract" "$publication_gate"
require_contains "$archive_gate_review_contract" "$static_validation"
require_contains "$archive_gate_review_contract" "$readiness"
require_contains "$archive_gate_review_contract" "$status"
require_contains "$archive_gate_review_contract" packaging/ubuntu/README.md
require_contains "$archive_gate_review_contract" README.md

for file in "$status" packaging/ubuntu/README.md README.md
do
  require_contains 'ubuntu_ppa_archive_publication_evidence_intake_denial_disposition_closeout_archive_gate_contract_present=1' "$file"
  require_contains 'ubuntu_publication_evidence_intake_denial_disposition_closeout_archive_gate_contract_present=1' "$file"
  ...
  require_contains 'publication_evidence_intake_denial_disposition_closeout_archive_gate_state=closed-no-effect' "$file"
  require_contains 'publication_evidence_denial_archive_gate_present=1' "$file"
  require_contains 'publication_evidence_denial_archive_gate_state=closed-no-effect' "$file"
  require_contains 'publication_evidence_denial_archive_gate_decision=deny-archive-and-re-request' "$file"
...
require_contains 'Ubuntu PPA Archive Publication Evidence Intake Denial Disposition Closeout Archive Gate Contract' "$workflow"
require_contains 'sh scripts/test-ubuntu-ppa-archive-publication-evidence-intake-denial-disposition-closeout-archive-gate-contract.sh' "$workflow"
require_contains 'Ubuntu PPA Evidence Intake Denial Archive Gate Review Contract' "$archive_gate_review_workflow"
require_contains 'sh scripts/test-ubuntu-ppa-archive-publication-evidence-intake-denial-disposition-closeout-archive-gate-review-contract.sh' "$archive_gate_review_workflow"

require_no_ubuntu_artifacts packaging/ubuntu

printf 'ubuntu_ppa_archive_publication_evidence_intake_denial_disposition_closeout_archive_gate_contract: ok\n'
