...
require_contains 'Current updater policy alignment' "$status"
require_contains 'ok: updater config' "$status"
require_contains 'ok: updater policy' "$status"
require_contains 'ok: updater status report' "$status"
require_contains 'ok: updater status dry-run command' "$status"
require_contains 'ok: updater status apply command' "$status"
require_contains 'ok: updater status network fetch authority disabled' "$status"
require_contains 'updater_preview_command=updater dry-run' "$status"
require_contains 'updater_apply_command=updater apply' "$status"
require_contains 'updater_apply_mode=guarded-local-prefix-reinstall' "$status"
require_contains 'updater_network_fetch_authority=0' "$status"
...
require_contains 'updater_policy_present=1' "$status"
require_contains 'updater_status_report_present=1' "$status"
