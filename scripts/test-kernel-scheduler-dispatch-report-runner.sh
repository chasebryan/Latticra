...
tmpdir="$(mktemp -d "${TMPDIR:-/tmp}/latticra-kernel-scheduler-dispatch-report.XXXXXX")"
trap 'rm -rf "$tmpdir"' EXIT INT HUP TERM

report_bin="$tmpdir/latticra-kernel-scheduler-dispatch-report"
report_txt="$tmpdir/latticra-kernel-scheduler-dispatch-report.txt"
...
grep -Fq 'LATTICRA KERNEL SCHEDULER DISPATCH REPORT' "$report_txt"
grep -Fq 'dispatch_status=scheduler-dispatch-seed-ready' "$report_txt"
...
grep -Fq 'dispatch_count=4' "$report_txt"
...
grep -Fq 'dispatch[0].process_label=kernel-report-process-metadata' "$report_txt"
grep -Fq 'dispatch[0].dispatch_class=kernel-report-scheduler-dispatch' "$report_txt"
grep -Fq 'dispatch[0].dispatch_rank=1' "$report_txt"
grep -Fq 'dispatch[0].scheduler_credit_ns=10000000' "$report_txt"
grep -Fq 'dispatch[0].dispatch_declared=1' "$report_txt"
grep -Fq 'dispatch[0].dispatch_planned=0' "$report_txt"
grep -Fq 'dispatch[0].dequeued=0' "$report_txt"
grep -Fq 'dispatch[0].dispatched=0' "$report_txt"
grep -Fq 'dispatch[0].context_switch_planned=0' "$report_txt"
grep -Fq 'dispatch[0].no_effect=1' "$report_txt"

printf 'kernel_scheduler_dispatch_report_runner: ok\n'
