...
usage() {
  cat <<'USAGE'
Usage:
  latticra-panel-signed-updater-state-transition-denial-disposition-closeout-audit-review-disposition.sh

Emits the no-effect disposition for the Latticra Panel signed updater state
transition denial disposition closeout audit review. It does not transition
update state, stage updates, write disposition files, write receipts, mutate
host state, or apply signed updates.
USAGE
}
...
signed_updater_state_transition_denial_disposition_closeout_audit_review_disposition_status=ok
repo_root=$ROOT
...
signed_updater_state_transition_denial_disposition_closeout_audit_review_reason=audit-confirms-closeout-preserves-retained-blocked-state-and-keeps-transition-apply-network-rollback-and-receipts-closed
