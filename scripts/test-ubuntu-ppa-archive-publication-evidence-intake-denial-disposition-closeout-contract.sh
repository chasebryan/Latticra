...
require_file() {
  file="$1"
  if [ ! -f "$file" ]; then
    printf 'ubuntu ppa/archive publication evidence intake denial disposition closeout contract: missing file: %s\n' "$file" >&2
    exit 1
  fi
}
...
require_contains() {
  ...
  if ! grep -Fq -- "$pattern" "$file"; then
    printf 'ubuntu ppa/archive publication evidence intake denial disposition closeout contract: missing required pattern in %s: %s\n' "$file" "$pattern" >&2
    exit 1
  fi
}
...
require_no_ubuntu_artifacts() {
  ...
  if find "$dir" \( -name '*.deb' -o -name '*.dsc' -o -name '*.changes' -o -name '*.build' -o -name '*.buildinfo' \) -print | grep -q .; then
    printf 'ubuntu ppa/archive publication evidence intake denial disposition closeout contract: unexpected Ubuntu package artifact under %s\n' "$dir" >&2
    exit 1
  fi
}

contract='docs/UBUNTU_PPA_ARCHIVE_PUBLICATION_EVIDENCE_INTAKE_DENIAL_DISPOSITION_CLOSEOUT_CONTRACT.md'
...
workflow='.github/workflows/ubuntu-ppa-archive-publication-evidence-intake-denial-disposition-closeout-contract.yml'
...
require_contains 'Status: no-effect publication evidence intake denial disposition closeout contract' "$contract"
...
require_contains 'ubuntu_ppa_archive_publication_evidence_intake_denial_disposition_closeout_contract: ok' "$contract"
...
require_contains "$archive_gate_contract" "$disposition_contract"
...
require_contains 'Ubuntu PPA Archive Publication Evidence Intake Denial Disposition Closeout Contract' "$workflow"
require_contains 'sh scripts/test-ubuntu-ppa-archive-publication-evidence-intake-denial-disposition-closeout-contract.sh' "$workflow"
...
printf 'ubuntu_ppa_archive_publication_evidence_intake_denial_disposition_closeout_contract: ok\n'
