...
for file in "$doc" "$status" "$status_index" "$foundation" "$installer_readme" "$ui_model" "$components_manifest" "$apply_script" "$panel_config" "$panel_ui" "$disposition_script" "$guard" "$workflow" Makefile
do
  require_file "$file"
done
...
require_contains 'requires_future_prompt_evaluation_result_release_receipt_review_disposition_release_contract=1' "$doc"
...
require_contains 'requires_future_prompt_evaluation_result_release_receipt_review_disposition_release_contract=1' "$status"
...
require_contains 'prompt-evaluation result release receipt review disposition metadata only' "$installer_readme"
...
require_contains 'prompt-evaluation result release receipt review disposition recording remains metadata-only' "$components_manifest"
...
require_contains 'requires_future_prompt_evaluation_result_release_receipt_review_disposition_release_contract=1' "$apply_script"
...
require_contains 'nadia-prompt-evaluation-result-release-receipt-review-disposition:' Makefile
...
output=$(
  NADIA_PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT_REVIEW_DISPOSITION_TIMESTAMP=20260526T050000Z \
...
report="$out/nadia-prompt-evaluation-result-release-receipt-review-disposition-contract-20260526T050000Z.txt"
...
require_contains 'requires_prompt_evaluation_result_release_receipt_review_contract=1' "$report"
require_contains 'requires_future_prompt_evaluation_result_release_receipt_review_disposition_release_contract=1' "$report"
