...
usage() {
  cat <<'USAGE'
Usage:
  latticra-panel-signed-updater-state-transition-denial-transcript.sh

Emits the no-effect denial transcript for the blocked Latticra Panel signed
updater state transition. It does not transition update state, stage updates,
write transcript files, write receipts, mutate host state, or apply signed
updates.
USAGE
}
...
signed_updater_state_transition_denial_transcript_status=ok
repo_root=$ROOT
...
next_lane=panel-signed-updater-state-transition-denial-disposition-closeout
REPORT
