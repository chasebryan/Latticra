...
tmpdir="$(mktemp -d "${TMPDIR:-/tmp}/latticra-kernel-runtime-entry-quiescent-return-observation-view-report.XXXXXX")"
trap 'rm -rf "$tmpdir"' EXIT INT HUP TERM

report_bin="$tmpdir/latticra-kernel-runtime-entry-quiescent-return-observation-view-report"
report_txt="$tmpdir/latticra-kernel-runtime-entry-quiescent-return-observation-view-report.txt"
...
grep -Fq 'LATTICRA KERNEL RUNTIME ENTRY QUIESCENT RETURN OBSERVATION VIEW REPORT' "$report_txt"
grep -Fq 'quiescent_return_observation_view_status=runtime-entry-quiescent-return-observation-view-seed-ready' "$report_txt"
...
grep -Fq 'evidence_level=42' "$report_txt"
grep -Fq 'quiescent_return_observation_view[0].process_label=kernel-report-process-metadata' "$report_txt"
grep -Fq 'quiescent_return_observation_view[0].quiescent_return_observation_view_class=kernel-report-runtime-entry-quiescent-return-observation-view' "$report_txt"
grep -Fq 'quiescent_return_observation_view[0].quiescent_slot=9216' "$report_txt"
grep -Fq 'quiescent_return_observation_view[0].quiescent_return_allowed=0' "$report_txt"
grep -Fq 'quiescent_return_observation_view[0].persistence_allowed=0' "$report_txt"
grep -Fq 'quiescent_return_observation_view[0].no_effect=1' "$report_txt"

printf 'kernel_runtime_entry_quiescent_return_observation_view_report_runner: ok\n'
