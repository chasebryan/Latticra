...
tmpdir="$(mktemp -d "${TMPDIR:-/tmp}/latticra-kernel-runtime-entry-persistence-boundary-observation-view-report.XXXXXX")"
trap 'rm -rf "$tmpdir"' EXIT INT HUP TERM

report_bin="$tmpdir/latticra-kernel-runtime-entry-persistence-boundary-observation-view-report"
report_txt="$tmpdir/latticra-kernel-runtime-entry-persistence-boundary-observation-view-report.txt"
...
grep -Fq 'LATTICRA KERNEL RUNTIME ENTRY PERSISTENCE BOUNDARY OBSERVATION VIEW REPORT' "$report_txt"
grep -Fq 'persistence_boundary_observation_view_status=runtime-entry-persistence-boundary-observation-view-seed-ready' "$report_txt"
...
grep -Fq 'evidence_level=43' "$report_txt"
grep -Fq 'persistence_boundary_observation_view[0].process_label=kernel-report-process-metadata' "$report_txt"
grep -Fq 'persistence_boundary_observation_view[0].persistence_boundary_observation_view_class=kernel-report-runtime-entry-persistence-boundary-observation-view' "$report_txt"
grep -Fq 'persistence_boundary_observation_view[0].persistence_boundary_slot=10240' "$report_txt"
grep -Fq 'persistence_boundary_observation_view[0].persistence_epoch_slot=27' "$report_txt"
grep -Fq 'persistence_boundary_observation_view[0].persistence_reason_slot=28' "$report_txt"
grep -Fq 'persistence_boundary_observation_view[0].recovery_authority_slot=29' "$report_txt"
grep -Fq 'persistence_boundary_observation_view[0].runtime_entry_persistence_boundary_observation_view_allowed=0' "$report_txt"
grep -Fq 'persistence_boundary_observation_view[0].persistence_boundary_allowed=0' "$report_txt"
grep -Fq 'persistence_boundary_observation_view[0].persistence_commit_allowed=0' "$report_txt"
grep -Fq 'persistence_boundary_observation_view[0].persistence_allowed=0' "$report_txt"
grep -Fq 'persistence_boundary_observation_view[0].recovery_authority_allowed=0' "$report_txt"
grep -Fq 'persistence_boundary_observation_view[0].no_effect=1' "$report_txt"

printf 'kernel_runtime_entry_persistence_boundary_observation_view_report_runner: ok\n'
