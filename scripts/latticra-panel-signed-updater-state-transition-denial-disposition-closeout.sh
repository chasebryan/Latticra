...
usage() {
  cat <<'USAGE'
Usage:
  latticra-panel-signed-updater-state-transition-denial-disposition-closeout.sh

Emits the no-effect closeout for the Latticra Panel signed updater state
transition denial disposition review. It does not transition update state,
stage updates, write closeout files, write receipts, mutate host state, or
...
signed_updater_state_transition_denial_disposition_closeout_status=ok
repo_root=$ROOT
