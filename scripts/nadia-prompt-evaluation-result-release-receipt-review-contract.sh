...
usage() {
  cat <<'USAGE'
Usage:
  nadia-prompt-evaluation-result-release-receipt-review-contract.sh [options]

Options:
  --prompt-evaluation-result-release-receipt <file>  Stage-35 Nadia prompt-evaluation result release receipt contract
  --request-class <label>                            Operator request classification label
  --review-family <label>                            Planned prompt-evaluation result release receipt review family label
  --review-format <label>                            Planned prompt-evaluation result release receipt review format label
  --output <dir>                                     Output directory for prompt-evaluation result release receipt review contracts

Creates Nadia Stage-36 prompt-evaluation result release receipt review
contract metadata only. It verifies the prompt-evaluation result release
receipt contract and records planned receipt-review requirements before any
review record, review decision, receipt record, receipt signing, release
record, model-output access, runtime invocation, prompt evaluation, dialogue
generation, token generation, or inference boundary. It does not create
reviews, approve receipts, reject receipts, create receipts, sign receipts,
...
fail() {
  printf 'nadia prompt evaluation result release receipt review contract: %s\n' "$1" >&2
  exit "${2:-1}"
}
...
require_field() {
  ...
  expected="$2"
  message="$3"
  actual=$(field_or_unknown "$key" "$PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT_ABS")
...
reject_boundary_label() {
  ...
  case "$normalized" in
    *sex*|*sexual*|*porn*|*erotic*|*nude*|*nudity*|*nsfw*|*fetish*|*adult-content*)
      fail "$name is outside Nadia prompt-evaluation-result-release-receipt-review boundary" 65
      ;;
  esac
}

PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT=""
REQUEST_CLASS="awareness-education"
REVIEW_FAMILY="operator-reviewed-prompt-evaluation-result-release-receipt-review"
REVIEW_FORMAT="contract-only-offline-evaluation-result-release-receipt-review"
OUTPUT=""

while [ "$#" -gt 0 ]; do
  case "$1" in
    --prompt-evaluation-result-release-receipt)
      [ "$#" -ge 2 ] || fail "missing value for --prompt-evaluation-result-release-receipt" 64
      PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT="$2"
      shift 2
      ;;
    --request-class)
...
if [ -z "$PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT" ] && [ -f "reports/nadia/prompt-evaluation-result-release-receipt/latest-prompt-evaluation-result-release-receipt-contract.txt" ]; then
  PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT="reports/nadia/prompt-evaluation-result-release-receipt/latest-prompt-evaluation-result-release-receipt-contract.txt"
fi

[ -n "$PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT" ] || fail "prompt evaluation result release receipt contract is required" 64
[ -f "$PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT" ] || fail "prompt evaluation result release receipt contract not found: $PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT" 66
...
if [ -z "$OUTPUT" ]; then
  OUTPUT="reports/nadia/prompt-evaluation-result-release-receipt-review"
fi
mkdir -p "$OUTPUT"
OUT_DIR=$(CDPATH= cd -- "$OUTPUT" && pwd -P)

TS=${NADIA_PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT_REVIEW_TIMESTAMP:-$(date -u +"%Y%m%dT%H%M%SZ")}
REPORT="$OUT_DIR/nadia-prompt-evaluation-result-release-receipt-review-contract-$TS.txt"

PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT_ABS=$(cd "$(dirname -- "$PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT")" && printf '%s/%s\n' "$(pwd -P)" "$(basename -- "$PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT")")
PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT_MEASUREMENT=$(measure_file "$PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT_ABS")

require_field stage 35-prompt-evaluation-result-release-receipt-contract "prompt evaluation result release receipt stage must be 35-prompt-evaluation-result-release-receipt-contract"
require_field prompt_evaluation_result_release_receipt_contract_status contract_only "prompt evaluation result release receipt contract must remain contract-only"
require_field prompt_evaluation_result_release_receipt_stage contract-only "prompt evaluation result release receipt stage must be contract-only"
require_field prompt_evaluation_result_release_receipt_authority 0 "prompt evaluation result release receipt authority must remain denied"
require_field prompt_evaluation_result_release_receipt_allowed 0 "prompt evaluation result release receipt must remain disallowed"
...
require_field prompt_evaluation_result_release_receipt_performed 0 "prompt evaluation result release receipt must not be performed"
require_field prompt_evaluation_result_release_receipt_metadata_present 1 "prompt evaluation result release receipt metadata must be present"
require_field prompt_evaluation_result_release_receipt_decision blocked_contract_only "prompt evaluation result release receipt decision must be blocked_contract_only"
require_field prompt_evaluation_result_release_receipt_evidence_present 1 "prompt evaluation result release receipt evidence must be present"
require_field prompt_evaluation_result_release_receipt_source_policy operator-reviewed-offline "prompt evaluation result release receipt source policy must be operator-reviewed-offline"
require_field prompt_evaluation_result_release_receipt_plan_recorded 1 "prompt evaluation result release receipt plan must be recorded"
require_field prompt_evaluation_result_release_receipt_result_recorded 0 "prompt evaluation result release receipt result must remain absent"
require_field prompt_evaluation_result_release_receipt_runtime_invoked 0 "prompt evaluation result release receipt runtime must not be invoked"
...
require_field requires_future_prompt_evaluation_result_release_receipt_review_contract 1 "future prompt evaluation result release receipt review contract must be required"
require_field prompt_evaluation_result_release_receipt_promotion_allowed 0 "prompt evaluation result release receipt must not allow promotion"
...
require_field requires_prompt_evaluation_result_release_receipt_schema_policy 1 "prompt evaluation result release receipt schema policy must be required"
require_field requires_prompt_evaluation_result_release_receipt_denial_policy 1 "prompt evaluation result release receipt denial policy must be required"
require_field requires_prompt_evaluation_result_release_receipt_review_policy 1 "prompt evaluation result release receipt review policy must be required"
...
require_field prompt_evaluation_result_release_receipt_decision_recorded 0 "prompt evaluation result release receipt decision must not be recorded"
require_field prompt_evaluation_result_release_receipt_emitted 0 "prompt evaluation result release receipt must not be emitted"
...
PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT_CONTRACT_STATUS=$(field_or_unknown prompt_evaluation_result_release_receipt_contract_status "$PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT_ABS")
PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT_STAGE=$(field_or_unknown prompt_evaluation_result_release_receipt_stage "$PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT_ABS")
PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT_FAMILY=$(field_or_unknown prompt_evaluation_result_release_receipt_family "$PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT_ABS")
PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT_FORMAT=$(field_or_unknown prompt_evaluation_result_release_receipt_format "$PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT_ABS")
PROMPT_EVALUATION_RESULT_RELEASE_STAGE=$(field_or_unknown prompt_evaluation_result_release_stage "$PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT_ABS")
PROMPT_EVALUATION_RESULT_DISPOSITION_STAGE=$(field_or_unknown prompt_evaluation_result_disposition_stage "$PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT_ABS")
PROMPT_EVALUATION_RESULT_REVIEW_STAGE=$(field_or_unknown prompt_evaluation_result_review_stage "$PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT_ABS")
PROMPT_EVALUATION_RESULT_STAGE=$(field_or_unknown prompt_evaluation_result_stage "$PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT_ABS")
PROMPT_EVALUATION_INVOCATION_STAGE=$(field_or_unknown prompt_evaluation_invocation_stage "$PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT_ABS")
PROMPT_EVALUATION_RUNTIME_HANDOFF_STAGE=$(field_or_unknown prompt_evaluation_runtime_handoff_stage "$PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT_ABS")
PROMPT_EVALUATION_INPUT_STAGE=$(field_or_unknown prompt_evaluation_input_stage "$PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT_ABS")
CONTEXT_WINDOW_ASSEMBLY_STAGE=$(field_or_unknown context_window_assembly_stage "$PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT_ABS")
PROMPT_TOKEN_SEQUENCE_STAGE=$(field_or_unknown prompt_token_sequence_stage "$PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT_ABS")
PROMPT_TOKENIZATION_STAGE=$(field_or_unknown prompt_tokenization_stage "$PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT_ABS")
...
prompt_evaluation_result_release_receipt_review_family=$REVIEW_FAMILY
prompt_evaluation_result_release_receipt_review_format=$REVIEW_FORMAT
prompt_evaluation_result_release_receipt_review_schema_planned=1
prompt_evaluation_result_release_receipt_review_schema_validated=0
prompt_evaluation_result_release_receipt_review_record_created=0
prompt_evaluation_result_release_receipt_review_record_validated=0
prompt_evaluation_result_release_receipt_review_record_serialized=0
prompt_evaluation_result_release_receipt_review_record_written=0
prompt_evaluation_result_release_receipt_review_record_submitted=0
...
prompt_evaluation_result_release_receipt_review_route_recorded=0
...
[inputs]
prompt_evaluation_result_release_receipt=$PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT_ABS
prompt_evaluation_result_release_receipt_measurement=$PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT_MEASUREMENT
prompt_evaluation_result_release_receipt_stage_required=35-prompt-evaluation-result-release-receipt-contract
prompt_evaluation_result_release_receipt_contract_status=$PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT_CONTRACT_STATUS
prompt_evaluation_result_release_receipt_stage=$PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT_STAGE
prompt_evaluation_result_release_receipt_family=$PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT_FAMILY
prompt_evaluation_result_release_receipt_format=$PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT_FORMAT
...
[prompt_evaluation_result_release_receipt_review_contract]
...
[prompt_evaluation_result_release_receipt_review_requirements]
...
[prompt_evaluation_result_release_receipt_review_denial_boundary]
prompt_evaluation_result_release_receipt_review_authority=0
prompt_evaluation_result_release_receipt_review_allowed=0
...
cp "$REPORT" "$OUT_DIR/latest-prompt-evaluation-result-release-receipt-review-contract.txt"

printf 'NADIA_PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT_REVIEW_CONTRACT=%s\n' "$REPORT"
printf 'NADIA_PROMPT_EVALUATION_RESULT_RELEASE_RECEIPT_REVIEW_RECORDED=0\n'
