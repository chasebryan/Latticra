#!/usr/bin/env sh
# SPDX-License-Identifier: AGPL-3.0-or-later
set -eu

require_file() {
  file="$1"
  if [ ! -f "$file" ]; then
    printf 'fedora vm cli payload readme alignment: missing file: %s\n' "$file" >&2
    exit 1
  fi
}

require_contains() {
  pattern="$1"
  file="$2"
  if ! grep -Fq -- "$pattern" "$file"; then
    printf 'fedora vm cli payload readme alignment: missing required pattern in %s: %s\n' "$file" "$pattern" >&2
    exit 1
  fi
}

require_absent() {
  pattern="$1"
  file="$2"
  if grep -Fq -- "$pattern" "$file"; then
    printf 'fedora vm cli payload readme alignment: unexpected pattern in %s: %s\n' "$file" "$pattern" >&2
    exit 1
  fi
}

readme='README.md'
status='docs/status/FEDORA_VM_CLI_PAYLOAD_README_ALIGNMENT_STATUS.md'
evidence='docs/status/FEDORA_VM_CLI_PAYLOAD_VALIDATION_EVIDENCE_STATUS.md'
runner_status='docs/status/FEDORA_VM_CLI_PAYLOAD_VALIDATION_STATUS.md'
index='docs/status/README.md'
workflow='.github/workflows/fedora-vm-cli-payload-readme-alignment.yml'
makefile='Makefile'

require_file "$readme"
require_file "$status"
require_file "$evidence"
require_file "$runner_status"
require_file "$index"
require_file "$workflow"
require_file "$makefile"
require_file docs/FEDORA_VM_CLI_PAYLOAD_VALIDATION_LANE.md
require_file packaging/fedora/latticra.spec
require_file src/latticra_cli.c

require_contains '## Fedora and local RPM work' "$readme"
require_contains 'Latticra has a Fedora-facing validation lane, but it is intentionally narrow.' "$readme"
require_contains 'The current RPM packaging work is a **local-only draft**.' "$readme"
require_contains 'local_only_draft=1' "$readme"
require_contains 'fedora_vm_cli_payload_validation_status=evidence-recorded' "$readme"
require_contains 'disposable_vm_cli_validation_transcript_present=1' "$readme"
require_contains 'disposable_vm_cli_validation_completed=1' "$readme"
require_contains 'host_install_ready_for_cli_payload=1' "$readme"
require_contains 'fedora_approval_claimed=0' "$readme"
require_contains 'fedora_distribution_ready=0' "$readme"
require_contains 'production_installer_ready=0' "$readme"
require_contains 'daily_driver_install_ready=0' "$readme"
require_contains 'immutable_fedora_ready=0' "$readme"
require_contains 'src/latticra_cli.c' "$readme"
require_contains '/usr/bin/latticra' "$readme"
require_contains '/usr/share/doc/latticra/README.md' "$readme"
require_contains 'A reviewed disposable Fedora VM CLI payload validation transcript has been accepted' "$readme"
require_contains 'CLI `--status`, `--version`, `--report`, invalid-command validation' "$readme"
require_contains 'This supports `host_install_ready_for_cli_payload=1` only for the bounded CLI payload above.' "$readme"
require_contains 'does not claim production installer readiness' "$readme"
require_contains 'Fedora approval' "$readme"
require_contains 'Fedora distribution readiness' "$readme"
require_contains 'immutable Fedora readiness' "$readme"
require_contains 'daily-driver readiness' "$readme"
require_contains 'security capability' "$readme"
require_contains 'update safety' "$readme"
require_contains 'recovery safety' "$readme"
require_contains 'sandboxing' "$readme"
require_contains 'malware prevention' "$readme"
require_contains 'ransomware prevention' "$readme"
require_contains 'operating-system replacement readiness' "$readme"
require_contains 'docs/status/FEDORA_VM_CLI_PAYLOAD_VALIDATION_EVIDENCE_STATUS.md' "$readme"
require_contains 'docs/status/FEDORA_VM_CLI_PAYLOAD_README_ALIGNMENT_STATUS.md' "$readme"

require_absent 'fedora_vm_cli_payload_validation_status=blocked-pending-real-vm-run' "$readme"
require_absent 'The manually gated disposable Fedora VM CLI payload validation runner exists, but no reviewed real-VM CLI validation transcript has been accepted yet.' "$readme"
require_absent 'The current evidence is runner-readiness only, not host install readiness for the CLI payload.' "$readme"

require_contains 'Status: README/status alignment' "$status"
require_contains 'Date: 2026-05-26' "$status"
require_contains 'fedora_vm_cli_payload_validation_status=evidence-recorded' "$status"
require_contains 'disposable_vm_cli_validation_transcript_present=1' "$status"
require_contains 'disposable_vm_cli_validation_completed=1' "$status"
require_contains 'host_install_ready_for_cli_payload=1' "$status"
require_contains 'production_installer_ready=0' "$status"
require_contains 'fedora_distribution_ready=0' "$status"
require_contains 'fedora_approval_claimed=0' "$status"
require_contains 'daily_driver_install_ready=0' "$status"
require_contains 'immutable_fedora_ready=0' "$status"
require_contains 'validated payload includes /usr/bin/latticra' "$status"
require_contains 'validated payload includes /usr/share/doc/latticra/README.md' "$status"
require_contains 'sh scripts/test-fedora-vm-cli-payload-readme-alignment.sh' "$status"
require_contains 'Plan the next Fedora CLI payload validation lane without widening README or announcement claims beyond disposable Fedora VM evidence' "$status"

require_contains 'FEDORA_VM_CLI_PAYLOAD_README_ALIGNMENT_STATUS.md' "$index"
require_contains 'Fedora VM CLI payload README alignment' "$index"
require_contains 'Current Fedora VM CLI payload README alignment checkpoint' "$index"
require_contains 'fedora_vm_cli_payload_validation_status=evidence-recorded' "$index"
require_contains 'disposable_vm_cli_validation_completed=1' "$index"
require_contains 'host_install_ready_for_cli_payload=1' "$index"

require_contains 'Status: evidence status alignment' "$evidence"
require_contains 'disposable_vm_cli_validation_completed=1' "$evidence"
require_contains 'host_install_ready_for_cli_payload=1' "$evidence"
require_contains 'production_installer_ready=0' "$evidence"
require_contains 'fedora_distribution_ready=0' "$evidence"
require_contains 'fedora_approval_claimed=0' "$evidence"

require_contains 'fedora_vm_cli_payload_validation_status=blocked-pending-real-vm-run' "$runner_status"
require_contains 'runner-readiness evidence only' "$runner_status"

require_contains 'fedora-vm-cli-payload-readme-alignment:' "$makefile"
require_contains 'sh ./scripts/test-fedora-vm-cli-payload-readme-alignment.sh' "$makefile"

require_contains 'name: Fedora VM CLI Payload README Alignment' "$workflow"
require_contains 'runs-on: ubuntu-latest' "$workflow"
require_contains 'sh scripts/test-fedora-vm-cli-payload-readme-alignment.sh' "$workflow"

printf 'fedora_vm_cli_payload_readme_alignment: ok\n'
