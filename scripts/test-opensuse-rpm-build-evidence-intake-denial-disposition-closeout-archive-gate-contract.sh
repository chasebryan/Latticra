...
require_file() {
  file="$1"
  if [ ! -f "$file" ]; then
    printf 'opensuse rpm build-evidence intake denial disposition closeout archive gate contract: missing file: %s\n' "$file" >&2
    exit 1
  fi
}
...
require_contains() {
  ...
  if ! grep -Fq -- "$pattern" "$file"; then
    printf 'opensuse rpm build-evidence intake denial disposition closeout archive gate contract: missing required pattern in %s: %s\n' "$file" "$pattern" >&2
    exit 1
  fi
}
...
require_no_rpm_artifacts() {
  ...
  if find "$dir" \( -name '*.rpm' -o -name '*.src.rpm' \) -print | grep -q .; then
    printf 'opensuse rpm build-evidence intake denial disposition closeout archive gate contract: unexpected RPM artifact under %s\n' "$dir" >&2
    exit 1
  fi
}

contract='docs/OPENSUSE_RPM_BUILD_EVIDENCE_INTAKE_DENIAL_DISPOSITION_CLOSEOUT_ARCHIVE_GATE_CONTRACT.md'
...
require_contains 'Status: active RPM build-evidence intake denial disposition closeout archive gate contract' "$contract"
require_contains 'documentation-only and static' "$contract"
require_contains 'opensuse_rpm_build_evidence_intake_denial_disposition_closeout_archive_gate_contract_present=1' "$contract"
require_contains 'rpm_build_evidence_intake_denial_disposition_closeout_archive_gate_contract_present=1' "$contract"
...
require_contains 'docs/OPENSUSE_RPM_BUILD_EVIDENCE_INTAKE_DENIAL_DISPOSITION_CLOSEOUT_CONTRACT.md' "$contract"
...
require_contains 'opensuse_rpm_build_evidence_intake_denial_disposition_closeout_archive_gate_contract: ok' "$contract"
...
require_contains 'OPENSUSE_RPM_BUILD_EVIDENCE_INTAKE_DENIAL_DISPOSITION_CLOSEOUT_ARCHIVE_GATE_CONTRACT.md' docs/README.md
require_contains 'OPENSUSE_RPM_BUILD_EVIDENCE_INTAKE_DENIAL_DISPOSITION_CLOSEOUT_ARCHIVE_GATE_CONTRACT.md' docs/status/README.md

for file in \
  docs/status/OPENSUSE_ECOSYSTEM_INTEGRATION_STATUS.md \
  packaging/opensuse/README.md
do
  require_contains 'opensuse_rpm_build_evidence_intake_denial_disposition_closeout_archive_gate_contract_present=1' "$file"
  require_contains 'rpm_build_evidence_intake_denial_disposition_closeout_archive_gate_contract_present=1' "$file"
...
require_contains 'opensuse_build_evidence_intake_denial_archive_gate_present=1' docs/status/OPENSUSE_ECOSYSTEM_INTEGRATION_STATUS.md
require_contains 'opensuse_build_evidence_intake_denial_archive_gate_closed=1' docs/status/OPENSUSE_ECOSYSTEM_INTEGRATION_STATUS.md
require_contains 'opensuse_denial_archive_record_write_allowed=0' docs/status/OPENSUSE_ECOSYSTEM_INTEGRATION_STATUS.md
require_contains 'opensuse_build_evidence_intake_denial_archive_gate_present=1' packaging/opensuse/README.md
require_contains 'opensuse_build_evidence_intake_denial_archive_gate_closed=1' packaging/opensuse/README.md
...
require_contains 'Run openSUSE RPM build evidence intake denial disposition closeout archive gate contract guard' .github/workflows/opensuse-rpm-build-evidence-intake-denial-disposition-closeout-archive-gate-contract.yml
require_contains 'sh scripts/test-opensuse-rpm-build-evidence-intake-denial-disposition-closeout-archive-gate-contract.sh' .github/workflows/opensuse-rpm-build-evidence-intake-denial-disposition-closeout-archive-gate-contract.yml

sh -n "$0"

printf 'opensuse_rpm_build_evidence_intake_denial_disposition_closeout_archive_gate_contract: ok\n'
