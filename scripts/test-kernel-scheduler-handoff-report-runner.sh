...
tmpdir="$(mktemp -d "${TMPDIR:-/tmp}/latticra-kernel-scheduler-handoff-report.XXXXXX")"
trap 'rm -rf "$tmpdir"' EXIT INT HUP TERM

report_bin="$tmpdir/latticra-kernel-scheduler-handoff-report"
report_txt="$tmpdir/latticra-kernel-scheduler-handoff-report.txt"
...
grep -Fq 'LATTICRA KERNEL SCHEDULER HANDOFF REPORT' "$report_txt"
grep -Fq 'handoff_status=scheduler-handoff-seed-ready' "$report_txt"
...
grep -Fq 'handoff_count=4' "$report_txt"
...
grep -Fq 'handoff[0].process_label=kernel-report-process-metadata' "$report_txt"
grep -Fq 'handoff[0].handoff_class=kernel-report-scheduler-handoff' "$report_txt"
grep -Fq 'handoff[0].handoff_rank=1' "$report_txt"
grep -Fq 'handoff[0].scheduler_credit_ns=10000000' "$report_txt"
grep -Fq 'handoff[0].handoff_declared=1' "$report_txt"
grep -Fq 'handoff[0].handoff_planned=0' "$report_txt"
grep -Fq 'handoff[0].dispatch_planned=0' "$report_txt"
grep -Fq 'handoff[0].dequeued=0' "$report_txt"
grep -Fq 'handoff[0].dispatched=0' "$report_txt"
grep -Fq 'handoff[0].context_switch_planned=0' "$report_txt"
grep -Fq 'handoff[0].no_effect=1' "$report_txt"

printf 'kernel_scheduler_handoff_report_runner: ok\n'
