...
int main(void) {
    ...
    if (latticra_kernel_scheduler_dispatch_default_request(&request) !=
            LATTICRA_STATUS_OK) {
        return 1;
    }
    if (latticra_kernel_scheduler_dispatch_evaluate(&request, &result) !=
            LATTICRA_STATUS_OK) {
        return 1;
    }
    if (latticra_kernel_scheduler_dispatch_report(&result, report,
