...
int main(int argc, char **argv) {
    ...
    if (argc == 2 &&
        (strcmp(argv[1], "processes") == 0 ||
         strcmp(argv[1], "processes-contract") == 0 ||
         strcmp(argv[1], "process-envelope") == 0 ||
         strcmp(argv[1], "lc-processes") == 0)) {
        if (latticra_console_processes_contract_report(
                processes_contract_report,
                sizeof(processes_contract_report)) != LATTICRA_STATUS_OK) {
            fputs("latticra_console_report: processes contract report render failed\n", stderr);
            return 1;
        }
        fputs(processes_contract_report, stdout);
        return 0;
    }
