...
int main(void) {
    ...
    if (latticra_kernel_runtime_entry_quiescent_return_observation_view_default_request(
        ...
        fputs("kernel_runtime_entry_quiescent_return_observation_view_report: default request failed\n",
            stderr);
        return 1;
    }

    if (latticra_kernel_runtime_entry_quiescent_return_observation_view_evaluate(
            &request, &result) != LATTICRA_STATUS_OK) {
        fputs("kernel_runtime_entry_quiescent_return_observation_view_report: evaluation failed\n",
            stderr);
        return 1;
    }

    if (latticra_kernel_runtime_entry_quiescent_return_observation_view_report(
            &result, report, sizeof(report)) != LATTICRA_STATUS_OK) {
        fputs("kernel_runtime_entry_quiescent_return_observation_view_report: report render failed\n",
            stderr);
        return 1;
    }
