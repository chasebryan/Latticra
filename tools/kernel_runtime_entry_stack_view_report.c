...
int main(void) {
    ...
    if (latticra_kernel_runtime_entry_stack_view_default_request(&request) !=
            LATTICRA_STATUS_OK) {
        return 1;
    }
    if (latticra_kernel_runtime_entry_stack_view_evaluate(&request,
            &result) != LATTICRA_STATUS_OK) {
        return 1;
    }
    if (latticra_kernel_runtime_entry_stack_view_report(&result, report,
