...
static void envelope_init(latticra_seal_report_envelope_t *envelope) {
    ...
    envelope->verified = 0u;
    envelope->authority_usable = 0u;
    envelope->receipt_capability_gate_allowed = 0u;
    envelope->gate_allowed = 0u;
    envelope->effect_allowed = 0u;
    envelope->handoff_eligible = 0u;
...
static void copy_report_metadata(
    ...
    out->authority_usable = report->authority_usable;
    out->receipt_capability_gate_allowed = report->receipt_capability_gate_allowed;
    out->gate_allowed = report->gate_allowed;
    copy_literal(out->gate_state, sizeof(out->gate_state), report->gate_state);
    copy_literal(out->decision_state, sizeof(out->decision_state), report->decision_state);
    out->effect_allowed = report->effect_allowed;
    copy_literal(out->handoff_state, sizeof(out->handoff_state), report->handoff_state);
    out->handoff_eligible = report->handoff_eligible;
...
latticra_status_t latticra_seal_report_envelope_from_report(
    ...
    report_only = strcmp(report->report_state, "ready-report-only") == 0;
    evaluate_only = strcmp(report->report_state, "ready-evaluate-only") == 0;
    if (!report_only && !evaluate_only) {
        out->error = LATTICRA_SEAL_REPORT_ENVELOPE_DENIED_REPORT;
        copy_literal(out->envelope_state, sizeof(out->envelope_state), "denied-report");
        copy_literal(out->status, sizeof(out->status), "denied-report");
        return LATTICRA_STATUS_OK;
    ...
    if ((report_only && strcmp(requested_envelope, "report-only") != 0) ||
        (evaluate_only && strcmp(requested_envelope, "evaluate-only") != 0)) {
...
latticra_status_t latticra_seal_report_envelope_render(
    ...
    written = snprintf(
        ...
        envelope->verified,
        envelope->authority_usable,
        envelope->receipt_capability_gate_allowed,
        envelope->gate_allowed,
        envelope->gate_state,
        envelope->decision_state,
        envelope->effect_allowed,
        envelope->handoff_state,
        envelope->handoff_eligible,
