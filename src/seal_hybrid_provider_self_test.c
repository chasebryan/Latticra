...
static void self_test_init(latticra_seal_hybrid_provider_self_test_t *out) {
    ...
    copy_literal(
        ...
        "NIST-SP-800-56A-REV3,NIST-FIPS-203,NIST-SP-800-56C-REV2,NIST-FIPS-197,NIST-SP-800-38D");
...
latticra_status_t latticra_seal_hybrid_provider_self_test_run(
    ...
    if (!derive_ecdh_secret(alice_p256, bob_p256, alice_ecdh_secret, &alice_ecdh_secret_len) ||
        !derive_ecdh_secret(bob_p256, alice_p256, bob_ecdh_secret, &bob_ecdh_secret_len)) {
        self_test_fail(
            out,
            LATTICRA_SEAL_HYBRID_PROVIDER_SELF_TEST_PROVIDER_FAILURE,
