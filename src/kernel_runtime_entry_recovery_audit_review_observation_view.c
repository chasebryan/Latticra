...
static void seed_result(
    ...
    recovery_audit_review_copy(
        result->recovery_audit_review_observation_view_status,
        sizeof(result->recovery_audit_review_observation_view_status),
        "pending");
...
latticra_kernel_runtime_entry_recovery_audit_review_observation_view_default_request(
    ...
    request->requested_recovery_audit_review_observation_view_count = 4u;
    return LATTICRA_STATUS_OK;
}

static const
latticra_kernel_runtime_entry_recovery_audit_observation_view_entry_t *
recovery_audit_observation_view_at(
    const latticra_kernel_runtime_entry_recovery_audit_observation_view_result_t
        *view,
    size_t index) {
    if (index < view->recovery_audit_observation_view_count &&
        view->recovery_audit_observation_views[index].declared) {
        return &view->recovery_audit_observation_views[index];
    }
    return 0;
}
...
static const char *class_for(size_t index) {
    switch (index) {
        case 0u:
            return "kernel-report-runtime-entry-recovery-audit-review-observation-view";
        case 1u:
            return "entropy-runtime-entry-recovery-audit-review-observation-view";
        case 2u:
            return "console-runtime-entry-recovery-audit-review-observation-view";
        case 3u:
            return "idle-runtime-entry-recovery-audit-review-observation-view";
        default:
            return "reserved-runtime-entry-recovery-audit-review-observation-view";
    }
}
...
static void fill_recovery_audit_review_observation_view(
    latticra_kernel_runtime_entry_recovery_audit_review_observation_view_entry_t
        *entry,
    const latticra_kernel_runtime_entry_recovery_audit_observation_view_result_t
        *view,
    ...
    const
    latticra_kernel_runtime_entry_recovery_audit_observation_view_entry_t
        *audit_view = recovery_audit_observation_view_at(view, index);

    memset(entry, 0, sizeof(*entry));
    entry->recovery_audit_review_observation_view_index = index;
    entry->recovery_audit_review_observation_view_token =
        index < 4u ? (unsigned long)index : 38000ul + (unsigned long)index;
    entry->recovery_audit_observation_view_token = audit_view ?
        audit_view->recovery_audit_observation_view_token :
        37000ul + (unsigned long)index;
    entry->recovery_closeout_observation_view_token = audit_view ?
        audit_view->recovery_closeout_observation_view_token :
        36000ul + (unsigned long)index;
    entry->recovery_outcome_observation_view_token = audit_view ?
        audit_view->recovery_outcome_observation_view_token :
        35000ul + (unsigned long)index;
    entry->recovery_audit_review_observation_slot =
        17408ul + (unsigned long)index;
    entry->recovery_audit_observation_slot = audit_view ?
        audit_view->recovery_audit_observation_slot :
        16384ul + (unsigned long)index;
    entry->recovery_closeout_observation_slot = audit_view ?
        audit_view->recovery_closeout_observation_slot :
        15360ul + (unsigned long)index;
    entry->recovery_outcome_observation_slot = audit_view ?
        audit_view->recovery_outcome_observation_slot :
        14336ul + (unsigned long)index;
    entry->recovery_audit_review_slot = 38ul;
    entry->recovery_audit_slot = audit_view ? audit_view->recovery_audit_slot :
        37ul;
    entry->recovery_closeout_slot = audit_view ?
        audit_view->recovery_closeout_slot : 36ul;
    entry->recovery_outcome_slot = audit_view ?
        audit_view->recovery_outcome_slot : 35ul;
    entry->pid_token = audit_view ? audit_view->pid_token :
    ...
    recovery_audit_review_copy(entry->process_label,
        ...
        audit_view ? audit_view->process_label : fallback_process(index));
    recovery_audit_review_copy(
        entry->recovery_audit_review_observation_view_class,
        sizeof(entry->recovery_audit_review_observation_view_class),
        class_for(index));
    recovery_audit_review_copy(
        entry->recovery_audit_review_observation_view_status,
        sizeof(entry->recovery_audit_review_observation_view_status),
        "declared-metadata");
    recovery_audit_review_copy(entry->authority_status,
        sizeof(entry->authority_status),
        "runtime-entry-recovery-audit-review-observation-view-authority-denied");
    entry->declared = 1;
    entry->no_effect = 1;
...
static void fill_recovery_audit_review_observation_views(
    latticra_kernel_runtime_entry_recovery_audit_review_observation_view_result_t
        *result,
    ...
    result->recovery_audit_review_observation_view_count = count;
    for (i = 0u; i < count; ++i) {
        fill_recovery_audit_review_observation_view(
            &result->recovery_audit_review_observation_views[i],
...
latticra_kernel_runtime_entry_recovery_audit_review_observation_view_evaluate(
    ...
    fill_recovery_audit_review_observation_views(result,
        request->requested_recovery_audit_review_observation_view_count);
    result->no_effect =
        result->runtime_entry_recovery_audit_observation_view.no_effect;
    recovery_audit_review_copy(
        result->recovery_audit_review_observation_view_status,
        sizeof(result->recovery_audit_review_observation_view_status),
        result->no_effect ?
            "runtime-entry-recovery-audit-review-observation-view-seed-ready" :
            "runtime-entry-recovery-audit-review-observation-view-seed-blocked");
...
latticra_status_t
latticra_kernel_runtime_entry_recovery_audit_review_observation_view_report(
    const
    latticra_kernel_runtime_entry_recovery_audit_review_observation_view_result_t
        *result,
    char *buffer,
    ...
    status = append_text(buffer, buffer_len, &used,
        "LATTICRA KERNEL RUNTIME ENTRY RECOVERY AUDIT REVIEW OBSERVATION VIEW REPORT\n"
        "recovery_audit_review_observation_view_status=%s\n"
        "policy_status=%s\n"
        ...
        (unsigned long)result->recovery_audit_review_observation_view_count,
        result->no_effect,
    ...
    for (i = 0u; i < result->recovery_audit_review_observation_view_count; ++i) {
        const
        latticra_kernel_runtime_entry_recovery_audit_review_observation_view_entry_t
            *entry = &result->recovery_audit_review_observation_views[i];
        status = append_text(buffer, buffer_len, &used,
            "recovery_audit_review_observation_view[%lu].process_label=%s\n"
            "recovery_audit_review_observation_view[%lu].recovery_audit_review_observation_view_class=%s\n"
            "recovery_audit_review_observation_view[%lu].recovery_audit_review_observation_view_token=%lu\n"
            "recovery_audit_review_observation_view[%lu].recovery_audit_observation_view_token=%lu\n"
            "recovery_audit_review_observation_view[%lu].recovery_closeout_observation_view_token=%lu\n"
            "recovery_audit_review_observation_view[%lu].recovery_audit_review_observation_slot=%lu\n"
            "recovery_audit_review_observation_view[%lu].recovery_audit_observation_slot=%lu\n"
            "recovery_audit_review_observation_view[%lu].recovery_audit_review_slot=%lu\n"
            "recovery_audit_review_observation_view[%lu].recovery_audit_slot=%lu\n"
            "recovery_audit_review_observation_view[%lu].recovery_audit_review_observation_declared=%d\n"
            "recovery_audit_review_observation_view[%lu].recovery_audit_review_declared=%d\n"
            "recovery_audit_review_observation_view[%lu].recovery_audit_observation_declared=%d\n"
            "recovery_audit_review_observation_view[%lu].recovery_audit_declared=%d\n"
            "recovery_audit_review_observation_view[%lu].recovery_audit_review_observation_planned=%d\n"
            "recovery_audit_review_observation_view[%lu].runtime_entry_recovery_audit_review_observation_view_allowed=%d\n"
            "recovery_audit_review_observation_view[%lu].runtime_entry_recovery_audit_observation_view_allowed=%d\n"
            "recovery_audit_review_observation_view[%lu].recovery_audit_review_observation_allowed=%d\n"
            "recovery_audit_review_observation_view[%lu].recovery_audit_review_allowed=%d\n"
            "recovery_audit_review_observation_view[%lu].recovery_audit_observation_allowed=%d\n"
            "recovery_audit_review_observation_view[%lu].recovery_audit_allowed=%d\n"
            "recovery_audit_review_observation_view[%lu].no_effect=%d\n",
