...
static void recovery_outcome_copy(char *dst, size_t dst_len,
                                  const char *src) {
    size_t n;
...
static void seed_result(
    ...
    recovery_outcome_copy(result->recovery_outcome_observation_view_status,
        sizeof(result->recovery_outcome_observation_view_status), "pending");
    recovery_outcome_copy(result->policy_status,
...
latticra_kernel_runtime_entry_recovery_outcome_observation_view_default_request(
    ...
    request->requested_recovery_outcome_observation_view_count = 4u;
    return LATTICRA_STATUS_OK;
}

static const
latticra_kernel_runtime_entry_recovery_disposition_observation_view_entry_t *
recovery_disposition_observation_view_at(
    const
    latticra_kernel_runtime_entry_recovery_disposition_observation_view_result_t
        *view,
    size_t index) {
    if (index < view->recovery_disposition_observation_view_count &&
        view->recovery_disposition_observation_views[index].declared) {
        return &view->recovery_disposition_observation_views[index];
    }
    return 0;
}
...
static const char *class_for(size_t index) {
    switch (index) {
        case 0u:
            return
                "kernel-report-runtime-entry-recovery-outcome-observation-view";
        case 1u:
            return "entropy-runtime-entry-recovery-outcome-observation-view";
        case 2u:
            return "console-runtime-entry-recovery-outcome-observation-view";
        case 3u:
            return "idle-runtime-entry-recovery-outcome-observation-view";
        default:
            return "reserved-runtime-entry-recovery-outcome-observation-view";
    }
}
...
static void deny_recovery_outcome_authority(
...
static void fill_recovery_outcome_observation_view(
    latticra_kernel_runtime_entry_recovery_outcome_observation_view_entry_t
        *entry,
    const
    latticra_kernel_runtime_entry_recovery_disposition_observation_view_result_t
    ...
    const
    latticra_kernel_runtime_entry_recovery_disposition_observation_view_entry_t
        *disposition_view = recovery_disposition_observation_view_at(view, index);

    memset(entry, 0, sizeof(*entry));
    entry->recovery_outcome_observation_view_index = index;
    entry->recovery_outcome_observation_view_token =
        index < 4u ? (unsigned long)index : 35000ul + (unsigned long)index;
    entry->recovery_disposition_observation_view_token = disposition_view ?
        disposition_view->recovery_disposition_observation_view_token :
        34000ul + (unsigned long)index;
    entry->recovery_plan_observation_view_token = disposition_view ?
        disposition_view->recovery_plan_observation_view_token :
        33000ul + (unsigned long)index;
    entry->recovery_boundary_observation_view_token = disposition_view ?
        disposition_view->recovery_boundary_observation_view_token :
        32000ul + (unsigned long)index;
    entry->persistence_boundary_observation_view_token = disposition_view ?
        disposition_view->persistence_boundary_observation_view_token :
        31000ul + (unsigned long)index;
    entry->quiescent_return_observation_view_token = disposition_view ?
        disposition_view->quiescent_return_observation_view_token :
        30000ul + (unsigned long)index;
    entry->idle_return_observation_view_token = disposition_view ?
        disposition_view->idle_return_observation_view_token :
        29000ul + (unsigned long)index;
    entry->process_return_observation_view_token = disposition_view ?
        disposition_view->process_return_observation_view_token :
        28000ul + (unsigned long)index;
    entry->scheduler_return_observation_view_token = disposition_view ?
        disposition_view->scheduler_return_observation_view_token :
        27000ul + (unsigned long)index;
    entry->pid_token = disposition_view ? disposition_view->pid_token :
        4000ul + (unsigned long)index;
    entry->persistence_boundary_slot = disposition_view ?
        disposition_view->persistence_boundary_slot : 10240ul + (unsigned long)index;
    entry->recovery_boundary_slot = disposition_view ?
        disposition_view->recovery_boundary_slot : 11264ul + (unsigned long)index;
    entry->recovery_plan_observation_slot = disposition_view ?
        disposition_view->recovery_plan_observation_slot :
        12288ul + (unsigned long)index;
    entry->recovery_disposition_observation_slot = disposition_view ?
        disposition_view->recovery_disposition_observation_slot :
        13312ul + (unsigned long)index;
    entry->recovery_outcome_observation_slot =
        14336ul + (unsigned long)index;
    entry->recovery_epoch_slot = disposition_view ?
        disposition_view->recovery_epoch_slot : 30ul;
    entry->recovery_reason_slot = disposition_view ?
        disposition_view->recovery_reason_slot : 31ul;
    entry->recovery_plan_slot = disposition_view ?
        disposition_view->recovery_plan_slot : 32ul;
    entry->recovery_authority_slot = disposition_view ?
        disposition_view->recovery_authority_slot : 33ul;
    entry->recovery_disposition_slot = disposition_view ?
        disposition_view->recovery_disposition_slot : 34ul;
    entry->recovery_outcome_slot = 35ul;
    ...
    recovery_outcome_copy(entry->process_label,
        sizeof(entry->process_label),
        disposition_view ? disposition_view->process_label :
            fallback_process(index));
    recovery_outcome_copy(entry->recovery_outcome_observation_view_class,
        sizeof(entry->recovery_outcome_observation_view_class),
        class_for(index));
    recovery_outcome_copy(entry->recovery_outcome_observation_view_status,
        sizeof(entry->recovery_outcome_observation_view_status),
        "declared-metadata");
    recovery_outcome_copy(entry->authority_status,
        sizeof(entry->authority_status),
        "runtime-entry-recovery-outcome-observation-view-authority-denied");
    entry->declared = 1;
    entry->no_effect = 1;
    entry->evidence_level = 47u;
    deny_recovery_outcome_authority(entry);
}

static void fill_recovery_outcome_observation_views(
    latticra_kernel_runtime_entry_recovery_outcome_observation_view_result_t
        *result,
    ...
    result->recovery_outcome_observation_view_count = count;
    for (i = 0u; i < count; ++i) {
        fill_recovery_outcome_observation_view(
            &result->recovery_outcome_observation_views[i],
...
latticra_kernel_runtime_entry_recovery_outcome_observation_view_evaluate(
    ...
    if (request == 0) {
        ...
        recovery_outcome_copy(result->recovery_outcome_observation_view_status,
            sizeof(result->recovery_outcome_observation_view_status),
            "null-request");
        return LATTICRA_STATUS_NULL_ARGUMENT;
    }
    ...
    fill_recovery_outcome_observation_views(result,
        request->requested_recovery_outcome_observation_view_count);
    result->no_effect =
        result->runtime_entry_recovery_disposition_observation_view.no_effect;
    recovery_outcome_copy(result->recovery_outcome_observation_view_status,
        sizeof(result->recovery_outcome_observation_view_status),
        result->no_effect ?
            "runtime-entry-recovery-outcome-observation-view-seed-ready" :
            "runtime-entry-recovery-outcome-observation-view-seed-blocked");
...
latticra_kernel_runtime_entry_recovery_outcome_observation_view_report(
    ...
    status = append_text(buffer, buffer_len, &used,
        ...
        "recovery_outcome_observation_view_status=%s\n"
        "policy_status=%s\n"
        ...
        "evidence_level=%u\n",
        result->recovery_outcome_observation_view_status,
        ...
        (unsigned long)result->recovery_outcome_observation_view_count,
        result->no_effect,
    ...
    for (i = 0u; i < result->recovery_outcome_observation_view_count; ++i) {
        const
        latticra_kernel_runtime_entry_recovery_outcome_observation_view_entry_t
            *entry = &result->recovery_outcome_observation_views[i];
        status = append_text(buffer, buffer_len, &used,
            "recovery_outcome_observation_view[%lu].process_label=%s\n"
            "recovery_outcome_observation_view[%lu].recovery_outcome_observation_view_token=%lu\n"
            "recovery_outcome_observation_view[%lu].recovery_disposition_observation_view_token=%lu\n"
            "recovery_outcome_observation_view[%lu].recovery_plan_observation_view_token=%lu\n"
            "recovery_outcome_observation_view[%lu].recovery_boundary_observation_view_token=%lu\n"
            "recovery_outcome_observation_view[%lu].persistence_boundary_observation_view_token=%lu\n"
            "recovery_outcome_observation_view[%lu].quiescent_return_observation_view_token=%lu\n"
            "recovery_outcome_observation_view[%lu].persistence_boundary_slot=%lu\n"
            "recovery_outcome_observation_view[%lu].recovery_boundary_slot=%lu\n"
            "recovery_outcome_observation_view[%lu].recovery_plan_observation_slot=%lu\n"
            "recovery_outcome_observation_view[%lu].recovery_disposition_observation_slot=%lu\n"
            "recovery_outcome_observation_view[%lu].recovery_outcome_observation_slot=%lu\n"
            "recovery_outcome_observation_view[%lu].recovery_epoch_slot=%lu\n"
            "recovery_outcome_observation_view[%lu].recovery_reason_slot=%lu\n"
            "recovery_outcome_observation_view[%lu].recovery_plan_slot=%lu\n"
            "recovery_outcome_observation_view[%lu].recovery_authority_slot=%lu\n"
            "recovery_outcome_observation_view[%lu].recovery_disposition_slot=%lu\n"
            "recovery_outcome_observation_view[%lu].recovery_outcome_slot=%lu\n"
            "recovery_outcome_observation_view[%lu].recovery_outcome_observation_view_class=%s\n"
            "recovery_outcome_observation_view[%lu].authority_status=%s\n"
            "recovery_outcome_observation_view[%lu].recovery_outcome_observation_declared=%d\n"
            "recovery_outcome_observation_view[%lu].recovery_outcome_observation_planned=%d\n"
            "recovery_outcome_observation_view[%lu].recovery_disposition_observation_declared=%d\n"
            "recovery_outcome_observation_view[%lu].recovery_disposition_observation_planned=%d\n"
            "recovery_outcome_observation_view[%lu].recovery_plan_observation_declared=%d\n"
            "recovery_outcome_observation_view[%lu].recovery_plan_observation_planned=%d\n"
            "recovery_outcome_observation_view[%lu].recovery_boundary_declared=%d\n"
            "recovery_outcome_observation_view[%lu].recovery_boundary_planned=%d\n"
            "recovery_outcome_observation_view[%lu].runtime_entry_recovery_outcome_observation_view_allowed=%d\n"
            "recovery_outcome_observation_view[%lu].runtime_entry_recovery_disposition_observation_view_allowed=%d\n"
            "recovery_outcome_observation_view[%lu].runtime_entry_recovery_plan_observation_view_allowed=%d\n"
            "recovery_outcome_observation_view[%lu].runtime_entry_recovery_boundary_observation_view_allowed=%d\n"
            "recovery_outcome_observation_view[%lu].runtime_entry_persistence_boundary_observation_view_allowed=%d\n"
            "recovery_outcome_observation_view[%lu].recovery_outcome_observation_allowed=%d\n"
            "recovery_outcome_observation_view[%lu].recovery_outcome_allowed=%d\n"
            "recovery_outcome_observation_view[%lu].recovery_disposition_observation_allowed=%d\n"
            "recovery_outcome_observation_view[%lu].recovery_disposition_allowed=%d\n"
            "recovery_outcome_observation_view[%lu].recovery_plan_observation_allowed=%d\n"
            "recovery_outcome_observation_view[%lu].recovery_plan_allowed=%d\n"
            "recovery_outcome_observation_view[%lu].recovery_boundary_allowed=%d\n"
            "recovery_outcome_observation_view[%lu].persistence_boundary_allowed=%d\n"
            "recovery_outcome_observation_view[%lu].persistence_commit_allowed=%d\n"
            "recovery_outcome_observation_view[%lu].persistence_allowed=%d\n"
            "recovery_outcome_observation_view[%lu].recovery_authority_allowed=%d\n"
            "recovery_outcome_observation_view[%lu].no_effect=%d\n",
            ...
            (unsigned long)i, entry->recovery_disposition_observation_slot,
            ...
            (unsigned long)i, entry->recovery_disposition_slot,
            (unsigned long)i, entry->recovery_outcome_slot,
            (unsigned long)i,
            entry->recovery_outcome_observation_view_class,
            ...
            (unsigned long)i, entry->recovery_disposition_observation_declared,
            (unsigned long)i, entry->recovery_disposition_observation_planned,
            ...
            (unsigned long)i,
            entry->runtime_entry_recovery_disposition_observation_view_allowed,
            (unsigned long)i,
            ...
            (unsigned long)i, entry->recovery_disposition_observation_allowed,
