...
static void idle_return_observation_view_copy(
...
static void seed_result(
    ...
    idle_return_observation_view_copy(
        result->idle_return_observation_view_status,
        sizeof(result->idle_return_observation_view_status), "pending");
    idle_return_observation_view_copy(result->policy_status,
    ...
    result->idle_return_observation_allowed = 0;
    result->idle_return_allowed = 0;
    result->idle_state_read_allowed = 0;
...
latticra_kernel_runtime_entry_idle_return_observation_view_default_request(
    ...
    request->requested_idle_return_observation_view_count = 4u;
    return LATTICRA_STATUS_OK;
}

static const
latticra_kernel_runtime_entry_process_return_observation_view_entry_t *
process_return_observation_view_at(
    const latticra_kernel_runtime_entry_process_return_observation_view_result_t
        *view,
    size_t index) {
    if (index < view->process_return_observation_view_count &&
        view->process_return_observation_views[index].declared) {
        return &view->process_return_observation_views[index];
    }
    return 0;
}

static unsigned long fallback_idle_return_observation_view_token(
    size_t index) {
    if (index < 4u) return (unsigned long)index;
    return 29000ul + (unsigned long)index;
}
...
static const char *class_for(size_t index) {
    switch (index) {
        ...
        case 1u:
            return "entropy-runtime-entry-idle-return-observation-view";
        case 2u:
            return "console-runtime-entry-idle-return-observation-view";
        case 3u:
            return "idle-runtime-entry-idle-return-observation-view";
        default:
            return "reserved-runtime-entry-idle-return-observation-view";
    }
}

static void fill_idle_return_observation_view(
    ...
    const
    latticra_kernel_runtime_entry_process_return_observation_view_entry_t
        *process_view = process_return_observation_view_at(view, index);

    memset(entry, 0, sizeof(*entry));
    entry->idle_return_observation_view_index = index;
    entry->idle_return_observation_view_token =
        fallback_idle_return_observation_view_token(index);
    entry->process_return_observation_view_token = process_view ?
        process_view->process_return_observation_view_token :
        fallback_process_return_observation_view_token(index);
    entry->scheduler_return_observation_view_token = process_view ?
        process_view->scheduler_return_observation_view_token :
        fallback_scheduler_return_observation_view_token(index);
    entry->post_resume_observation_view_token = process_view ?
        process_view->post_resume_observation_view_token :
        fallback_post_resume_observation_view_token(index);
    entry->user_mode_resume_view_token = process_view ?
        process_view->user_mode_resume_view_token :
        fallback_user_mode_resume_view_token(index);
    entry->syscall_exit_view_token = process_view ?
        process_view->syscall_exit_view_token :
        fallback_syscall_exit_view_token(index);
    entry->syscall_return_view_token = process_view ?
        process_view->syscall_return_view_token :
        fallback_syscall_return_view_token(index);
    entry->pid_token = process_view ? process_view->pid_token :
        4000ul + (unsigned long)index;
    entry->idle_return_observation_view_rank =
        (unsigned long)index + 1ul;
    entry->scheduler_return_observation_view_rank = process_view ?
        process_view->scheduler_return_observation_view_rank :
        (unsigned long)index + 1ul;
    entry->syscall_number = process_view ?
        process_view->syscall_number : 512ul + (unsigned long)index;
    entry->resume_slot = process_view ? process_view->resume_slot :
        4096ul + (unsigned long)index;
    entry->observation_slot = process_view ?
        process_view->observation_slot : 5120ul + (unsigned long)index;
    entry->scheduler_return_slot = process_view ?
        process_view->scheduler_return_slot : 6144ul + (unsigned long)index;
    entry->process_return_slot = process_view ?
        process_view->process_return_slot : 7168ul + (unsigned long)index;
    entry->idle_return_slot = 8192ul + (unsigned long)index;
    entry->idle_return_epoch_slot = 22ul;
    entry->idle_return_reason_slot = 23ul;
    entry->idle_state_slot = 24ul;
    entry->instruction_pointer_slot = process_view ?
        process_view->instruction_pointer_slot : 13ul;
    entry->stack_pointer_slot = process_view ?
        process_view->stack_pointer_slot : 14ul;
    entry->cpu_mode_slot = process_view ? process_view->cpu_mode_slot :
        15ul;
    entry->argument_count = process_view ? process_view->argument_count :
        6ul;
    ...
    idle_return_observation_view_copy(entry->process_label,
        sizeof(entry->process_label),
        process_view ? process_view->process_label :
            fallback_process(index));
    idle_return_observation_view_copy(
        entry->idle_return_observation_view_class,
        sizeof(entry->idle_return_observation_view_class),
        class_for(index));
    idle_return_observation_view_copy(
        entry->idle_return_observation_view_status,
        sizeof(entry->idle_return_observation_view_status),
        "declared-metadata");
    idle_return_observation_view_copy(entry->authority_status,
        sizeof(entry->authority_status),
        "runtime-entry-idle-return-observation-view-authority-denied");
    entry->declared = 1;
    entry->no_effect = 1;
    entry->evidence_level = 41u;
}

static void deny_idle_return_observation_view_authority(
    latticra_kernel_runtime_entry_idle_return_observation_view_entry_t
        *entry) {
...
static void fill_idle_return_observation_views(
    latticra_kernel_runtime_entry_idle_return_observation_view_result_t
        *result,
    size_t requested_idle_return_observation_view_count) {
    size_t count = requested_idle_return_observation_view_count;
    size_t i;
    ...
    result->idle_return_observation_view_count = count;
    for (i = 0u; i < count; ++i) {
        fill_idle_return_observation_view(
            &result->idle_return_observation_views[i],
            &result->runtime_entry_process_return_observation_view, i);
        deny_idle_return_observation_view_authority(
            &result->idle_return_observation_views[i]);
    }
}
...
latticra_kernel_runtime_entry_idle_return_observation_view_evaluate(
    const latticra_kernel_runtime_entry_idle_return_observation_view_request_t
        *request,
    latticra_kernel_runtime_entry_idle_return_observation_view_result_t
        *result) {
    ...
    if (request == 0) {
        ...
        idle_return_observation_view_copy(
            result->idle_return_observation_view_status,
            sizeof(result->idle_return_observation_view_status),
            "null-request");
        return LATTICRA_STATUS_NULL_ARGUMENT;
    }
    ...
    fill_idle_return_observation_views(result,
        request->requested_idle_return_observation_view_count);
    result->no_effect =
        result->runtime_entry_process_return_observation_view.no_effect;
    idle_return_observation_view_copy(
        result->idle_return_observation_view_status,
        sizeof(result->idle_return_observation_view_status),
        result->no_effect ?
            "runtime-entry-idle-return-observation-view-seed-ready" :
            "runtime-entry-idle-return-observation-view-seed-blocked");
...
latticra_kernel_runtime_entry_idle_return_observation_view_report(
    ...
    status = append_text(buffer, buffer_len, &used,
        ...
        "idle_return_observation_view_status=%s\n"
        "policy_status=%s\n"
        ...
        "idle_return_allowed=%d\n"
        "idle_state_read_allowed=%d\n"
        ...
        (unsigned long)result->idle_return_observation_view_count,
        result->no_effect,
        ...
        result->idle_return_observation_allowed,
        result->idle_return_allowed,
        result->idle_state_read_allowed,
    ...
    for (i = 0u; i < result->idle_return_observation_view_count; ++i) {
        const latticra_kernel_runtime_entry_idle_return_observation_view_entry_t
            *entry = &result->idle_return_observation_views[i];
        status = append_text(buffer, buffer_len, &used,
            "idle_return_observation_view[%lu].process_label=%s\n"
            "idle_return_observation_view[%lu].idle_return_observation_view_token=%lu\n"
            "idle_return_observation_view[%lu].process_return_observation_view_token=%lu\n"
            "idle_return_observation_view[%lu].scheduler_return_observation_view_token=%lu\n"
            "idle_return_observation_view[%lu].post_resume_observation_view_token=%lu\n"
            "idle_return_observation_view[%lu].user_mode_resume_view_token=%lu\n"
            "idle_return_observation_view[%lu].syscall_exit_view_token=%lu\n"
            "idle_return_observation_view[%lu].syscall_return_view_token=%lu\n"
            "idle_return_observation_view[%lu].pid_token=%lu\n"
            "idle_return_observation_view[%lu].idle_return_observation_view_rank=%lu\n"
            "idle_return_observation_view[%lu].scheduler_return_observation_view_rank=%lu\n"
            "idle_return_observation_view[%lu].syscall_number=%lu\n"
            "idle_return_observation_view[%lu].resume_slot=%lu\n"
            "idle_return_observation_view[%lu].observation_slot=%lu\n"
            "idle_return_observation_view[%lu].scheduler_return_slot=%lu\n"
            "idle_return_observation_view[%lu].process_return_slot=%lu\n"
            "idle_return_observation_view[%lu].idle_return_slot=%lu\n"
            "idle_return_observation_view[%lu].idle_return_epoch_slot=%lu\n"
            "idle_return_observation_view[%lu].idle_return_reason_slot=%lu\n"
            "idle_return_observation_view[%lu].idle_state_slot=%lu\n"
            "idle_return_observation_view[%lu].process_return_observation_view_declared=%d\n"
            "idle_return_observation_view[%lu].idle_return_observation_view_declared=%d\n"
            "idle_return_observation_view[%lu].process_return_observation_declared=%d\n"
            "idle_return_observation_view[%lu].process_return_declared=%d\n"
            "idle_return_observation_view[%lu].idle_return_observation_declared=%d\n"
            "idle_return_observation_view[%lu].idle_return_declared=%d\n"
            "idle_return_observation_view[%lu].idle_state_observation_declared=%d\n"
            "idle_return_observation_view[%lu].idle_return_observation_view_class=%s\n"
            "idle_return_observation_view[%lu].idle_return_observation_view_status=%s\n"
            "idle_return_observation_view[%lu].authority_status=%s\n"
            "idle_return_observation_view[%lu].idle_return_observation_view_planned=%d\n"
            "idle_return_observation_view[%lu].idle_return_observation_planned=%d\n"
            "idle_return_observation_view[%lu].idle_return_planned=%d\n"
            "idle_return_observation_view[%lu].process_return_observation_planned=%d\n"
            "idle_return_observation_view[%lu].process_return_planned=%d\n"
            "idle_return_observation_view[%lu].idle_state_read_planned=%d\n"
            "idle_return_observation_view[%lu].process_state_read_planned=%d\n"
            "idle_return_observation_view[%lu].scheduler_execution_planned=%d\n"
            "idle_return_observation_view[%lu].runtime_entry_idle_return_observation_view_allowed=%d\n"
            "idle_return_observation_view[%lu].runtime_entry_process_return_observation_view_allowed=%d\n"
            "idle_return_observation_view[%lu].runtime_entry_scheduler_return_observation_view_allowed=%d\n"
            "idle_return_observation_view[%lu].process_return_observation_allowed=%d\n"
            "idle_return_observation_view[%lu].process_return_allowed=%d\n"
            "idle_return_observation_view[%lu].idle_return_observation_allowed=%d\n"
            "idle_return_observation_view[%lu].idle_return_allowed=%d\n"
            "idle_return_observation_view[%lu].idle_state_read_allowed=%d\n"
            "idle_return_observation_view[%lu].process_state_read_allowed=%d\n"
            "idle_return_observation_view[%lu].context_switch_allowed=%d\n"
            "idle_return_observation_view[%lu].run_queue_mutation_allowed=%d\n"
            "idle_return_observation_view[%lu].process_wake_allowed=%d\n"
            "idle_return_observation_view[%lu].hardware_effect_allowed=%d\n"
            "idle_return_observation_view[%lu].host_effect_allowed=%d\n"
            "idle_return_observation_view[%lu].no_effect=%d\n",
            (unsigned long)i, entry->process_label,
            (unsigned long)i, entry->idle_return_observation_view_token,
            (unsigned long)i,
            ...
            (unsigned long)i, entry->idle_return_observation_view_rank,
            (unsigned long)i,
            ...
            (unsigned long)i, entry->idle_return_slot,
            (unsigned long)i, entry->idle_return_epoch_slot,
            (unsigned long)i, entry->idle_return_reason_slot,
            (unsigned long)i, entry->idle_state_slot,
            (unsigned long)i,
                entry->process_return_observation_view_declared,
            (unsigned long)i,
                entry->idle_return_observation_view_declared,
            (unsigned long)i, entry->process_return_observation_declared,
            (unsigned long)i, entry->process_return_declared,
            (unsigned long)i, entry->idle_return_observation_declared,
            (unsigned long)i, entry->idle_return_declared,
            (unsigned long)i, entry->idle_state_observation_declared,
            (unsigned long)i, entry->idle_return_observation_view_class,
            (unsigned long)i, entry->idle_return_observation_view_status,
            (unsigned long)i, entry->authority_status,
            (unsigned long)i,
                entry->idle_return_observation_view_planned,
            (unsigned long)i, entry->idle_return_observation_planned,
            (unsigned long)i, entry->idle_return_planned,
            (unsigned long)i, entry->process_return_observation_planned,
            (unsigned long)i, entry->process_return_planned,
            (unsigned long)i, entry->idle_state_read_planned,
            (unsigned long)i, entry->process_state_read_planned,
            (unsigned long)i, entry->scheduler_execution_planned,
            (unsigned long)i,
                entry->runtime_entry_idle_return_observation_view_allowed,
            (unsigned long)i,
            ...
            (unsigned long)i, entry->idle_return_observation_allowed,
            (unsigned long)i, entry->idle_return_allowed,
            (unsigned long)i, entry->idle_state_read_allowed,
