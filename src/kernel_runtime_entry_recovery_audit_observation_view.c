...
static void recovery_audit_copy(char *dst, size_t dst_len, const char *src) {
    size_t n;
...
static void seed_result(
    ...
    recovery_audit_copy(result->recovery_audit_observation_view_status,
        sizeof(result->recovery_audit_observation_view_status), "pending");
    recovery_audit_copy(result->policy_status, sizeof(result->policy_status),
        "report-only");
    result->no_effect = 1;
...
latticra_kernel_runtime_entry_recovery_audit_observation_view_default_request(
    ...
    request->requested_recovery_audit_observation_view_count = 4u;
    return LATTICRA_STATUS_OK;
}

static const
latticra_kernel_runtime_entry_recovery_closeout_observation_view_entry_t *
recovery_closeout_observation_view_at(
    const latticra_kernel_runtime_entry_recovery_closeout_observation_view_result_t
        *view,
    size_t index) {
    if (index < view->recovery_closeout_observation_view_count &&
        view->recovery_closeout_observation_views[index].declared) {
        return &view->recovery_closeout_observation_views[index];
    }
    return 0;
}
...
static const char *class_for(size_t index) {
    switch (index) {
        case 0u:
            return "kernel-report-runtime-entry-recovery-audit-observation-view";
        case 1u:
            return "entropy-runtime-entry-recovery-audit-observation-view";
        case 2u:
            return "console-runtime-entry-recovery-audit-observation-view";
        case 3u:
            return "idle-runtime-entry-recovery-audit-observation-view";
        default:
            return "reserved-runtime-entry-recovery-audit-observation-view";
    }
}
...
static void deny_recovery_audit_authority(
...
static void fill_recovery_audit_observation_view(
    latticra_kernel_runtime_entry_recovery_audit_observation_view_entry_t
        *entry,
    const latticra_kernel_runtime_entry_recovery_closeout_observation_view_result_t
        *view,
    ...
    const
    latticra_kernel_runtime_entry_recovery_closeout_observation_view_entry_t
        *closeout_view = recovery_closeout_observation_view_at(view, index);

    memset(entry, 0, sizeof(*entry));
    entry->recovery_audit_observation_view_index = index;
    entry->recovery_audit_observation_view_token =
        index < 4u ? (unsigned long)index : 37000ul + (unsigned long)index;
    entry->recovery_closeout_observation_view_token = closeout_view ?
        closeout_view->recovery_closeout_observation_view_token :
        36000ul + (unsigned long)index;
    entry->recovery_outcome_observation_view_token = closeout_view ?
        closeout_view->recovery_outcome_observation_view_token :
        35000ul + (unsigned long)index;
    entry->recovery_disposition_observation_view_token = closeout_view ?
        closeout_view->recovery_disposition_observation_view_token :
        34000ul + (unsigned long)index;
    entry->recovery_plan_observation_view_token = closeout_view ?
        closeout_view->recovery_plan_observation_view_token :
        33000ul + (unsigned long)index;
    entry->recovery_boundary_observation_view_token = closeout_view ?
        closeout_view->recovery_boundary_observation_view_token :
        32000ul + (unsigned long)index;
    entry->persistence_boundary_observation_view_token = closeout_view ?
        closeout_view->persistence_boundary_observation_view_token :
        31000ul + (unsigned long)index;
    entry->recovery_audit_observation_slot = 16384ul + (unsigned long)index;
    entry->recovery_closeout_observation_slot = closeout_view ?
        closeout_view->recovery_closeout_observation_slot :
        15360ul + (unsigned long)index;
    entry->recovery_outcome_observation_slot = closeout_view ?
        closeout_view->recovery_outcome_observation_slot :
        14336ul + (unsigned long)index;
    entry->recovery_disposition_observation_slot = closeout_view ?
        closeout_view->recovery_disposition_observation_slot :
        13312ul + (unsigned long)index;
    entry->recovery_epoch_slot = closeout_view ?
        closeout_view->recovery_epoch_slot : 30ul;
    entry->recovery_reason_slot = closeout_view ?
        closeout_view->recovery_reason_slot : 31ul;
    entry->recovery_plan_slot = closeout_view ?
        closeout_view->recovery_plan_slot : 32ul;
    entry->recovery_authority_slot = closeout_view ?
        closeout_view->recovery_authority_slot : 33ul;
    entry->recovery_audit_slot = 37ul;
    entry->recovery_closeout_slot = closeout_view ?
        closeout_view->recovery_closeout_slot : 36ul;
    entry->recovery_outcome_slot = closeout_view ?
        closeout_view->recovery_outcome_slot : 35ul;
    entry->recovery_disposition_slot = closeout_view ?
        closeout_view->recovery_disposition_slot : 34ul;
    entry->pid_token = closeout_view ? closeout_view->pid_token :
    ...
    recovery_audit_copy(entry->process_label, sizeof(entry->process_label),
        closeout_view ? closeout_view->process_label : fallback_process(index));
    recovery_audit_copy(entry->recovery_audit_observation_view_class,
        sizeof(entry->recovery_audit_observation_view_class), class_for(index));
    recovery_audit_copy(entry->recovery_audit_observation_view_status,
        sizeof(entry->recovery_audit_observation_view_status),
        "declared-metadata");
    recovery_audit_copy(entry->authority_status,
        sizeof(entry->authority_status),
        "runtime-entry-recovery-audit-observation-view-authority-denied");
    entry->declared = 1;
    entry->no_effect = 1;
    entry->evidence_level = 49u;
    deny_recovery_audit_authority(entry);
}

static void fill_recovery_audit_observation_views(
    latticra_kernel_runtime_entry_recovery_audit_observation_view_result_t
        *result,
    ...
    result->recovery_audit_observation_view_count = count;
    for (i = 0u; i < count; ++i) {
        fill_recovery_audit_observation_view(
            &result->recovery_audit_observation_views[i],
...
latticra_kernel_runtime_entry_recovery_audit_observation_view_evaluate(
    ...
    if (request == 0) {
        ...
        recovery_audit_copy(result->recovery_audit_observation_view_status,
            sizeof(result->recovery_audit_observation_view_status),
            "null-request");
        return LATTICRA_STATUS_NULL_ARGUMENT;
    }
    ...
    fill_recovery_audit_observation_views(result,
        request->requested_recovery_audit_observation_view_count);
    result->no_effect =
        result->runtime_entry_recovery_closeout_observation_view.no_effect;
    recovery_audit_copy(result->recovery_audit_observation_view_status,
        sizeof(result->recovery_audit_observation_view_status),
        result->no_effect ?
            "runtime-entry-recovery-audit-observation-view-seed-ready" :
            "runtime-entry-recovery-audit-observation-view-seed-blocked");
...
latticra_kernel_runtime_entry_recovery_audit_observation_view_report(
    ...
    status = append_text(buffer, buffer_len, &used,
        ...
        "recovery_audit_observation_view_status=%s\n"
        "policy_status=%s\n"
        ...
        "evidence_level=%u\n",
        result->recovery_audit_observation_view_status,
        ...
        (unsigned long)result->recovery_audit_observation_view_count,
        result->no_effect,
    ...
    for (i = 0u; i < result->recovery_audit_observation_view_count; ++i) {
        const
        latticra_kernel_runtime_entry_recovery_audit_observation_view_entry_t
            *entry = &result->recovery_audit_observation_views[i];
        status = append_text(buffer, buffer_len, &used,
            "recovery_audit_observation_view[%lu].process_label=%s\n"
            "recovery_audit_observation_view[%lu].recovery_audit_observation_view_class=%s\n"
            "recovery_audit_observation_view[%lu].recovery_audit_observation_view_token=%lu\n"
            "recovery_audit_observation_view[%lu].recovery_closeout_observation_view_token=%lu\n"
            "recovery_audit_observation_view[%lu].recovery_outcome_observation_view_token=%lu\n"
            "recovery_audit_observation_view[%lu].recovery_disposition_observation_view_token=%lu\n"
            "recovery_audit_observation_view[%lu].recovery_plan_observation_view_token=%lu\n"
            "recovery_audit_observation_view[%lu].recovery_boundary_observation_view_token=%lu\n"
            "recovery_audit_observation_view[%lu].persistence_boundary_observation_view_token=%lu\n"
            "recovery_audit_observation_view[%lu].recovery_audit_observation_slot=%lu\n"
            "recovery_audit_observation_view[%lu].recovery_closeout_observation_slot=%lu\n"
            "recovery_audit_observation_view[%lu].recovery_outcome_observation_slot=%lu\n"
            "recovery_audit_observation_view[%lu].recovery_disposition_observation_slot=%lu\n"
            "recovery_audit_observation_view[%lu].recovery_audit_slot=%lu\n"
            "recovery_audit_observation_view[%lu].recovery_closeout_slot=%lu\n"
            "recovery_audit_observation_view[%lu].recovery_outcome_slot=%lu\n"
            "recovery_audit_observation_view[%lu].recovery_disposition_slot=%lu\n"
            "recovery_audit_observation_view[%lu].recovery_audit_observation_declared=%d\n"
            "recovery_audit_observation_view[%lu].recovery_audit_declared=%d\n"
            "recovery_audit_observation_view[%lu].recovery_closeout_observation_declared=%d\n"
            "recovery_audit_observation_view[%lu].recovery_closeout_declared=%d\n"
            "recovery_audit_observation_view[%lu].recovery_audit_observation_planned=%d\n"
            "recovery_audit_observation_view[%lu].runtime_entry_recovery_audit_observation_view_allowed=%d\n"
            "recovery_audit_observation_view[%lu].runtime_entry_recovery_closeout_observation_view_allowed=%d\n"
            "recovery_audit_observation_view[%lu].runtime_entry_recovery_outcome_observation_view_allowed=%d\n"
            "recovery_audit_observation_view[%lu].recovery_audit_observation_allowed=%d\n"
            "recovery_audit_observation_view[%lu].recovery_audit_allowed=%d\n"
            "recovery_audit_observation_view[%lu].recovery_closeout_observation_allowed=%d\n"
            "recovery_audit_observation_view[%lu].recovery_closeout_allowed=%d\n"
            "recovery_audit_observation_view[%lu].recovery_outcome_observation_allowed=%d\n"
            "recovery_audit_observation_view[%lu].recovery_outcome_allowed=%d\n"
            "recovery_audit_observation_view[%lu].no_effect=%d\n",
            (unsigned long)i, entry->process_label,
            (unsigned long)i, entry->recovery_audit_observation_view_class,
            ...
            (unsigned long)i, entry->recovery_plan_observation_view_token,
            (unsigned long)i, entry->recovery_boundary_observation_view_token,
            (unsigned long)i, entry->persistence_boundary_observation_view_token,
            ...
            (unsigned long)i, entry->recovery_closeout_declared,
            (unsigned long)i, entry->recovery_audit_observation_planned,
            (unsigned long)i,
