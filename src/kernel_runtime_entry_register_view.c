...
static void register_view_copy(char *dst, size_t dst_len, const char *src) {
    size_t n;
...
static void seed_result(
    ...
    register_view_copy(result->register_view_status,
        sizeof(result->register_view_status), "pending");
    register_view_copy(result->policy_status, sizeof(result->policy_status),
        "report-only");
    result->no_effect = 1;
...
latticra_status_t latticra_kernel_runtime_entry_register_view_default_request(
    ...
    request->requested_register_view_count = 4u;
    return LATTICRA_STATUS_OK;
}

static const latticra_kernel_runtime_entry_frame_entry_t *frame_at(
    const latticra_kernel_runtime_entry_frame_result_t *frame,
    size_t index) {
    if (index < frame->frame_count && frame->frames[index].declared) {
        return &frame->frames[index];
    }
    return 0;
}

static unsigned long frame_value_or(
    const latticra_kernel_runtime_entry_frame_result_t *frame,
    size_t index,
    unsigned long fallback,
    unsigned long (*reader)(
        const latticra_kernel_runtime_entry_frame_entry_t *entry)) {
    const latticra_kernel_runtime_entry_frame_entry_t *entry =
...
static const char *class_for(size_t index) {
    switch (index) {
        case 0u:
            return "kernel-report-runtime-entry-register-view";
        case 1u:
            return "entropy-runtime-entry-register-view";
        case 2u:
            return "console-runtime-entry-register-view";
        case 3u:
            return "idle-runtime-entry-register-view";
        default:
            return "reserved-runtime-entry-register-view";
    }
}
...
static void fill_register_view(
    latticra_kernel_runtime_entry_register_view_entry_t *entry,
    const latticra_kernel_runtime_entry_frame_result_t *frame,
    size_t index) {
    memset(entry, 0, sizeof(*entry));
    entry->register_view_index = index;
    entry->register_view_token = fallback_register_view_token(index);
    entry->frame_token = frame_value_or(frame, index,
        fallback_frame_token(index), read_frame_token);
    entry->admission_token = frame_value_or(frame, index,
        fallback_admission_token(index), read_admission_token);
    entry->run_entry_token = frame_value_or(frame, index,
        fallback_run_entry_token(index), read_run_entry_token);
    entry->activation_token = frame_value_or(frame, index,
        13000ul + (unsigned long)index, read_activation_token);
    entry->pid_token = frame_value_or(frame, index,
        2000ul + (unsigned long)index, read_pid_token);
    entry->run_entry_rank = frame_value_or(frame, index,
        (unsigned long)index + 1ul, read_run_entry_rank);
    entry->admission_rank = frame_value_or(frame, index,
        (unsigned long)index + 1ul, read_admission_rank);
    entry->frame_rank = frame_value_or(frame, index,
        (unsigned long)index + 1ul, read_frame_rank);
    entry->register_view_rank = (unsigned long)index + 1ul;
    entry->remaining_budget_ns = frame_value_or(frame, index,
        0ul, read_remaining_budget);
    entry->register_slot_count = 16ul;
    ...
    register_view_copy(entry->process_label, sizeof(entry->process_label),
        process_for(frame, index, fallback_process(index)));
    register_view_copy(entry->register_view_class,
        sizeof(entry->register_view_class), class_for(index));
    register_view_copy(entry->register_view_status,
        sizeof(entry->register_view_status), "declared-metadata");
    register_view_copy(entry->authority_status,
        sizeof(entry->authority_status),
        "runtime-entry-register-view-authority-denied");
    entry->declared = 1;
    ...
    entry->no_effect = 1;
    entry->evidence_level = 29u;
}

static void fill_register_views(
    latticra_kernel_runtime_entry_register_view_result_t *result,
    size_t requested_register_view_count) {
    size_t count = requested_register_view_count;
    size_t i;

    if (count == 0u) count = 4u;
    if (count > LATTICRA_KERNEL_RUNTIME_ENTRY_REGISTER_VIEW_MAX) {
        count = LATTICRA_KERNEL_RUNTIME_ENTRY_REGISTER_VIEW_MAX;
    }

    result->register_view_count = count;
    for (i = 0u; i < count; ++i) {
        fill_register_view(&result->register_views[i],
            &result->runtime_entry_frame, i);
    }
}
...
latticra_status_t latticra_kernel_runtime_entry_register_view_evaluate(
    ...
    if (request == 0) {
        ...
        register_view_copy(result->register_view_status,
            sizeof(result->register_view_status), "null-request");
        return LATTICRA_STATUS_NULL_ARGUMENT;
    }
    ...
    if (status != LATTICRA_STATUS_OK) {
        ...
        register_view_copy(result->register_view_status,
            sizeof(result->register_view_status),
            "runtime-entry-frame-not-ready");
        result->no_effect = 0;
        return status;
    }

    fill_register_views(result, request->requested_register_view_count);
    result->no_effect = result->runtime_entry_frame.no_effect;
    register_view_copy(result->register_view_status,
        sizeof(result->register_view_status),
        result->no_effect ? "runtime-entry-register-view-seed-ready" :
            "runtime-entry-register-view-seed-blocked");
...
latticra_status_t latticra_kernel_runtime_entry_register_view_report(
    ...
    status = append_text(buffer, buffer_len, &used,
        ...
        "evidence_level=%u\n",
        result->register_view_status,
        result->policy_status,
        result->runtime_entry_frame.frame_status,
        result->runtime_entry_frame.runtime_entry_admission.admission_status,
        result->runtime_entry_frame.runtime_entry_admission.scheduler_run_entry.
            run_entry_status,
        (unsigned long)result->register_view_count,
        result->no_effect,
    ...
    for (i = 0u; i < result->register_view_count; ++i) {
        const latticra_kernel_runtime_entry_register_view_entry_t *entry =
            &result->register_views[i];
        status = append_text(buffer, buffer_len, &used,
            "register_view[%lu].process_label=%s\n"
            "register_view[%lu].register_view_token=%lu\n"
            "register_view[%lu].frame_token=%lu\n"
            "register_view[%lu].admission_token=%lu\n"
            "register_view[%lu].run_entry_token=%lu\n"
            "register_view[%lu].activation_token=%lu\n"
            "register_view[%lu].pid_token=%lu\n"
            "register_view[%lu].run_entry_rank=%lu\n"
            "register_view[%lu].admission_rank=%lu\n"
            "register_view[%lu].frame_rank=%lu\n"
            "register_view[%lu].register_view_rank=%lu\n"
            "register_view[%lu].remaining_budget_ns=%lu\n"
            "register_view[%lu].register_slot_count=%lu\n"
            "register_view[%lu].frame_declared=%d\n"
            "register_view[%lu].register_view_declared=%d\n"
            "register_view[%lu].instruction_pointer_declared=%d\n"
            "register_view[%lu].stack_pointer_declared=%d\n"
            "register_view[%lu].flags_declared=%d\n"
            "register_view[%lu].register_view_class=%s\n"
            "register_view[%lu].register_view_status=%s\n"
            "register_view[%lu].authority_status=%s\n"
            "register_view[%lu].register_view_planned=%d\n"
            "register_view[%lu].register_view_built=%d\n"
            "register_view[%lu].register_view_installed=%d\n"
            "register_view[%lu].register_capture_planned=%d\n"
            "register_view[%lu].register_save_planned=%d\n"
            "register_view[%lu].register_restore_planned=%d\n"
            "register_view[%lu].runtime_entry_entered=%d\n"
            "register_view[%lu].execution_entered=%d\n"
            "register_view[%lu].context_switch_planned=%d\n"
            "register_view[%lu].runtime_entry_register_view_allowed=%d\n"
            "register_view[%lu].runtime_entry_frame_allowed=%d\n"
            "register_view[%lu].runtime_entry_admission_allowed=%d\n"
            "register_view[%lu].runtime_entry_allowed=%d\n"
            "register_view[%lu].scheduler_run_entry_allowed=%d\n"
            "register_view[%lu].dispatch_allowed=%d\n"
            "register_view[%lu].run_queue_mutation_allowed=%d\n"
            "register_view[%lu].context_switch_allowed=%d\n"
            "register_view[%lu].register_save_allowed=%d\n"
            "register_view[%lu].register_restore_allowed=%d\n"
            "register_view[%lu].stack_switch_allowed=%d\n"
            "register_view[%lu].address_space_switch_allowed=%d\n"
            "register_view[%lu].hardware_effect_allowed=%d\n"
            "register_view[%lu].host_effect_allowed=%d\n"
            "register_view[%lu].no_effect=%d\n",
            ...
            (unsigned long)i, entry->register_slot_count,
            (unsigned long)i, entry->frame_declared,
            (unsigned long)i, entry->register_view_declared,
            (unsigned long)i, entry->instruction_pointer_declared,
            (unsigned long)i, entry->stack_pointer_declared,
            (unsigned long)i, entry->flags_declared,
            (unsigned long)i, entry->register_view_class,
            (unsigned long)i, entry->register_view_status,
            (unsigned long)i, entry->authority_status,
            (unsigned long)i, entry->register_view_planned,
            (unsigned long)i, entry->register_view_built,
            (unsigned long)i, entry->register_view_installed,
            (unsigned long)i, entry->register_capture_planned,
            (unsigned long)i, entry->register_save_planned,
            (unsigned long)i, entry->register_restore_planned,
