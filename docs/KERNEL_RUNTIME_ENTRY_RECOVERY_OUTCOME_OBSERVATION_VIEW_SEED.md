...
Status: no-effect runtime-entry recovery-outcome observation metadata
Scope: report-only metadata layered on the recovery-disposition observation view.

## Purpose

This slice records candidate recovery-outcome observation metadata after the recovery-disposition observation view.
...
tests/kernel_runtime_entry_recovery_outcome_observation_view.c
tools/kernel_runtime_entry_recovery_outcome_observation_view_report.c
scripts/test-kernel-runtime-entry-recovery-outcome-observation-view.sh
scripts/test-kernel-runtime-entry-recovery-outcome-observation-view-report-runner.sh
...
The default request asks for four recovery-outcome observation entries and carries the recovery-disposition, recovery-plan, recovery-boundary, persistence-boundary, quiescent-return, idle-return, and process-return observation requests forward.
...
```text
recovery_outcome_observation_view[0].process_label=kernel-report-process-metadata
recovery_outcome_observation_view[0].recovery_outcome_observation_view_class=kernel-report-runtime-entry-recovery-outcome-observation-view
recovery_outcome_observation_view[0].recovery_outcome_observation_view_token=0
recovery_outcome_observation_view[0].recovery_disposition_observation_view_token=0
recovery_outcome_observation_view[0].recovery_plan_observation_view_token=0
...
recovery_outcome_observation_view[0].recovery_epoch_slot=30
recovery_outcome_observation_view[0].recovery_reason_slot=31
recovery_outcome_observation_view[0].recovery_plan_slot=32
recovery_outcome_observation_view[0].recovery_authority_slot=33
recovery_outcome_observation_view[0].recovery_disposition_slot=34
recovery_outcome_observation_view[0].recovery_outcome_slot=35
```
