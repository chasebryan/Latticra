...
Status: no-effect runtime-entry recovery-closeout observation metadata
Scope: report-only metadata layered on the recovery-outcome observation view.

## Purpose

This slice records candidate recovery-closeout observation metadata after the recovery-outcome observation view.

It does not close out recovery, persist state, commit data, recover state, open recovery authority, enter runtime, dispatch a scheduler, mutate a run queue, wake a process, touch hardware, or affect the host.
...
tests/kernel_runtime_entry_recovery_closeout_observation_view.c
tools/kernel_runtime_entry_recovery_closeout_observation_view_report.c
scripts/test-kernel-runtime-entry-recovery-closeout-observation-view.sh
scripts/test-kernel-runtime-entry-recovery-closeout-observation-view-report-runner.sh
...
The default request asks for four recovery-closeout observation entries and carries the recovery-outcome, recovery-disposition, recovery-plan, recovery-boundary, persistence-boundary, quiescent-return, idle-return, and process-return observation requests forward.
...
```text
recovery_closeout_observation_view[0].process_label=kernel-report-process-metadata
recovery_closeout_observation_view[0].recovery_closeout_observation_view_class=kernel-report-runtime-entry-recovery-closeout-observation-view
recovery_closeout_observation_view[0].recovery_closeout_observation_view_token=0
recovery_closeout_observation_view[0].recovery_outcome_observation_view_token=0
recovery_closeout_observation_view[0].recovery_disposition_observation_view_token=0
recovery_closeout_observation_view[0].recovery_outcome_observation_slot=14336
recovery_closeout_observation_view[0].recovery_closeout_observation_slot=15360
recovery_closeout_observation_view[0].recovery_epoch_slot=30
recovery_closeout_observation_view[0].recovery_reason_slot=31
recovery_closeout_observation_view[0].recovery_plan_slot=32
recovery_closeout_observation_view[0].recovery_authority_slot=33
recovery_closeout_observation_view[0].recovery_disposition_slot=34
recovery_closeout_observation_view[0].recovery_outcome_slot=35
