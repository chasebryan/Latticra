# Debian, FreeBSD, and OpenBSD Package Build-Evidence Intake Denial Disposition Closeout Archive Gate Contract

Status: active package build-evidence intake denial disposition closeout archive gate contract
Scope: keep archive and re-request closed for the closed-out Debian, FreeBSD, and OpenBSD package build-evidence intake denial while package validation promotion remains blocked.

## Purpose

This contract records the no-effect archive gate for the closed-out Debian, FreeBSD, and OpenBSD package build-evidence intake denial disposition.

The goal is narrow: a closed-out denied build-evidence intake record is not archivable, re-requestable, promotable, or usable as package validation evidence while the shared validation promotion blocker matrix remains closed.
...
package_build_evidence_intake_denial_disposition_closeout_archive_gate_contract_present=1
...
Debian build evidence remains denied, closed out, unarchived, unaccepted, and unusable as build-lane opening evidence.
...
FreeBSD ports evidence remains denied, closed out, unarchived, unaccepted, and unusable as build-lane opening evidence.
...
OpenBSD ports evidence remains denied, closed out, unarchived, unaccepted, unusable as build-lane opening evidence, and unable to enable `PERMIT_PACKAGE`.
...
The package-build gate remains closed. This archive gate contract only records that archive and re-request stay blocked after closeout while the validation promotion blocker matrix remains closed.
...
```sh
sh scripts/test-debian-freebsd-openbsd-package-build-evidence-intake-denial-disposition-closeout-archive-gate-contract.sh
```

Expected output:

```text
debian_freebsd_openbsd_package_build_evidence_intake_denial_disposition_closeout_archive_gate_contract: ok
```
