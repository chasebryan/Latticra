...
The Readiness blocker filter now exposes a live visible-count summary, clamps query text before local storage and QA handoff replay, mirrors the filter state into the right context inspector, and records the same visible/total/filter-limit state in the no-effect receipt.
