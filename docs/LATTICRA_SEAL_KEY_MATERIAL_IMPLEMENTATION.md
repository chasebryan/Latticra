...
The next valid Latticra Seal planning slice is public-key parsing metadata implementation or another narrow status/index alignment follow-up that still must not add public-key parsing without separate key-material, public-key, and guard contracts.
