...
Current estimate mathematical rebase updates the live public estimate table to a weighted 44% overall planning estimate after recent guarded Seal, Ubuntu, macOS, Nadia, Lat/LIR, kernel lifecycle, and public-entry alignment work while preserving no implementation behavior change, no security hardening claim, no product-readiness promotion, and no runtime authority.
...
```text
Latticra Seal capability gate status/workflow guard alignment
```
