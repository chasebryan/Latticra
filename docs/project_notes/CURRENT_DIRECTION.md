...
The current Lat-to-LIR lowering implementation consumes normalized Lat model metadata directly, creates a `lat_module` LIR shape, preserves source spans, no-effect flags, model counts, and transition source indices, and emits deterministic lowering reports. The older parser-plus-semantic lowering entry point remains available as a compatibility wrapper that normalizes a local model first.
