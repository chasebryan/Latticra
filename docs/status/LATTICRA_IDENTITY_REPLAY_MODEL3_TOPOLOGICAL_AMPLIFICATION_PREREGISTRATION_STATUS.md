# Latticra Identity-Replay Model-3 Topological Amplification Pre-Registration Status

Status: guarded Model-3 topological amplification pre-registration status
Date: 2026-05-29 CDT
Scope: status checkpoint for Model-3 pre-registration and dynamic-range capacity.

## Summary

Latticra now has a target-blind Model-3 topological amplification pre-registration and a separate post-receipt capacity gate.

The status meaning is narrow. Model-3 is authorized for a prediction-only runner because the pre-registered class spans the target dynamic range, but no mass-ratio recovery or Higgs-checkmate claim is open.

## Status Fields

```text
latticra_identity_replay_model3_topological_amplification_preregistration_present=1
latticra_identity_replay_model3_topological_amplification_preregistration_guard_present=1
