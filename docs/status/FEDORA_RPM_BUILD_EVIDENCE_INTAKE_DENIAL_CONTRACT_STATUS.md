# Fedora RPM Build-Evidence Intake Denial Contract Status

Status: rpm-build-evidence-intake-denial/status alignment
Date: 2026-05-28
Scope: status record for the no-effect Fedora RPM build-evidence intake denial contract.

## Summary

Latticra now has a no-effect Fedora RPM build-evidence intake denial contract.

The contract records that Fedora `rpmbuild`, `mock`, `rpmlint`, source RPM, binary RPM, payload, install/remove, package review, Koji, Bodhi, Rawhide, and official package evidence cannot be requested, accepted, or promoted while the blocker matrix remains closed.

It does not run `rpmbuild`, `mock`, `rpmlint`, `dnf`, `fedpkg`, `koji`, `bodhi`, or `copr-cli`.

It does not request build evidence, accept build evidence, create RPM artifacts, inspect RPM artifacts, install packages, remove packages, create Bugzilla tickets, publish packages, submit to Fedora, promote validation results, or mutate a host.
