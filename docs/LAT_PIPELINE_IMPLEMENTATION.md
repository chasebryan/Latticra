...
This slice adds a no-effect Lat pipeline that composes the existing Lat parser, Lat semantic validator, and Lat-to-LIR lowering layer into one deterministic report boundary.
...
The function records parser, semantic, lowering, and LIR errors separately, then classifies the aggregate pipeline state.
...
with deterministic fields for status, aggregate error, stage errors, module name, source length, validity, counts, no-effect flags, and source spans.
...
The implementation is a composition layer. It does not modify parser, semantic validator, LIR, Lat-to-LIR lowering, L-UI, C++ authority, Nucleus, or runtime-boundary APIs.
