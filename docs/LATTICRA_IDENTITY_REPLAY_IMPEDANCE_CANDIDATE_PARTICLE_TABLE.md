...
The Model-3 Rejection Analysis uses this table only through the already emitted evaluation receipt. It derives the next target-blind requirement: sector-resolved topological charge with family monotonicity, not a particle-name lookup or target-ratio weighting layer.
