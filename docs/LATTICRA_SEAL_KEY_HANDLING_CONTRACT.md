...
After key-handling metadata, its status/public-entry checkpoint, and the key-material boundary contract exist and are guarded, the next valid planning slice is key-material metadata implementation that still must not add signing without separate implementation, key-handling, key-material, and guard contracts.
