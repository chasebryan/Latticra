...
**Role:** Project-level technical handbook for Latticra, Latticra Seal trust-boundary metadata, authority-neutral Ed25519 verification evidence, metadata-only capability/effect/handoff/report/envelope/signature-request/signing-operation classification and status guards, disposable Fedora VM CLI payload validation evidence, Latticra Console host/receipt/signature/OS/VM contract surfaces, Runtime Boundary Lat/LIR provenance records, Nucleus, Nadia Stage-46 contract-only offline-AI release/receipt/review/disposition metadata, Panel, platform install validation lanes, Lat/LIR contract surfaces, receipts, reports, and future runtime-boundary research.
