...
Status: no-effect runtime-entry recovery-boundary observation metadata
Scope: report-only metadata layered on the persistence-boundary observation view.

## Purpose

This slice records candidate recovery-boundary observation metadata after the persistence-boundary observation view.
...
tests/kernel_runtime_entry_recovery_boundary_observation_view.c
tools/kernel_runtime_entry_recovery_boundary_observation_view_report.c
scripts/test-kernel-runtime-entry-recovery-boundary-observation-view.sh
scripts/test-kernel-runtime-entry-recovery-boundary-observation-view-report-runner.sh
...
The default request asks for four recovery-boundary observation entries and carries the persistence-boundary, quiescent-return, idle-return, and process-return observation requests forward.
...
```text
recovery_boundary_observation_view[0].process_label=kernel-report-process-metadata
recovery_boundary_observation_view[0].recovery_boundary_observation_view_class=kernel-report-runtime-entry-recovery-boundary-observation-view
recovery_boundary_observation_view[0].recovery_boundary_observation_view_token=0
recovery_boundary_observation_view[0].persistence_boundary_observation_view_token=0
recovery_boundary_observation_view[0].quiescent_return_observation_view_token=0
recovery_boundary_observation_view[0].persistence_boundary_slot=10240
recovery_boundary_observation_view[0].recovery_boundary_slot=11264
recovery_boundary_observation_view[0].recovery_epoch_slot=30
recovery_boundary_observation_view[0].recovery_reason_slot=31
recovery_boundary_observation_view[0].recovery_plan_slot=32
recovery_boundary_observation_view[0].recovery_authority_slot=33
```
...
```text
kernel_runtime_entry_recovery_boundary_observation_view: ok
kernel_runtime_entry_recovery_boundary_observation_view_report_runner: ok
```
