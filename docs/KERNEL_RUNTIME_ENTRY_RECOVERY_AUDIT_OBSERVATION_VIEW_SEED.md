...
This seed records the no-effect runtime-entry recovery audit observation view.
It follows the recovery closeout observation view and keeps the OS-readiness
chain report-only.
...
The guard is `scripts/test-kernel-runtime-entry-recovery-audit-observation-view.sh`.
The report runner is
`scripts/test-kernel-runtime-entry-recovery-audit-observation-view-report-runner.sh`.
