...
The `nadia status` command reports whether the Panel configuration has selected the Nadia component and repeats the denied authority posture. The `nadia commands` command lists the Stage-1 through Stage-38 command map. Neither command launches an external host process.
...
Before Stage-39 starts, the context-window assembly contract must remain contract-only and keep context window assembly, prompt evaluation input creation, runtime handoff, runtime invocation, prompt evaluation, dialogue generation, token generation, inference, and tool execution blocked.
...
Before Stage-39 starts, Latticra should keep these guards passing:
