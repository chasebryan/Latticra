...
The signer invocation metadata implementation, status/public-entry alignment, signing operation contract, signing operation metadata implementation, signing operation status/public-entry alignment, key-handling boundary contract, key-handling metadata implementation, key-handling status/public-entry alignment, and key-material boundary contract now exist and are guarded. The next valid planning slice is key-material metadata implementation or another narrow status/index alignment follow-up that still must not add signing without separate implementation, key-handling, key-material, and guard contracts.
