...
After signing operation metadata, its status/public-entry checkpoint, the key-handling boundary contract, key-handling metadata implementation, and key-handling status/public-entry alignment exist and are guarded, the next valid planning slice is a key-material boundary contract or another narrow status/index alignment follow-up that still must not add signing without a separate implementation contract, key-handling contract, key-material contract, and guards.
