...
After signing operation metadata exists and is guarded, the next valid planning slice is signing operation status/public-entry alignment or a future key-handling contract that still must not add signing without a separate implementation contract, key-handling contract, and guards.
