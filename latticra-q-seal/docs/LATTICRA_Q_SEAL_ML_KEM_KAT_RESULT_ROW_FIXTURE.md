...
Status: KAT result row fixture
Date: 2026-05-27

This document mirrors `latticra-q-seal/evidence/ML_KEM_KAT_RESULT_ROW_FIXTURE.md` for the Q-Seal documentation set. The row fixture records the fail-closed row families required before known-answer replay outcomes can be accepted as reviewed evidence.

The current slice is a fixture contract only. It does not record result rows, load fixture bundles, execute ML-KEM, generate ACVP responses, submit ACVP material, or claim production cryptography readiness.

## Files

```text
latticra-q-seal/include/latticra/q_seal_ml_kem_kat_result_row_fixture.h
...
latticra-q-seal/scripts/test-latticra-q-seal-ml-kem-kat-result-row-fixture.sh
...
All zero-valued review, fixture-digest, row-family, CI transcript, runner-execution, response-evidence, submission, operation, production, FIPS, and runtime-authority fields must remain zero until audited evidence exists. The row fixture gives Q-Seal a typed lane for future replay outcomes without treating fixture presence as KAT passage.
...
```text
latticra q-seal ml-kem kat result row fixture invariants: ok
latticra q-seal ml-kem kat result row fixture: ok
```
