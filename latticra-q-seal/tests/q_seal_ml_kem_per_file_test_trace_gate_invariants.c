...
static int per_file_test_trace_gate_is_fail_closed(void) {
    latticra_q_seal_ml_kem_per_file_test_trace_gate_t gate;
    char rendered[LATTICRA_Q_SEAL_ML_KEM_TEST_TRACE_REPORT_MAX];

    EXPECT_TRUE(
        latticra_q_seal_ml_kem_per_file_test_trace_gate_prepare(&gate) ==
            LATTICRA_Q_SEAL_STATUS_OK,
        "prepare");
    EXPECT_TRUE(
        strcmp(
            gate.test_trace_profile,
            "latticra-q-seal-ml-kem-per-file-test-trace-gate/0.1") == 0,
        "profile");
    EXPECT_TRUE(
        strcmp(
            gate.formal_title,
            "Latticra Q-Seal ML-KEM Per-File Test Trace Gate") == 0,
        "title");
    EXPECT_TRUE(
        strcmp(
    ...
    EXPECT_TRUE(gate.per_file_test_trace_gate_present == 1u, "present");
    ...
    EXPECT_TRUE(gate.kat_runner_contract_bound == 1u, "kat runner");
    EXPECT_TRUE(gate.kat_result_schema_bound == 1u, "kat schema");
    EXPECT_TRUE(gate.kat_result_row_fixture_bound == 1u, "kat rows");
    EXPECT_TRUE(gate.acvp_contracts_bound == 1u, "acvp");
    EXPECT_TRUE(gate.vector_schema_bound == 1u, "vector schema");
    EXPECT_TRUE(gate.vector_fixture_lock_bound == 1u, "fixture lock");
    ...
    EXPECT_TRUE(gate.test_family_columns_recorded == 1u, "test columns");
    ...
    EXPECT_TRUE(gate.vector_reference_columns_recorded == 1u, "vector columns");
    EXPECT_TRUE(gate.expected_result_columns_recorded == 1u, "expected columns");
    ...
    EXPECT_TRUE(gate.per_file_test_trace_rows_recorded == 0u, "rows");
    EXPECT_TRUE(gate.kat_trace_coverage_reviewed == 0u, "kat review");
    EXPECT_TRUE(gate.acvp_trace_coverage_reviewed == 0u, "acvp review");
    EXPECT_TRUE(gate.negative_test_trace_reviewed == 0u, "negative review");
    EXPECT_TRUE(gate.trace_review_approved == 0u, "trace review");
    EXPECT_TRUE(gate.per_file_test_trace_accepted == 0u, "accepted");
    ...
    EXPECT_TRUE(gate.required_test_trace_items_total == 39u, "total");
    EXPECT_TRUE(gate.required_test_trace_items_satisfied == 32u, "satisfied");
    EXPECT_TRUE(gate.error == LATTICRA_Q_SEAL_ML_KEM_TEST_TRACE_BLOCKED, "blocked");
    EXPECT_TRUE(
        latticra_q_seal_ml_kem_per_file_test_trace_gate_is_no_effect(&gate) == 1,
        "no effect");
    EXPECT_TRUE(
        latticra_q_seal_ml_kem_per_file_test_trace_gate_allows_trace_acceptance(&gate) ==
            0,
    ...
    EXPECT_TRUE(
        ...
        "report");
    EXPECT_TRUE(
        strstr(rendered, "LATTICRA Q-SEAL ML-KEM PER-FILE TEST TRACE GATE") != 0,
        "header");
    EXPECT_TRUE(
        strstr(rendered, "per_file_test_trace_gate_present=1") != 0,
        "present report");
    EXPECT_TRUE(
        strstr(rendered, "test_family_columns_recorded=1") != 0,
        "test column report");
    EXPECT_TRUE(
        strstr(rendered, "per_file_test_trace_accepted=0") != 0,
        "accepted report");
    EXPECT_TRUE(
        strstr(rendered, "shared_secret_emission_allowed=0") != 0,
        "shared secret report");
    EXPECT_TRUE(
        strstr(rendered, "required_test_trace_items_total=39") != 0,
        "total report");
    EXPECT_TRUE(
        strstr(rendered, "status=ml-kem-per-file-test-trace-gate-blocked") != 0,
        "status");
    return 0;
}
...
static int invalid_and_buffer_paths_fail_closed(void) {
    ...
    EXPECT_TRUE(
        latticra_q_seal_ml_kem_per_file_test_trace_gate_prepare(0) ==
            LATTICRA_Q_SEAL_STATUS_NULL_ARGUMENT,
        "null prepare");
    EXPECT_TRUE(
        latticra_q_seal_ml_kem_per_file_test_trace_gate_is_no_effect(0) == 0,
        "null no effect");
    EXPECT_TRUE(
        latticra_q_seal_ml_kem_per_file_test_trace_gate_allows_trace_acceptance(0) ==
            0,
...
int main(void) {
    if (per_file_test_trace_gate_is_fail_closed() != 0) {
        return 1;
    }
    if (invalid_and_buffer_paths_fail_closed() != 0) {
        return 1;
    }
    printf("latticra q-seal ml-kem per-file test trace gate invariants: ok\n");
    return 0;
}
