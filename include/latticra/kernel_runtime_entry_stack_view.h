#ifndef LATTICRA_KERNEL_RUNTIME_ENTRY_STACK_VIEW_H
#define LATTICRA_KERNEL_RUNTIME_ENTRY_STACK_VIEW_H
...
#define LATTICRA_KERNEL_RUNTIME_ENTRY_STACK_VIEW_LABEL_MAX 64u
#define LATTICRA_KERNEL_RUNTIME_ENTRY_STACK_VIEW_MAX 8u
#define LATTICRA_KERNEL_RUNTIME_ENTRY_STACK_VIEW_REPORT_MAX 49152u
...
typedef struct {
    ...
    unsigned long stack_slot_count;
    unsigned long stack_window_bytes;
    int register_view_declared;
    int stack_view_declared;
    int stack_base_declared;
    int stack_pointer_declared;
    int stack_limit_declared;
    int guard_page_declared;
    char process_label[LATTICRA_KERNEL_RUNTIME_ENTRY_STACK_VIEW_LABEL_MAX];
    char stack_view_class[
        LATTICRA_KERNEL_RUNTIME_ENTRY_STACK_VIEW_LABEL_MAX];
    char stack_view_status[
        LATTICRA_KERNEL_RUNTIME_ENTRY_STACK_VIEW_LABEL_MAX];
    char authority_status[
        LATTICRA_KERNEL_RUNTIME_ENTRY_STACK_VIEW_LABEL_MAX];
    int declared;
    int stack_view_planned;
    int stack_view_built;
    int stack_view_installed;
    int stack_capture_planned;
    int stack_switch_planned;
    int stack_map_planned;
    int admitted;
...
} latticra_kernel_runtime_entry_stack_view_entry_t;

typedef struct {
    latticra_status_t status;
    char stack_view_status[
        LATTICRA_KERNEL_RUNTIME_ENTRY_STACK_VIEW_LABEL_MAX];
    char policy_status[LATTICRA_KERNEL_RUNTIME_ENTRY_STACK_VIEW_LABEL_MAX];
    latticra_kernel_runtime_entry_register_view_result_t
        runtime_entry_register_view;
    latticra_kernel_runtime_entry_stack_view_entry_t
        stack_views[LATTICRA_KERNEL_RUNTIME_ENTRY_STACK_VIEW_MAX];
    size_t stack_view_count;
    int no_effect;
    int runtime_entry_stack_view_allowed;
    int runtime_entry_register_view_allowed;
    int runtime_entry_frame_allowed;
    ...
    int host_effect_allowed;
    unsigned int evidence_level;
} latticra_kernel_runtime_entry_stack_view_result_t;
